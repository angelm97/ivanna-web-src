...
///////////////////////
$query_list = "select r2_id,resume_id,start_month, start_year, end_month, end_year,still_work,job_title from " .JOBSEEKER_RESUME2_TABLE . " where resume_id ='".$resume_id."' order by start_year desc ,start_month desc ";
//echo $query_list ;
...
{
 $add_save_button='<button class="btn btn-primary mb-2 mr-2" type="submit" onclick=set_action("experience_add")>Agregar nueva experiencia</button> <button class="btn btn-secondary mb-2 mr-2" type="submit" onclick=set_action("experience_add_next")>Guardar y siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'add_new_experience.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action('experience_add')")."&nbsp;&nbsp;&nbsp;".tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action('experience_add_next')");
 $add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME6."?query_string=".$query_string)."' class='btn btn-secondary mb-2' >".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
}
else
{
 $add_save_button='<button class="btn btn-primary mb-2 mr-2" type="submit" onclick=set_action("experience_add")>Agregar nueva experiencia</button> <button class="btn btn-secondary mb-2 mr-2" type="submit" onclick=set_action("experience_add_next")>Guardar y siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'add_new_experience.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action('experience_add')")."&nbsp;&nbsp;&nbsp;".tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action('experience_add_next')");
...
if($error)
{
 ...
 $reference_button='<button class="btn btn-primary mb-2 mr-2" type="submit" onclick=set_action1("reference_add")>Agregar nueva experiencia</button> <button class="btn btn-secondary mb-2 mr-2" type="submit" onclick=set_action1("reference_add_next")>Guardar y siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action1('reference_add_next')")." ".tep_image_submit(PATH_TO_BUTTON.'button_save_add_new.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action1('reference_add')");
 ...
 if($_POST['action']=="edit" || $_POST['action']=="experience_add_next")
  ...
  if($_POST['action']=="edit")
  {
   ...
   $currency     = $currency;
			...
			$description  = $description;
			$still_work   =$still_work;
   $add_save_button     = '<button class="btn btn-secondary mb-2 mr-2" type="submit">Actualizar</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
   $add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME2."?query_string=".$query_string)."' class='btn btn-secondary mb-2'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
...
else
{
 ...
 $resume1='<a class="list-group-item list-group-item-action" href ="#"  onclick="document.resume.submit()"><i class="fa fa-file-text icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_RESUME.'</a>';
		  $resume2='<a class="list-group-item list-group-item-action active" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME2.'?query_string='.$query_string).'" ><i class="fa fa-briefcase icon-resume-left active-icon" aria-hidden="true"></i> '.INFO_TEXT_LEFT_EXPERIENCE.'</a>';
    $resume3='<a class="list-group-item list-group-item-action" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME3.'?query_string='.$query_string).'" ><i class="fa fa-graduation-cap icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_EDUCATION.'</a>';
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_EXPERIENCE'         => INFO_TEXT_EXPERIENCE,
	'INFO_TEXT_EXPERIENCE1'        => year_month_experience_drop("name='experience_year' class='form-control form-control-sm'",$check1['experience_year'],"name='experience_month' class='form-control form-control-sm'",$check1['experience_month'],$required=false,$show_name=true ,$change_order=false),
...
//  'INFO_TEXT_SALARY1'            => LIST_SET_DATA(CURRENCY_TABLE,"",'code','currencies_id',"code",'name="currency" class="form-control" ',TEXT_PLEASE_SELECT,'',$currency)." ".tep_draw_input_field('salary', $salary,'size="10" class="form-control my-1"',false).'&nbsp;&nbsp;Per&nbsp;'.tep_draw_radio_field('salary_per', 'Year', '', $salary_per, 'id="salary_per1"').'&nbsp;<label for="salary_per1">'.INFO_TEXT_YEAR.'</label>&nbsp;'.tep_draw_radio_field('salary_per', 'Month', '', $salary_per, 'id="salary_per2"').'&nbsp;<label for="salary_per2">'.INFO_TEXT_MONTH.'</label>&nbsp;'.tep_draw_radio_field('salary_per', 'Hour', '', $salary_per, 'id="salary_per3"').'&nbsp;<label for="salary_per3">'.INFO_TEXT_HOUR.'</label>',
 'INFO_TEXT_SALARY1'            => "<div class='col-3'>".LIST_SET_DATA(CURRENCY_TABLE,"",'code','currencies_id',"code",'name="currency" class="form-control" ',TEXT_PLEASE_SELECT,'',$currency)."</div><div class='col-6'>".tep_draw_input_field('salary', $salary,'size="10" class="form-control my-1"',false).'</div>&nbsp;&nbsp;<div class="form-check"><span class="mr-2">Por</span> '.tep_draw_radio_field('salary_per', 'Año', '', $salary_per, 'id="salary_per1"').'&nbsp;<label for="salary_per1">'.INFO_TEXT_YEAR.'</label>&nbsp;'.tep_draw_radio_field('salary_per', 'Mes', '', $salary_per, 'id="salary_per2"').'&nbsp;<label for="salary_per2">'.INFO_TEXT_MONTH.'</label>&nbsp;'.tep_draw_radio_field('salary_per', 'Hora', '', $salary_per, 'id="salary_per3"').'&nbsp;<label for="salary_per3">'."Hora".'</label></div>',
 'INFO_TEXT_COMPANY_INDUSTRY'   => INFO_TEXT_COMPANY_INDUSTRY,
 'INFO_TEXT_COMPANY_INDUSTRY1'  => LIST_SET_DATA(JOB_CATEGORY_TABLE,"",TEXT_LANGUAGE.'category_name','id',TEXT_LANGUAGE."category_name",'class="form-control" name="company_industry" ',INFO_TEXT_PLEASE_SELECT."...",'',$company_industry),
 'INFO_TEXT_START_DATE'         => INFO_TEXT_START_DATE,
 'INFO_TEXT_START_DATE1'        => year_month_list("name='TR_start_year' class='form-control required' ",'1970',date(Y),$start_year,"name='TR_start_month' class='form-control required'",$start_month,true,true,true),
 ...
 'INFO_TEXT_END_DATE1'          => year_month_list("name='TR_end_year' class='form-control required' ",'1970',date(Y),$end_year,"name='TR_end_month' class='form-control required'",$end_month,true,true,true),

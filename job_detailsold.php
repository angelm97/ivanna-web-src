...
//	$button_apply='<td>'.tep_image_button(PATH_TO_BUTTON.'re_apply.gif',"Re-Apply Now",'onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'\'"').'</td>';
	$button_apply='<span class="btn btn-sm btn-success">Already Applied</span>';
  else
	$button_apply='<input type="button" name="apply" value="Apply Now" class="btn btn-primary mr-2" onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'\'">';
}
else
$button_apply='<input type="button" name="apply" value="Login to apply" class="btn btn-primary mb-2 mr-2" onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'\'">
						<input type="button" name="apply" value="Register and apply" class="btn btn-success" onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_REGISTER1,'job='.$query_string).'\'">'.($row['recruiter_applywithoutlogin']=='Yes'?
'<input type="button" name="apply" value="Apply without login" class="btn btn-sm btn-success ml-2" onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOLOGIN,'query_string='.$query_string).'\'">':'');
...
//$job_skill = getSkillTagLink ($row['job_skills']);
		...
		if(tep_not_null($row['job_skills']))
		{
		 ...
		 $count_job_skills=count($job_skills1);
			 for($i=0;$i<$count_job_skills;$i++)
			 {
				...
				$final_skills.=tep_draw_form('search_skill'.$i, FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('keyword',$job_skills1[$i]).'<a class="badge badge-secondary mr-1 customMargin" href="#" onclick="document.search_skill'.$i.'.submit()">'.$job_skills1[$i].'</a></form>';
...
'form_nologin'=>tep_draw_form('apply_nologin', FILENAME_JOB_DETAILS,'','post'),
 ...
 'company_logo'=>$company_logo,
  ...
  'location_search'=>LIST_TABLE(COUNTRIES_TABLE,TEXT_LANGUAGE."country_name","priority","name='country' class='form-control mb-2'","All Location","",DEFAULT_COUNTRY_ID),
  ...
  'jobs_by_location'=>'<a href="'.tep_href_link(FILENAME_JOB_SEARCH_BY_LOCATION).'"  title="'.INFO_TEXT_L_BY_LOCATION.'">'.INFO_TEXT_L_BY_LOCATION.'</a>',
   'jobs_by_map'=> (GOOGLE_MAP=='true'?'<li><a href="'.tep_href_link(FILENAME_JOB_BY_MAP).'" title="'.INFO_TEXT_L_BY_MAP.'">'.INFO_TEXT_L_BY_MAP.'</a></li>':''),
...
else
{
 ...
 if(tep_not_null($row['indeed_id']) || tep_not_null($row['zr_id']) )

...
if($article_id=="" && $cat_Path=="")
{
 ...
 if(tep_db_num_rows($article_query) > 0)
 {
  ...
  while ($article = tep_db_fetch_array($article_query))
  {
   ...
   $article_image=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_ARTICLE_PHOTO.$article["article_photo"]."&size=200",'','','','align="center" class="card-img-top"');
			$article_url=tep_href_link(get_display_link($ide,$seo_name));
   $template->assign_block_vars('article', array(
    
    'article_name' =>'<h6 class="mt-0"><a href="'.$article_url.' " class="article_title">'.tep_db_output($article['title']).'</a></h6>
                      
                      <span class="text-muted my-2 d-block"><i class="fa fa-pencil-square" aria-hidden="true"> '
                        .tep_db_output($article[TEXT_LANGUAGE.'category_name']).'</i>
                      </span>
                      
                      <div align="justify" class=" for-mobile text-muted">'
                      .nl2br(stripslashes(strip_tags($article['short_description']))).
                      '</div>',
    
    'article_image' =>$article_image,
    
    'more_article'  =>'<a class="font30" href="'.$article_url.'"class="style20" >'.INFO_TEXT_MORE.'&gt;&gt;</a>',
    ));
  }
...
else if($cat_Path!="")
{
 ...
 if(tep_db_num_rows($article_query) > 0)
 {
  ...
  while ($article = tep_db_fetch_array($article_query))
  {
   ...
   $article_image='';
   if(tep_not_null($article["article_photo"]) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_ARTICLE_PHOTO.$article["article_photo"]))
   $article_image=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_ARTICLE_PHOTO.$article["article_photo"]."&size=200",'','','','class="mr-3 ml-0 img-thumbnail img-hover logo-dashboard mobile-margin-bottom mobile-full-img"');
   $seo_name    = $article["seo_name"];
   $article_url = tep_href_link(get_display_link($ide,$seo_name));
...
//    'article_name' => '&#8226&nbsp;<a href="'.tep_href_link('article_'.$ide.'.html').'">'.tep_db_output($article['title']).'</a></li>',
     'article_name' =>'<h5 class="mt-0"><a href="'.$article_url.' " class="article_title">'
...
{
  $article_categories.="
  <a class='border px-3 py-2 radius mr-3 bg-blue text-center' style='border-radius:50px;' href='".tep_href_link('article_cat_'.$category['id'].'.html')."' title='".$category[TEXT_LANGUAGE.'category_name']."'>".ucfirst($category[TEXT_LANGUAGE."category_name"])."</a>

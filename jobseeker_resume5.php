 ...
 switch($action)
 {
   ...
   case 'edit':
    $resume_photo_check=getAnyTableWhereData(JOBSEEKER_RESUME1_TABLE,"jobseeker_id='".$_SESSION['sess_jobseekerid']."' and  resume_id ='".$resume_id."'","jobseeker_resume,jobseeker_photo");
     ...
     {
      if($obj_resume = new upload('my_resume', PATH_TO_MAIN_PHYSICAL_RESUME.$resume_directory.'/','644',array('doc','pdf','txt','docx','png','jpg','HEIC')))
      {
...
else
{
 /*********************addition of photo*******************/
     ...
     $resume="&nbsp;&nbsp;[&nbsp;&nbsp;<a href='".tep_href_link(FILENAME_JOBSEEKER_RESUME_DOWNLOAD,(tep_not_null($resume_id)?'query_string='.$query_string1:''))."'>Download</a>&nbsp;&nbsp;]";
		...
		if(tep_not_null($photo))
		{
			if(is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$photo))
			{
				...
				$photo="&nbsp;&nbsp;[&nbsp;&nbsp;<a href='#' onclick=\"javascript:popupimage('".PATH_TO_PHOTO.$photo."','')\">Preview</a>&nbsp;&nbsp;]  <a href='".tep_href_link(FILENAME_JOBSEEKER_RESUME5,'query_string='.$query_string.'&action1=delete_photo')."' onclick='return confirm(\"Do you want to delete this photo\");'><u>delete this</u></a>";
   ...
   $upload_button='<button class="btn btn-primary" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
		...
		{
  	$upload_button='<button class="btn btn-primary" type="submit">Siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
		}
	}
}
 /*********************************************************/
$query_string       = encode_string("resume_id@@@".$resume_id."@@@resume");
$add_next_button2    = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_RESUMES)."' class='btn btn-skip-next'>Omitir el paso</a>";
 ...
 $resume1='<a class="list-group-item list-group-item-action" href ="#"  onclick="document.resume.submit()"><i class="fa fa-file-text icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_RESUME.'</a>';
    ...
    $resume3='<a class="list-group-item list-group-item-action" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME3.'?query_string='.$query_string).'" ><i class="fa fa-graduation-cap icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_EDUCATION.'</a>';
		  ...
		  $resume5='<a class="list-group-item list-group-item-action active" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME5.'?query_string='.$query_string).'" ><i class="fa fa-upload icon-resume-left active-icon" aria-hidden="true"></i> '.INFO_TEXT_LEFT_UPLOAD.'</a>';
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_UPLOAD_RESUME1'        => tep_draw_file_field("my_resume").$resume.'&nbsp;'.INFO_TEXT_UPLOAD_RESUME_HELP,

 'INFO_TEXT_UPLOAD_PHOTO'          => INFO_TEXT_UPLOAD_PHOTO,
 'INFO_TEXT_PHOTO1'                => tep_draw_file_field("my_photo").$photo.'&nbsp;'.INFO_TEXT_UPLOAD_PHOTO_HELP,

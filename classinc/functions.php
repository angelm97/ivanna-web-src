                         ...
                         $startYear="", $endYear="", $required=false,$display_name=false)
{
	...
	$datelist.= '<div class="col-md-3 mb-3"><select '.$day_parameter.'><option value="">'.INFO_TEXT_DAY.'</option>';
	$ct=0;
	for($ct=1;$ct<=31;$ct++)
	{
		if(strlen($ct)<2)
	...
	$datelist.="</select></div>";
	// $datelist.=" - ";
	$datelist.='<div class="col-md-3 mb-3"><select '.$month_parameter.'><option value="">'.INFO_TEXT_MONTH1.'</option>';
	$ct=1;
 $month_array=array("01"=>INFO_TEXT_JANUARY,"02"=>INFO_TEXT_FEBRUARY,"03"=>INFO_TEXT_MARCH,
	...
	$datelist.='</select></div>';
	// $datelist.="-";
	$datelist.= '<div class="col-md-3 mb-3"><select '.$year_parameter.'><option value="">'.INFO_TEXT_YEAR1.'</option>';
	$ct=0;
	if($startYear=="")
	{
		$s_yy=(date('Y')-80);
...
function JOB_TYPE($checked="")
{
 ...
 $checked=explode(",",$checked);
	while($row = tep_db_fetch_array($result))
	{
		...
		$JOB_TYPE_STRING.="<td>";
		$JOB_TYPE_STRING.=tep_draw_checkbox_field('job_type[]', $cid, (in_array($cid,$checked)?true:false), '', 'id="job_type_checkbox'.$cid.'"');
		$JOB_TYPE_STRING.="</td>\n<td class='small'><label for='job_type_checkbox".$cid."' onMouseOver=\"this.style.color='#0000ff'\" onMouseOut=\"this.style.color='#000080'\">".$c."</label></td>\n";
		if($i % 3==0)
		{
			$JOB_TYPE_STRING.="</tr>\n<tr>\n";
		}

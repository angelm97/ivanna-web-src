...
$form1='<form name="checkout_confirmation1" target="myNewWin" action="'.tep_href_link(FILENAME_JOBSEEKER_CHECKOUT_CONFIRMATION, 'product_id='.$product_id."&action1=print", 'SSL').'" method="post">'.($_SESSION['sess_gift']?$hidden_fields:'').tep_draw_hidden_field('payment',$payment).tep_draw_hidden_field('comments',$_POST['comments']);
if (is_array($payment_modules->modules)) 
{
...
$template->assign_vars(array(
 ...
 'form'=>$form,
 'form1'=>$form1,
 'button'=>'<a href="' . tep_href_link(FILENAME_JOBSEEKER_CHECKOUT_PAYMENT, 'product_id='.$product_id, 'SSL') . '" class="btn btn-secondary">'
 .IMAGE_BUTTON_BACK.'</a>'.'&nbsp;&nbsp;'
...
//  'button_print'=>tep_image_button(PATH_TO_BUTTON.'button_print.gif',IMAGE_BUTTON_PRINT,'onClick="print_page();"'),
 'button_print'=>tep_button_submit('btn btn-secondary','Print','onClick="print_page();"'),

  ...
  ///only for sorting ends
   ...
   if($x > 0 && $x1 > 0)
   {
    ...
    while ($topic = tep_db_fetch_array($topic_query)) 
    {
     ...
     $topic_name   = "<a href='".tep_href_link(PATH_TO_FORUM.encode_forum($key).'_'.$topic['id'].'.html')."'  title='".tep_db_output($key1)."' class='forum_title1'>".tep_db_output($key)."</a>";
...
'INFO_TEXT_FORUM_HEADER_LINK'  => $forum_header_link,
 'INFO_TEXT_HEADING_SEARCH' => '<a class="btn btn-sm btn-primary" href="'.tep_href_link(PATH_TO_FORUM.FILENAME_FORUM_SEARCH).'">'.tep_db_output(INFO_TEXT_HEADING_SEARCH).'</a>',

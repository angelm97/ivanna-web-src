...
$search_within_array[]= array('id'=>'','text'=>'Todos los títulos y descripciones ');
$search_within_array[]= array('id'=>'title','text'=>'Solo títulos');
$search_within_array[]= array('id'=>'description','text'=>'Solo descripciones');

$search_posted_array  = array();
$search_posted_array[]= array('id'=>'','text'=>'Todas las publicaciones');
$search_posted_array[]= array('id'=>'0','text'=>'Hoy');
$search_posted_array[]= array('id'=>'1','text'=>'Ayer');
$search_posted_array[]= array('id'=>'7','text'=>'Última semana');
$search_posted_array[]= array('id'=>'14','text'=>'Últimas 2 semanas ');
$search_posted_array[]= array('id'=>'21','text'=>'Últimas 3 semanas');
$search_posted_array[]= array('id'=>'30','text'=>'Últimos 30 dias');
$search_posted_array[]= array('id'=>'90','text'=>'Últimos 90 dias');
$search_posted_array[]= array('id'=>'custom','text'=>'Búsqueda personalizada');
  ...
  if($x > 0 && $x1 > 0)
  {
   ...
   while ($result_row = tep_db_fetch_array($query_result))
   {
    ...
    $template->assign_block_vars('search_result', array(
                                 ...
                                 'title'        => '<a href="'.$post_link.'"  title="'.tep_db_input($key1).'" class="forum_title"  target="_blank">'.tep_db_output($key).'</a>',
...
else
{
 ...
 $template->assign_vars(array(
  ...
  'INFO_TEXT_KEAYWORD1'        => tep_draw_input_field('keyword',$keyword,'size="45" class="form-control"'),
  'INFO_TEXT_KEYWORD_CRITERIA' => INFO_TEXT_KEYWORD_CRITERIA,
  'INFO_TEXT_KEYWORD_CRITERIA1'=> tep_draw_radio_field('word1', 'Yes', '', $word1,'id=radio_word1').'<label for="radio_word1">'.INFO_TEXT_KEYWORD_WORD1.'</label>'.tep_draw_radio_field('word1', 'No', true, $word1,'id=radio_word2').'<label for="radio_word2">'.INFO_TEXT_KEYWORD_WORD2.'</label>',
  'INFO_TEXT_SEARCH_WITHIN'    => INFO_TEXT_SEARCH_WITHIN, 
  'INFO_TEXT_SEARCH_WITHIN1'   => tep_draw_pull_down_menu('search_within', $search_within_array, $search_within,'class="form-control"'),
  'INFO_TEXT_SEARCH_CATEGORY'  => INFO_TEXT_SEARCH_CATEGORY,
  'INFO_TEXT_SEARCH_CATEGORY1' => LIST_SET_DATA(FORUM_CATEGORIES_TABLE,"",'category_name','id',"category_name","name='category' class='form-control'" ,'Todas ','',$category),
...
//  'INFO_TEXT_SEARCH_FORUM1'    => LIST_TABLE2(FORUM_TABLE.'as f',"where is_show ='Yes' and show_date <= curdate()",'title','id',"title","name='forum_id'" ,'All ','',$forum_id),
  'INFO_TEXT_SEARCH_FORUM1'    => LIST_TABLE2(0,'',"",'forum_title','category_name','id',$order_by='',$addoption_value="",$addstart="" ,$addmiddle=" (",$addend=")", $forum_query,"name='forum_id' class='form-control'","Todas","",$selected="",$footer="",$footer_value=""),
 
  'INFO_TEXT_FORUM_POSTED'     => INFO_TEXT_FORUM_POSTED,
  'INFO_TEXT_FORUM_POSTED1'    => tep_draw_pull_down_menu('posted', $search_posted_array, $posted,'onchange="show_custom_search();" id="search_time" class="form-control"'),
  'INFO_TEXT_SEARCH_FROM_DATE' => datelisting($start_date,'name="s_date" class="form-control"','name="s_month" class="form-control"','name="s_year" class="form-control"','2009',date('Y'),'','',false),
  'INFO_TEXT_SEARCH_TO_DATE'   => datelisting($end_date,'name="e_date" class="form-control"','name="e_month" class="form-control"','name="e_year" class="form-control"','2009',date('Y'),'','',false),
  ...
  'button'=>tep_button_submit('btn btn-primary','Search'),
  'update_message'=> $messageStack->output()));

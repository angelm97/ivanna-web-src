...
$i=1;
$forum_row_class=' class="forum_row_content"';
$forum_row_class1=' class="forum_row_content1"';
if($x>0)
{
 while($row11 = tep_db_fetch_array($result))
 {
  ...
  $forum_category='<a href="'.tep_href_link(PATH_TO_FORUM,'category='.$ide).'" class="forum_heading">'.tep_db_output($key).'</a>';
  ...
  if($total_forum)
  {
   ...
   while($forum_row = tep_db_fetch_array($forum_result))
   {
    ...
    $forum.='<tr class="forum_row">
              <td width="35"  '.$forum_row_class.' ><div align=""><i class="fa fa-comments forum-icon-bg p-3 mr-2" aria-hidden="true"></i></div></td>
              <td  valign="top" '.$forum_row_class.'>
               <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                 <td class="forum_title"><a href="'.$forum_link.'" class="forum_title">'.$forum_row['title'].'</a></td>
                </tr>
                <tr>
                 <td class="forum_content">'.$forum_desc.'</td>
                </tr>
               </table>
              </td>
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_HEADING_SEARCH' => '<a class="btn btn-sm btn-primary" href="'.tep_href_link(PATH_TO_FORUM.FILENAME_FORUM_SEARCH).'" class="forum_heading_link">'.tep_db_output(INFO_TEXT_HEADING_SEARCH).'</a>',

...
if($row['post_url']=='Yes')
{
 ...
 $post_url='http://'.$post_url;
	$button_apply='<input type="button" name="apply" value="Aplica Ahora" class="btn btn-primary" onclick="location.href=\''.$post_url.'\'">';
}
...
//	$button_apply='<td>'.tep_image_button(PATH_TO_BUTTON.'re_apply.gif',"Re-Aplica Ahora",'onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'\'"').'</td>';
	$button_apply='<span class="btn btn-sm btn-success">Ya plicaste a esta oportunidad!</span>';
  else
	$button_apply='<input type="button" name="apply" value="Aplica Ahora" class="btn btn-primary mr-2" onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'\'">';
}
else
$button_apply='<input type="button" name="apply" value="Inicia sesión para aplicar" class="btn btn-primary mb-2 mr-2" onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'\'">
						<input type="button" name="apply" value="Registrate y aplica" class="btn btn-success mb-2" onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_REGISTER1,'job='.$query_string).'\'">'.($row['recruiter_applywithoutlogin']=='Yes'?
'<input type="button" name="apply" value="Apply without login" class="btn btn-success mb-2 ml-2" onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOLOGIN,'query_string='.$query_string).'\'">':'');
...
if(tep_not_null($row['indeed_id']))
{
 $template->assign_vars(array(
                               ...
                               'INFO_TEXT_JOB_DETAILS1'=>nl2br(stripslashes($row['job_short_description'])).'<div align="right" ><a href="'.$row['indeed_url'].'" target="_blank" class="red">Mas >></a></div><br>',
...
{
$button_apply='<input type="button" name="apply" value="Aplica Ahora" class="btn btn-primary" onclick="location.href=\''.$row['zr_url'].'\'">';
...
{
$button_apply='<input type="button" name="apply" value="Aplica Ahora" class="btn btn-primary" onclick="location.href=\''.$row['usajobs_url'].'\'">';

 $template->assign_vars(array(
                               //'INFO_TEXT_JOB_DETAILS1'=>'<div id="outteriframe"><iframe src="'.$row['indeed_url'].'" scrolling="no" id="inneriframe"></iframe></div>',
                               'INFO_TEXT_JOB_DETAILS1'=>nl2br(stripslashes($row['job_description'])).'<div align="right" ><a href="'.$row['usajobs_url'].'" target="_blank" class="red">Mas >></a></div><br>',
                               'button'=>'<span id="job_detail_back"><a href="javascript:history.back();">'.INFO_TEXT_BACK.'</a>&nbsp;|&nbsp;</span><a href="'.tep_href_link($job_id.'/'.$title_format.'.html','action=save').'">'.INFO_TEXT_SAVE.'</a>&nbsp;|&nbsp;<a href="'.tep_href_link(FILENAME_TELL_TO_FRIEND,'query_string='.$query_string).'">'.INFO_TEXT_TELL_TO_FRIEND.'</a>',
                              ));
}
else
{
 $template->assign_vars(array(
   ...
   'button'=>'<a class="btn btn-outline-secondary" href="'.tep_href_link($job_id.'/'.$title_format.'.html','action=save').'"><i class="fa fa-floppy-o" title="Save" aria-hidden="true"></i></a>

   <a class="btn btn-outline-secondary" href="'.tep_href_link(FILENAME_TELL_TO_FRIEND,'query_string='.$query_string).'"><i class="fa fa-share-square" title="Send to Friend" aria-hidden="true"></i></a>

   <a class="btn btn-outline-secondary" href="#" onclick="popUp(\''.tep_href_link($job_id.'/'.$title_format.'.html','action=print').'\')"><i class="fa fa-print" title="Print" aria-hidden="true"></i></a>',
 ));
}
...
'form_nologin'=>tep_draw_form('apply_nologin', FILENAME_JOB_DETAILS,'','post'),
 ...
 'company_logo'=>$company_logo,
  ...
  'keyword_search'=>tep_draw_input_field('keyword','','placeholder="Ej. Ejecutivo de ventas" type="text" class="form-control mb-2"',false),
  ...
  'search_button'=>'<input type="submit" name="login2" id="login2" value="Buscar" class="btn btn-primary btn-block mb-2" />',
  'advance_search'=>'<a href="'.tep_href_link(FILENAME_JOB_SEARCH).'">Busqueda Avanzada</a>',
  ...
  'jobs_by_location'=>'<a href="'.tep_href_link(FILENAME_JOB_SEARCH_BY_LOCATION).'"  title="'.INFO_TEXT_L_BY_LOCATION.'">'.INFO_TEXT_L_BY_LOCATION.'</a>',
   ...
   'one_week'=> tep_draw_form('week1_form', FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('job_post_day','7').'<i class="fa fa-angle-right" aria-hidden="true"></i> <a href="#" onclick="document.week1_form.submit()"  title="'.INFO_TEXT_LAST_ONE_WEEK.'">'.INFO_TEXT_LAST_ONE_WEEK.'</a></form>',
   'two_week'=> tep_draw_form('week2_form', FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('job_post_day','14').'<i class="fa fa-angle-right" aria-hidden="true"></i> <a href="#" onclick="document.week2_form.submit()"  title="'.INFO_TEXT_LAST_TWO_WEEKS.'">'.INFO_TEXT_LAST_TWO_WEEKS.'</a></form>',
   'three_week'=> tep_draw_form('week3_form', FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('job_post_day','21').'<i class="fa fa-angle-right" aria-hidden="true"></i> <a href="#" onclick="document.week3_form.submit()"  title="'.INFO_TEXT_LAST_THREE_WEEKS.'">'.INFO_TEXT_LAST_THREE_WEEKS.'</a></form>',
   'one_month'=> tep_draw_form('week4_form', FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('job_post_day','30').'<i class="fa fa-angle-right" aria-hidden="true"></i> <a href="#" onclick="document.week4_form.submit()"  title="'.INFO_TEXT_LAST_ONE_MONTH.'">'.INFO_TEXT_LAST_ONE_MONTH.'</a></form>',
 ...
 'INFO_TEXT_JOB_POSTED1'=>tep_date_long(tep_db_output($row['re_adv'])),

...
#################FEATURED EMPLOYER############################
$featured_emp=banner_display("7",12,130,"class='img-fluid img-thumbnail theme6-featured mr-2 mb-2'","class='img-fluid img-thumbnail theme6-featured mr-2 mb-2'");
//$featured_emp =array(1,2);
//print_r($featured_emp);die();
for($i=0;$i<count($featured_emp);$i=$i+5)
{
...
/*------------------*/
 ...
 //$key=((strlen($row11['category_name'])<30)?$row11['category_name']:substr($row11['category_name'],0,27)."...");
	...
	$job_category.="<div style='padding: 5px 0;'><a href='".$ide.'/'.encode_category($key1)."-jobs.html"."'  title='".tep_db_output($key1)."'>".tep_db_output($key)."</a>".$total_jobs."</div>";
	if($i%10 == 0)
	{
...
$description=(tep_not_null(strlen($row['job_short_description']))>100?substr($row['job_short_description'],0,98).'..':$row['job_short_description']);
 $title=' <a class="" href="'.tep_href_link($ide.'/'.$title_format.'.html').'" target="_blank">'.$row['job_title'].'</a>';
...
//////////////////// CAREER TOOLS STARTS ///////////////////
$now=date("Y-m-d H:i:s");
$query = "select a.id,a.title,a.short_description,a.article_photo  from ".ARTICLE_TABLE." as a  where a.show_date <='$now' and a.is_show='Yes'  order by rand() limit 0,6";
//echo "<br>$query";//exit;
...
while($article = tep_db_fetch_array($result1))
{
 ...
 $title='<a class="" href="article_'.$ide.'.html"  target="_blank">'.$article['title'].'</a>';
  $article_image='';
  if(tep_not_null($article["article_photo"]) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_ARTICLE_PHOTO.$article["article_photo"]))
    ...
    $article_image='<!--<a href="article_'.$ide.'.html" target="_blank">'.tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_ARTICLE_PHOTO."blank_com.gif&size=58",'','','','').'</a>-->';
 	$description=((strlen($article['short_description'])<0)?$article['short_description']:substr($article['short_description'],0,0)."");
$MORE='<!--<a href="article_'.$ide.'.html"  target="_blank"><span >...<u>more&gt;&gt;</u></span></a>-->';
$articles1.='<div class="">'.$title.'</div><div class="mb-3">'.tep_db_output($description).''.$MORE.' </span></div>
																				';
/*  if($count%2 == 0)
		{
...
if(check_login("jobseeker"))
{
    ...
    $INFO_TEXT_ALERT_TEXT=$save_search.(($action1=='save_search')?'':"<a href='#' onclick='document.save_search.submit();' class='btn btn-sm btn-dark btn-block py-2 mt-2'>Create Job Alert</a></form>");
}
else
{
	 ...
	 $INFO_TEXT_ALERT_TEXT=$save_search.''.tep_draw_input_field('TREF_job_alert_email', $TREF_job_alert_email,'class="form-control" placeholder="Email Address" ',false).''."<button type='submit' class='btn btn-sm btn-dark btn-block py-2 mt-2'>Create Job Alert</button></form>";
}
...
'JOBSEEKER_REGISTER'=>'<a href="'.tep_href_link(FILENAME_JOBSEEKER_REGISTER1).'" class="readmore button">Register Now!</a>',
...
'REC_REGISTER'=>'<a href="'.tep_href_link(FILENAME_RECRUITER_REGISTRATION).'" class="readmore button">Register Now!</a>',
...
'REC_SEARCH_RESUME'=>'<a href="'.tep_href_link(FILENAME_RECRUITER_SEARCH_RESUME).'">Find Resume</a>',
 'ALL_JOBS'=>tep_draw_form('all_jobs', FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').'<a onclick="document.all_jobs.submit()" class="btn-text">View all</a></form>',
'POST_RES'=>'<button onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_RESUME1).'\'" type="submit" class="btn btn-dark">Post Resume</button>',
 'ALL_CATEGORY'=>'<a href="'.tep_href_link(FILENAME_JOB_SEARCH_BY_INDUSTRY).'" class="btn-text">View all</a>',
'JOB_COMPANY'=> $job_company,
 'ALL_COMPANY'=>'<a href="'.tep_href_link(FILENAME_JOBSEEKER_COMPANY_PROFILE).'" class="btn-text">View all</a>',
 'ALL_ARTICLES'=> '<a href="'.tep_href_link(FILENAME_ARTICLE).'" class="btn-text">View all</a>',
 'POST_RESUME_MIDDLELINK'=>(check_login("jobseeker")?'<button type="button" class="btn btn-dark button" onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_RESUME1).'\'">Post Resume</button>':'<button type="button" class="btn btn-dark button" onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_LOGIN).'\'">Post Resume</button>'),
 'VIEW_ALL_EMPLOYER'=>'<a href="'.tep_href_link(FILENAME_JOBSEEKER_COMPANY_PROFILE).'" class="btn-text">View all</a>',
 'LEFT_BOX_WIDTH'=> '',

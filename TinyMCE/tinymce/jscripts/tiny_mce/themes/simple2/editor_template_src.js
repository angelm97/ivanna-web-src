...
var TinyMCE_Simple2Theme = {
	...
	getControlHTML : function(button_name) {
		...
		for (i=0; i<TinyMCE_Simple2Theme._buttons.length; i++) {
			var but = TinyMCE_Simple2Theme._buttons[i];
			...
			case "mceLink":
				...
				if (inst.settings['insertlink_callback']) {
					...
					if (returnVal && returnVal['href'])
						TinyMCE_Simple2Theme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class']);
				} else {
			...
			case "mceImage":
				...
				if (inst.settings['insertimage_callback']) {
					...
					if (returnVal && returnVal['src'])
						TinyMCE_Simple2Theme._insertImage(returnVal['src'], returnVal['alt'], returnVal['border'], returnVal['hspace'], returnVal['vspace'], returnVal['width'], returnVal['height'], returnVal['align'], returnVal['title'], returnVal['onmouseover'], returnVal['onmouseout']);
				} else
	...
	getEditorTemplate : function(settings, editorId) {
		...
		if (settings['theme_advanced_styles']) {
			...
			TinyMCE_Simple2Theme._autoImportCSSClasses = false;
		}
	...
	initInstance : function(inst) {
		if (tinyMCE.getParam("theme_advanced_resizing", false)) {
			if (tinyMCE.getParam("theme_advanced_resizing_use_cookie", true)) {
				var w = TinyMCE_Simple2Theme._getCookie("TinyMCE_" + inst.editorId + "_width");
				var h = TinyMCE_Simple2Theme._getCookie("TinyMCE_" + inst.editorId + "_height");

				TinyMCE_Simple2Theme._resizeTo(inst, w, h, tinyMCE.getParam("theme_advanced_resize_horizontal", true));
			}
		}
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		if (selectElm) {
			TinyMCE_Simple2Theme._setupCSSClasses(editor_id);
	...
	_setupCSSClasses : function(editor_id) {
		...
		if (!TinyMCE_Simple2Theme._autoImportCSSClasses)
			return;
	...
	_resizeEventHandler : function(e) {
		var resizer = TinyMCE_Simple2Theme._resizer;
		...
		switch (e.type) {
			...
			case "mouseup":
				TinyMCE_Simple2Theme._setResizing(e, editorId, false);
				TinyMCE_Simple2Theme._resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
				...
				if (tinyMCE.getParam("theme_advanced_resizing_use_cookie", true)) {
					...
					// Set the cookies
					TinyMCE_Simple2Theme._setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
					TinyMCE_Simple2Theme._setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
				}
				break;
		}
	},
	...
	_setResizing : function(e, editor_id, state) {
		...
		if (state) {
			...
			if (!resizer.eventHandlers) {
				if (tinyMCE.isMSIE)
					tinyMCE.addEvent(document, "mousemove", TinyMCE_Simple2Theme._resizeEventHandler);
				else
					tinyMCE.addEvent(window, "mousemove", TinyMCE_Simple2Theme._resizeEventHandler);

				tinyMCE.addEvent(document, "mouseup", TinyMCE_Simple2Theme._resizeEventHandler);
...
tinyMCE.addTheme("advanced", TinyMCE_Simple2Theme);

// Add default buttons maps for advanced theme and all internal plugins
tinyMCE.addButtonMap(TinyMCE_Simple2Theme._buttonMap);

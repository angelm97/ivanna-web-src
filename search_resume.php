 ...
 switch($action1)
 {
  ...
  case 'save_search':
   ...
   $template->assign_vars(array('INFO_TEXT_TITLE_NAME' => INFO_TEXT_TITLE_NAME,
								'INFO_SEARCH_ALL_RESUME' => INFO_SEARCH_ALL_RESUME,
                                'INFO_TEXT_TITLE_NAME1'=>tep_draw_input_field('TR_search_name', $search_name,'class="form-control form-control-sm" placeholder="Enter search agent name" ',true)));
 }
}
// search
...
//   echo $x;//exit;
   ...
   if($x > 0 && $x1 > 0)
   {
    ...
    while($row = tep_db_fetch_array($result))
    {
     ...
     {
      $photo = tep_image(FILENAME_IMAGE.'?image_name='.PATH_TO_PHOTO.$row['jobseeker_photo'],'','','','class="img-thumbnail img-fluid resume--result-profile-img mr-3"');
     }
					else
					{
      $photo ='<img src="'.HOST_NAME.'img/nopic.jpg" class="img-thumbnail img-fluid resume--result-profile-img mr-3">';
					}
					...
					$div_extended_values='<table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF">
                            ...
                            <tr>
                             ...
                             <td valign="top">'.tep_db_output($jobseeker_education_check['education_level_name']).(tep_not_null($jobseeker_education_check['specialization'])?' ('.tep_db_output($jobseeker_education_check['specialization']).')':'').'</td>
																												...
																												<tr>
                             <td valign="top">'.(tep_not_null($experience_row['company'])?'Laborando en '.$experience_row['company'] :'').(tep_not_null($experience_row['job_title'])? 'como '.$experience_row['job_title'] :'').'</td>
			                         </tr>').'
																											</table>
																										</td>
																									</tr>
...
if(check_login('admin'))
{
 ...
 $rate_it_string.='</td>';
 $rate_it_string.='<td valign="middle" >'.tep_draw_submit_button_field('','Velocidad','class="btn btn-primary"').'</td></tr>';
}
if(check_login('recruiter') && $adminedit==false)
{
 ...
 $rate_it_string.='</td></tr><tr><td>&nbsp;</td><td valign="middle">'.(check_login("recruiter")?tep_draw_submit_button_field('','Agregar','class="btn btn-primary"'):'').'&nbsp;</td></tr>';
}
...
/********---------------------------------------------------*/
     $template->assign_block_vars('search_resume_result', array(
		...
		'contact'=>'<a class="btn btn-sm btn-text border"  href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1.'&action1=save&action=contact').'">'.INFO_TEXT_CONTACT.'</a>',
		'working'=>(tep_not_null($experience_row['company'])?'Laborando en '.$experience_row['company'] :'').(tep_not_null($experience_row['job_title'])? ' como '.$experience_row['job_title'] :''),
      ...
      'email_address' =>($row['jobseeker_privacy']==3?'<a href="mailto:'.$row['jobseeker_email_address'].'">'.tep_db_output($row['jobseeker_email_address']).'</a>':'*****'),
		...
		'view'     => '<a class="btn btn-sm btn-text border" target="_blank" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1).'">'.INFO_TEXT_VIEW.'</a>',
		'download' => '<a target="_blank" class="btn btn-sm btn-text border"  href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1.'&action=download').'">Descargar</a>',
'save'=>tep_draw_form('save_form', FILENAME_JOBSEEKER_VIEW_RESUME.'?query_string1='.$query_string1,'','post', '').tep_draw_hidden_field('action1','save_resume').'<button type="submit" class="btn btn-primary" >Guardar</button></form>',
	'connect'=> '<a class="btn btn-sm btn-text border" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1.'&action=book_mark').'">'.INFO_TEXT_CONNECT_TO_JOB.'</a>',
	...
	'rate_resume'=> '<a class="btn btn-sm btn-text border" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1).'">'.INFO_TEXT_RATE_RESUME.'</a>',

'rate_resume_main'=>(tep_not_null($row['point'])||tep_not_null($row['point1'])?'<a class="btn btn-sm btn-text border" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1).'">'.(tep_not_null($row['point'])?tep_db_output($row['point']).'<span style="color:blue"> *</span>':'').(tep_not_null($row['point1'])?tep_db_output($row['point1']).' <span class="red"> **</span>':'').'</a>':'<a class="btn btn-sm btn-text border" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1).'">'.INFO_TEXT_RATE_RESUME.'</a>'),
    ...
    $plural=($x1=="1")?TABLE_HEADING_RESUME:TABLE_HEADING_RESUMES;
				$check_link = '<td width="60%" valign="left"><div align="left"><span class="checkbox-resume">
                            <a href="#normal_resume" class="btn btn-sm btn-text border mr-1" onclick="checkall()">'.tep_db_output(INFO_TEXT_SELECT_ALL).'</a> <a href="#normal_resume" class="btn btn-sm btn-text border mr-1" onclick="uncheckall()">'.tep_db_output(INFO_TEXT_CLEAR_SELECTED).'</a> <a href="#normal_resume" class="btn btn-sm btn-text border" onclick="SaveSelected(\'save\')">'.tep_db_output(INFO_TEXT_SAVE_SELECTED).'</a>
                          </span> </div></td>
...
array_unshift($cat_array,array("id"=>0,"text"=>"Todas las categorias"));
//////////////////////////////

    $template->assign_vars(array('total'=>"<span class='badge-custom'>$x1</span> ".$plural,'rate_mark'=>'<span class="red">**</span><span class="resume_result3">'.INFO_TEXT_RATED_ADMIN.'</span><span style="color:blue">*</span><span class="resume_result3">'.INFO_TEXT_RATED_SELF.'</span>',
					...
					'search_resume_form_keyword'=>tep_draw_input_field('keyword','','placeholder="Ej. Ejecutivo de ventas" class="form-control"',false),
					'search_resume_form_location'=>LIST_TABLE(COUNTRIES_TABLE,TEXT_LANGUAGE."country_name","priority","name='country' class='form-control'","All Locations","",DEFAULT_COUNTRY_ID),
					'search_resume_form_cat'=> tep_draw_pull_down_menu('industry_sector[]', $cat_array, '', 'class="form-control"'),
					'search_resume_form_exp'=>experience_drop_down('name="experience" class="form-control"', 'Experiencia', '', $experience),
					'search_resume_form_button'=>tep_draw_submit_button_field('login2','Buscar','class="btn btn-primary btn-block"'),
					'advance_search'=>'<a href="'.tep_href_link(FILENAME_RECRUITER_SEARCH_RESUME).'">Búsqueda Avanzada</a>',
...
array_unshift($cat_array,array("id"=>0,"text"=>"Todas las categorias"));
    ...
    $template->assign_vars(array('total'=>'No '.TABLE_HEADING_RESUME,//tep_db_output(SITE_TITLE)." ".INFO_TEXT_HAS_NOT_MATCHED,
                                 'result_class' =>'class="result_hide"',
                            					'map_view_link'=>'',
	'search_resume_form'=>tep_draw_form('search_resume', FILENAME_RECRUITER_SEARCH_RESUME,'','post').tep_draw_hidden_field('action','search'),
	'search_resume_form_keyword'=>tep_draw_input_field('keyword','','placeholder="Ej. Ejecutivo de ventas" type="text" class="form-control-applicant"',false),
	'search_resume_form_location'=>LIST_TABLE(COUNTRIES_TABLE,TEXT_LANGUAGE."country_name","priority","name='country' class='form-control'","All Locations","",DEFAULT_COUNTRY_ID),
	'search_resume_form_cat'=> tep_draw_pull_down_menu('industry_sector[]', $cat_array, '', 'class="form-control"'),
	'search_resume_form_exp'=>experience_drop_down('name="experience" class="form-control"', 'Experiencia', '', $experience),
	'search_resume_form_button'=>tep_draw_submit_button_field('login2','Buscar Ahora','class="btn btn-primary"'),
	'advance_search'=>'<a href="'.tep_href_link(FILENAME_RECRUITER_SEARCH_RESUME).'">Busqueda Avanzada</a>',
	'applicant_tracking'=>'<li><a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS).'">All Applicants</a></li>
                                <li><a href="'.tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT).'">Applicant Pipeline</a></li>
   ...
   if($x > 0 && $x2 > 0)
   {
    ...
    while($row = tep_db_fetch_array($result))
    {
     ...
     $template->assign_block_vars('search_resume_result1', array(
		...
		'working'=>(tep_not_null($experience_row['company'])?'Labora en '.$experience_row['company'] :'').(tep_not_null($experience_row['job_title'])? ' como '.$experience_row['job_title'] :''),
		'check_box'=> tep_draw_checkbox_field('resume_id[]',$ide,false),
		'name' => '<a target="_blank" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1).'">'.tep_db_output($row['jobseeker_name']).'</a>',
		'city'     => (tep_not_null($row['jobseeker_city'])?', '.$row['jobseeker_state']:$row['jobseeker_state']).' '.($row['jobseeker_country']),//ucfirst(tep_db_output($row['jobseeker_city'])),
      //'email_address' =>($row['jobseeker_privacy']==3?'<a href="mailto:'.$row['jobseeker_email_address'].'">'.tep_db_output($row['jobseeker_email_address']).'</a>':'*****'),
		...
		'download' => '<a target="_blank" class="btn-default-custom"  href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string1='.$query_string1.'&action=download').'">Descargar Perfil</a>',
...
$template->assign_vars(array( 'hidden_fields' => $hidden_fields,
 'button'                  => tep_draw_submit_button_field('',INFO_SEARCH_BUTTON,'class="btn btn-primary"'),
 ...
 'save_button'             => tep_draw_submit_button_field('','Guardar','class="btn btn-sm btn-primary float-right"').($action1=='save_search'?'&nbsp;'.'':''),
...
//  'INFO_TEXT_KEYWORD1'      => tep_draw_input_field('keyword', $keyword,'class="form-control"',false).INFO_TEXT_KEYWORD_EXAMPLE,
 ...
 'INFO_TEXT_KEYWORD3'      => '<small class="mr-3">'.INFO_TEXT_KEYWORD_CRITERIA.'</small> <small class="mr-3">'.tep_draw_radio_field('word1', 'Yes', 'Yes', $word1,'id=radio_word1').'<label for="radio_word1">'.INFO_TEXT_KEYWORD_WORD1.'</label></small> <small>'.tep_draw_radio_field('word1', 'No', '', $word1,'id=radio_word2').'<label for="radio_word2">'.INFO_TEXT_KEYWORD_WORD2.'</label></small>',

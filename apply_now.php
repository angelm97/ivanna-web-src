...
$template->set_filenames(array('apply_now' => 'apply_now.htm','email'=>'application_send_template.htm','es_email'=>'es_application_send_template.htm'));
...
{
 $cover_letters=tep_draw_textarea_field('TR_cover_letter', 'soft', '50', '5', INFO_TEXT_DEFALUT_COVER_LETTER, 'class="form-control required"', false, false);
}
...
if($db_resume_name_num_row > 0)
{
 while ($resume_name = tep_db_fetch_array($db_resume_name_query))
 {
  ...
  $resume_names_array[]='<tr><td>'.tep_draw_radio_field('my_resume_id', $ide, '', $ide, 'id="radio_resume_name'.$ide.'"').'&nbsp;<label for="radio_resume_name'.$ide.'" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'"></td><td class="small"><a href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME."?resume=".$query_string1).'" target="_resume">'.tep_db_output($resume_name['resume_title']).'</a></label></td><tr>';
 }
}
else
{
...
//  'button'=>tep_image_submit(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM),
 ...
 'INFO_TEXT_DEFALUT_COVER_LETTER1'=>sprintf("<a href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_COVER_LETTERS, 'action=add_new')."' class='btn btn-default border'>".INFO_TEXT_ADD_NEW_COVER_LETTER."</a>"),

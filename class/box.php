...
class left_box extends tableBlockLeft
{
  ...
  {
  	$this->table_row_parameters = 'class="menuBoxHeading1"';
		 $this->table_data_parameters = '';
  }
		...
		if (isset($heading[0]['link']))
		{
			...
			$heading[0]['text'] = '<table border="0"><tr><td width="20">'.$text_image.'</td><td><a href="' . $heading[0]['link'] . '" '.((($heading[0]['default_row'])==1)?' class="menuBoxHeadingLink1"':'class="menuBoxHeadingLink" ').' >' . $heading[0]['text'] . '</a></td>	</tr></table>';
		}
		else
		{
  ...
  $this->table_parameters= 'class="left_contents"';

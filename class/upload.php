...
class upload
{
  ...
  $this->set_filesize($file['size']);
  if($this->filesize <=0)
  {
   if($this->message_location=='direct')
   {
    $messageStack->add(ERROR_WRONG_FILENAME,'error');
   }
   else
   {
    $messageStack->add_session(ERROR_WRONG_FILENAME,'error');
   }
   return false;
  }
		if ( tep_not_null($file['tmp_name']) && ($file['tmp_name'] != 'none') && is_uploaded_file($file['tmp_name']) )
		{
			...
			$this->set_file($file);
			$this->set_filename(str_replace(array("'"," ","(",")"),array("_","_","_","_"),$file['name']));
 ...
 {
  $this->filename=date("YmdHis").$filename;
 }

...
class splitPageResults 
{
 ...
 function display_links($query_numrows, $max_rows_per_page, $max_page_links, $current_page_number, $parameters = '', $page_name = 'page') 
 {
  ...
  if ($num_pages > 1) 
  {
   $display_links = "<form name='pages' method='get' action='".basename($_SERVER['PHP_SELF'])."'>";
   if ($current_page_number > 1) 
   {
    $display_links .= '<a href="' . basename($_SERVER['PHP_SELF'])."?". $parameters . $page_name . '=' . ($current_page_number - 1) . '" class="splitPageLink">' . PREVNEXT_BUTTON_PREV . '</a>&nbsp;&nbsp;';
   } 
   else 
   {
   ...
   {
    $display_links .= '&nbsp;&nbsp;<a href="' . basename($_SERVER['PHP_SELF'])."?". $parameters . $page_name . '=' . ($current_page_number + 1) . '" class="splitPageLink">' . PREVNEXT_BUTTON_NEXT . '</a>';
   } 
   else 
   {

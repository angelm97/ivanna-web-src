...
class VideoportalPlayer
{
    ...
    function getYoutube($posting)
    {
       ...
       /* $tmpl_player= '<object width="'.$this->width.'" height="'.$this->height.'">
        ...
        return $posting;
		*/
		$tmpl_player='<iframe width="560" height="315" src="https://www.youtube.com/embed/[ID]?rel=0&amp;controls=0&amp;showinfo=0" frameborder="0" allowfullscreen></iframe>';
        preg_match('/[(^https|^http)]:\/\/(www\.)?youtube\.com\/watch\?v=([\d\w\-_]+)/', $posting, $results);
        if($results)
        $posting = str_replace('[ID]', $results[2], $tmpl_player);
        return $posting;
    }

...
///////////// Only for this site ends ////////
			...
			if(objFrm[i].type=='select-one' && objDataTypeHolder=="TR_")
   {
    ...
    else if(objFrm[i].options[objFrm[i].selectedIndex].value=='' && test!='aa')
    {
     ...
     alert("Por favor seleccione "+ sChangedName +".");
     objFrm[i].focus();
     return false;
     break;
    }
   ...
   if(objFrm[i].type=='password' && objFrm[i].value!='' &&
			...
			{
				alert("La contraseña no pueden incluir espacio");
				objFrm[i].select();
				return false;
				break;
			}
			...
			{
				alert("La contraseña deben tener minimo 5 caracteres.");
				objFrm[i].select();
				return false;
				break;
			}
			...
			{
				alert("La contraseña no puede tener mas de 15 caracteres");
				objFrm[i].select();
				return false;
				break;
			}
			...
			if((objDataTypeHolder=="TR_")&& (objFrm[i].value=='') && ! ((objFrm[i].name=='TR_end_month' || objFrm[i].name=='TR_end_year')))
			{
				...
				alert("Por favor introduzca "+ traductor[sChangedName] +".");
        {
				 objFrm[i].focus();
 				objFrm[i].select();
        }
				...
				objFrm[i].value!='' ))
			{
				...
				sChangedName = objFrm[i].name.substring(5);
				sChangedName = getFormattedmsg(sChangedName)
				alert("Por favor introduzca "+ traductor[sChangedName] +" valido.");
				objFrm[i].focus();
				objFrm[i].select();
				return false;
				break;
			...
			{
				alert("El correo y la confirmacion del correo no son iguales");
				objFrm[i].focus();
				objFrm[i].select();
				return false;
				break;
			...
			if(objFrm[i].type=='file' && objFrm[i].value!="")
		 {
		 	for(var fi=0; fi < objFrm[i].value.length;fi++)
		 	{
		 		...
		 		{
		 			alert("(') No esta permitido en los nombres de archivos.");
		 			objFrm[i].focus();
		 			objFrm[i].select();
		 			return false;
		 			break;
			 ...
			 {
				 validformFile = /(.txt|.doc|.docx|.pdf|.jpg|.png|.HEIC)$/i;
				 if(!validformFile.test(objFrm[i].value))
				 {
				 	alert("Archivo no compatible. Intentalo de nuevo");
				 	objFrm[i].focus();
				 	objFrm[i].select();
				 	return false;
				 	break;

...
if(isset($_GET['query_string2'])  && $_GET['query_string2']!='')
{
  ...
  'INFO_TEXT_BUTTON'  => '<a href="'.tep_href_link(FILENAME_RECRUITER_ATS_MAILS,"query_string1=".$query_string2).'">'.tep_draw_submit_button_field('back', IMAGE_BACK,'class="btn btn-secondary"').'</a>&nbsp;&nbsp;'.tep_draw_submit_button_field('','Enviar','class="btn btn-primary"'),
...
elseif($mail_id>0)
{
 ...
 tep_db_query("update ".APPLICANT_INTERACTION_TABLE." set user_see='Yes' where id='".$mail_id."'");
  ...
  'INFO_TEXT_BACK'=>'<a href="'.FILENAME_RECRUITER_ATS_MAILS.'"><button class="btn btn-primary" >Atras</button>',
...
else
{
 ...
 $view_link="";
 if($mail_status=='inactive')
  $show_mail_status=INFO_TRASH;
 elseif($mail_status==INFO_SEND)
  $show_mail_status=INFO_SEND;
 else
  $show_mail_status=INFO_INBOX;

 if($show_mail_status==INFO_INBOX)
  $view_link1 =INFO_TEXT_VIEW_IN."<a href='#'  onclick='view_data(\"all\",\"send\")'>".INFO_TEXT_VIEW_SEND."</a> | <a href='#' class='red'  onclick='view_data(\"all\",\"inactive\")'>".INFO_TEXT_VIEW_TRASH."</a>  | <a href='".tep_href_link(FILENAME_RECRUITER_APPLICANT_TRACKING)."'>".INFO_COMPOSE."</a>";
 elseif($show_mail_status==INFO_SEND)
  $view_link1 =INFO_TEXT_VIEW_IN."<a href='#'  onclick='view_data(\"all\",\"active\")'>".INFO_TEXT_VIEW_INBOX."</a> | <a href='#' class='red'  onclick='view_data(\"all\",\"inactive\")'>".INFO_TEXT_VIEW_TRASH."</a>  | <a href='".tep_href_link(FILENAME_RECRUITER_APPLICANT_TRACKING)."'>".INFO_COMPOSE."</a>";
 else
  $view_link1 =INFO_TEXT_VIEW_IN. "<a href='#'  onclick='view_data(\"all\",\"send\")'>".INFO_TEXT_VIEW_SEND."</a> | <a href='#'   onclick='view_data(\"all\",\"active\")'>".INFO_TEXT_VIEW_INBOX."</a>  | <a href='".tep_href_link(FILENAME_RECRUITER_APPLICANT_TRACKING)."'>".INFO_COMPOSE."</a>";
 ...
 if($x > 0 && $x1 > 0)
 {
  ...
  while ($jobseeker_mail = tep_db_fetch_array($result))
  {
   ...
   $row_selected=' class="dataTableRow'.(($jobseeker_mail['user_see']=='No' && $show_mail_status!=INFO_SEND)?'3':($alternate%2==1?'1':'2')).'" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"';

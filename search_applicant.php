...
$experience_row=getAnyTableWhereData(JOBSEEKER_RESUME2_TABLE.' as ex ',"resume_id='".$ide."' order by start_year desc ,start_month desc","ex.company,ex.job_title");
       ...
       $template->assign_block_vars('search_applicant', array(
		  ...
		  'applicant_pipeline'=>'<a class="btn btn-sm btn-text border" href="'.tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT,'jobID='.$job_id).'#Pipeline">Estadisticas del candidato</a>',
		  'contact'=>'<a class="btn btn-sm btn-text border" href="'.tep_href_link(FILENAME_EMPLOYER_INTERACTION,'query_string3='.$query_string3).'">Contactar</a>',
		  'view_resume'=>'<a class="btn btn-sm btn-text border" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.$query_string).'">Ver hoja de vida</a>',
  		  'photo'=>(tep_not_null($row['jobseeker_photo']) && is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$row['jobseeker_photo'])?tep_image(FILENAME_IMAGE."?size=95&image_name=".PATH_TO_PHOTO.$row['jobseeker_photo'],tep_db_output(SITE_TITLE),'','','class="img-thumbnail mr-3"'):'<img src="'.HOST_NAME.'/img/nopic.jpg" class="img-thumbnail mini-profile-img mr-3">'),
...
'search_resume_form'=>tep_draw_form('search_applicant', FILENAME_RECRUITER_SEARCH_APPLICANT,'','post').tep_draw_hidden_field('action1','search'),
  ...
  'INFO_TEXT_FIRST_NAME1' => tep_draw_input_field('first_name',$first_name, 'class="form-control mb-2" placeholder="Primer nombre"',false),
  'INFO_TEXT_LAST_NAME1'  => tep_draw_input_field('last_name',$last_name, 'class="form-control mb-2" placeholder="Apellido"',false),
  'INFO_TEXT_EXPERIENCE'   => INFO_TEXT_EXPERIENCE,
  'INFO_TEXT_EXPERIENCE1'  => experience_drop_down('name="experience" class="form-control mb-2"', 'Sin experiencia', '', $experience),
  'INFO_TEXT_EMAIL_ADDRESS1'=> tep_draw_input_field('TNEF_email_address',$email_address, 'class="form-control mb-2" placeholder="Correo electrónico"', false),
  'buttons'  => tep_draw_submit_button_field('','Buscar','class="btn btn-primary btn-block"'),
...
else
{
 $template->assign_vars(array(
  ...
  'buttons'               => tep_draw_submit_button_field('','Buscar ','class="btn btn-primary"'),
  ...
  'INFO_TEXT_APPLICATION1'=> tep_draw_input_field('application_id','', 'class="form-control mb-2" placeholder="Nombre de vacante"', false ),
  'INFO_TEXT_FIRST_NAME'  => INFO_TEXT_FIRST_NAME,
  'INFO_TEXT_FIRST_NAME1' => tep_draw_input_field('first_name',$first_name, 'class="form-control mb-2" placeholder="Nombre"',false),

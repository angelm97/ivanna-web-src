...
///*******************************************COMPANY RATING ***************************************//
   ...
   if($x > 0 && $x1 > 0)
   {
	...
	$job_category_ids=get_name_from_table(JOB_JOB_CATEGORY_TABLE,'job_category_id','job_id',$ide);
     ...
     $template->assign_block_vars('result', array( 'row_selected' => $row_selected,
      ...
      'apply' => '<a class="text-primary text-capitalize" href="'.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'">'.INFO_TEXT_APPLY_NOW.'</a>',
      ));
     $alternate++;
     $lower = $lower + 1;
    }
    $plural=($x1=="1")? INFO_TEXT_JOB:INFO_TEXT_JOBS;
    $template->assign_vars(array('total'=>tep_db_output(SITE_TITLE).INFO_TEXT_HAS_MATCHED." <span class='badge badge-danger'>$x1</span> ".$plural. " ".INFO_TEXT_TO_YOUR_SEARCH_CRITERIA));
   }
   else
   {
  ...
  $company_name=tep_db_prepare_input($_POST['company_name']);
  $recruiter_email_address=check_data1($company_name,"=","recruiter_email","mail");
  $filed_names='r.recruiter_company_name,recruiter_logo,r.recruiter_url,d.description,r.recruiter_city,if(r.recruiter_state_id,z.zone_name,r.recruiter_state) as recruiter_state,c.country_name,r.recruiter_zip,r.recruiter_telephone,r.recruiter_id';
  $recuiter_address='';
  $company_logo1='';
  $company_description='';
...
//////////////////////////////////////////////////////////////////////////////////////
 ...
 'back_button'=>tep_button_submit('btn btn-sm btn-secondary','Back', 'onclick="history.back();"' ),
...
// $whereClause="where rl.recruiter_status='Yes'";
 ...
 $hidden_fields.=tep_draw_hidden_field('sort',$sort);
 $totalpage=ceil($x1/$higher);
 $fields="recruiter_company_name,recruiter_email_address,recruiter_logo";
	...
	if($x > 0 && $x1 > 0)
 {
  ...
  while($row =  tep_db_fetch_array($result))
  {
   ...
   if(tep_not_null($company_logo) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_LOGO.$company_logo))
			{
			 $company_logo=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_LOGO.$company_logo."&size=80");
   ...
   $company_name="<a href='#' onclick='search_company(\"".$query_string1."\")' class='blue'>".tep_db_output($row['recruiter_company_name'])."</a> ";
   $row_selected=' class="dataTableRow'.($alternate%2==1?'1':'2').'" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"';
   $alternate++;
  ...
  see_page_number();
  $template->assign_vars(array('total'=>SITE_TITLE." ".INFO_TEXT_HAVE."  <span class='badge badge-danger'>$x1</span> ".INFO_TEXT_COMPANY_IN_DIRECTORY));
 }
 else
  {

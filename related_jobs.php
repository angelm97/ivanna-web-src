...
$field_names="j.job_id, j.job_title,concat(case when j.job_location='' then '' else concat(j.job_location,', ') end, if(j.job_state_id,z.zone_name,j.job_state)) as job_location,r.recruiter_company_name,j.min_experience,j.max_experience,r.recruiter_logo,r.recruiter_url,j.job_short_description,j.expired,j.job_featured,j.url,j.post_url";
...
while($row = tep_db_fetch_array($result))
{
 ...
 if(tep_not_null($company_logo) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_LOGO.$company_logo))
 {
  ...
  {
   $photo=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_LOGO.$company_logo."&width=220");
   $company_logo=$photo;
  }
	}
	else

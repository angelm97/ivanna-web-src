...
if(tep_not_null($action))
{
 switch($action)
 {
  ...
  case 'edit':
   ...
   $google_url   = tep_db_prepare_input($_POST['google_url']);
   ...
   if($total_industry_sector <=0)
   {
    ...
    $facebook_url ='http://'.$facebook_url;
			...
			if(tep_not_null($google_url))
		 {
	   if(!preg_match('/^(http|https):\/\//i',$google_url))
...
{
	$add_save_button='<button class="btn  btn-primary mr-2" type="submit">Siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
}
else
{
	$add_save_button='<button class="btn  btn-primary mr-2" type="submit">Siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
...
if($error)
{
 ...
 {
  $add_save_button     = '<button class="btn btn-primary mr-2" type="submit">Actualizar</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
  $query_string=encode_string("resume_id@@@".$_POST['resume_id']."@@@resume");
  $add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME2."?query_string=".$query_string)."' class='btn btn-secondary'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
...
else
{
 ...
 if($row2=getAnyTableWhereData(JOBSEEKER_RESUME1_TABLE,"jobseeker_id='".$_SESSION['sess_jobseekerid']."' and resume_id ='".$resume_id."'",$fields))
 {
  ...
  $add_save_button          = '<button class="btn btn-primary mr-2" type="submit">Actualizar</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
  $add_next_button          = "<a class='btn btn-secondary' href='".tep_href_link(FILENAME_JOBSEEKER_RESUME2."?query_string=".$query_string)."' >".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
 ...
 else
 {
  ...
  $add_save_button   = '<button id="vp" class="btn  btn-primary mr-2" type="submit">Siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
  $add_next_button   = '';
  $registration_form = tep_draw_form('defineForm', FILENAME_JOBSEEKER_RESUME1, '', 'post', 'onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','add');
  $resume_id         = "";
  $resume_name       = $row12['jobseeker_first_name']." ".$row12['jobseeker_last_name'];
  $job_type          = "";
  $currency          ='';
...
if(check_login("jobseeker"))
{
    $resume1='<a class="list-group-item list-group-item-action active" href ="#"><i class="fa fa-file-text icon-resume-left active-icon" aria-hidden="true"></i> '.INFO_TEXT_LEFT_RESUME.'</a>';
...
if($messageStack->size('job_category') > 0)
 ...
 'INFO_TEXT_OBJECTIVE1'=>tep_draw_textarea_field('objective', 'soft', '60', '5', stripslashes($objective), 'class="form-control"', true, false),
 ...
 'INFO_TEXT_DESIRED_SALARY1'=> "<div class='col-3'>"
 .LIST_SET_DATA(CURRENCY_TABLE,"",'code','currencies_id',"code",'class="form-control" name="currency" ',TEXT_PLEASE_SELECT,'',$currency)
 ."</div><div class='col-6'>".tep_draw_input_field('expected_salary', $expected_salary,'class="form-control" size="10"',false)
 .'</div><div class="form-check mt-2">Por&nbsp;'
 .tep_draw_radio_field('expected_salary_per', 'Year', '', $expected_salary_per, 'id="expected_salary_per1"')
 .'<label class="mr-3" for="expected_salary_per1">'.INFO_TEXT_RES_YEAR.'</label>'
 .tep_draw_radio_field('expected_salary_per', 'Month', '', $expected_salary_per, 'id="expected_salary_per2"')
 .'<label class="mr-3" for="expected_salary_per2">'.INFO_TEXT_RES_MONTH.'</label>'
 .tep_draw_radio_field('expected_salary_per', 'Hour', '', $expected_salary_per, 'id="expected_salary_per3"')
 .'&nbsp;<label for="expected_salary_per3">'.INFO_TEXT_RES_HOUR.'</label></div>',
 ...
 'INFO_TEXT_NATIONALITY1'=>LIST_SET_DATA(COUNTRIES_TABLE,"",TEXT_LANGUAGE.'country_name','id',TEXT_LANGUAGE."country_name","name='TR_nationality' class='form-control' " ,"",'',$nationality),

 'INFO_TEXT_INDUSTRY'=>INFO_TEXT_INDUSTRY,
 'INFO_TEXT_INDUSTRY1'=>get_drop_down_list(JOB_CATEGORY_TABLE,"name='TR_industry_sector[]' id='industry' class='form-control required selectpicker' size='5' multiple","","0",$industry_sector),
 ...
 'INFO_TEXT_RELOCATE1'=>tep_draw_radio_field('relocate', 'Yes', true, $relocate, 'id="radio_relocate1"').'&nbsp;<label for="radio_relocate1" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'">'.INFO_TEXT_YES.'</label>&nbsp;'.tep_draw_radio_field('relocate', 'No', '', $relocate, 'id="radio_relocate2"').'&nbsp;<label for="radio_relocate2" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'">'.INFO_TEXT_NO.'</label>',
	...
	'INFO_TEXT_GOOGLE_URL'    => INFO_TEXT_GOOGLE_URL,
	'INFO_TEXT_GOOGLE_URL1'   => tep_draw_input_field('google_url', $google_url,'size="60" class="form-control"'),
 ...
 'LEFT_HTML'=>LEFT_HTML,
 'INFO_TEXT_JOBSEEKER_LEFT'=>((check_login("jobseeker") && tep_not_null($_POST['resume_id']))? JOBSEEKER_RESUME_LEFT:'<td width="19%">'.LEFT_HTML_JOBSEEKER.'</td><td width="1%"><img src="img/spacer.gif" width="10" height="5"></td>'),

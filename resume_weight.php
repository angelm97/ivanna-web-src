...
switch($action)
{
 case'edit_weight':
 {
  ...
  $job_skill_weight=tep_db_prepare_input($_POST['IN_job_skill_weight']);
  $experience_weight=tep_db_prepare_input($_POST['IN_experience_weight']);
  $total=0;
  $total=$location_weight+$industry_weight+$job_type_weight+$job_skill_weight+$experience_weight;
  if($total!=100)
  {
   $error=true;
   if($total>100)
...
if(!$error)
{
 ...
 $job_skill_weight=tep_db_output($rows['skill']); 
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_skill_TYPE1'=>tep_draw_input_field('IN_job_skill_weight', $job_skill_weight,'onFocus="calculate_total();" onBlur="calculate_total();" class="form-control"'),
 ...
 'save_button'    => tep_draw_submit_button_field('','Guardar','class="btn btn-primary"'),//tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE),

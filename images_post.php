...
$template->set_filenames(array('image_post' => 'image_post.htm'));
include_once(FILENAME_BODY);

$id_job_for_post = $_GET['jobID'];


//recruiter_id

/*
  nombre de empresa  recruiter_company_name

  nombre del puesto  job_title
  descripcion  job_short_description

*/

$db_job_query_raw = "SELECT j.job_title, j.job_short_description,  e.recruiter_logo, e.recruiter_company_name, el.recruiter_email_address FROM jobs j, recruiter e, recruiter_login el WHERE j.job_id = $id_job_for_post AND e.recruiter_id = j.recruiter_id AND e.recruiter_id = el.recruiter_id ";
...
if ($job_datos_post['recruiter_logo'] == null) { 
  $job_datos_post['recruiter_logo'] = '/image/no_pic.gif';
}else{
  $job_datos_post['recruiter_logo'] = 'logo/' . $job_datos_post['recruiter_logo'];
}
...
//else
{
 ...
 if($list_row > 0)
 {
  ...
  while ($row= tep_db_fetch_array($result_query_list))
  {
   ...
   $round_report['name'][]  = INFO_TEXT_ROUND."  ".$row[TEXT_LANGUAGE.'round_name'];
   $round_report['value'][] = $no_of_total_application;
   ///////////////////////////////////////////////////////////////////////////////
   $row_selected=' class="dataTableRow'.($alternate%2==1?'1':'2').'" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"';
   $template->assign_block_vars('applicant_round', array( 'row_selected' => $row_selected,
    'round'   => tep_db_output(INFO_TEXT_ROUND."  ".$row[TEXT_LANGUAGE.'round_name']." "),
...
$template->assign_vars(array(
  ...
  'INFO_TEXT_BACK'=>tep_draw_form('search_applicant',FILENAME_RECRUITER_SEARCH_APPLICANT,'','post').tep_draw_hidden_field('action1','search').'<button class="btn btn-secondary btn-sm" type="submit">'.INFO_TEXT_BACK.'</button>',//'<a class="btn btn-secondary btn-sm" href="#" onclick="javascript:history.back();">'.INFO_TEXT_BACK.'</a>',
  'INFO_TEXT_JOB_LISTING'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS).'">'.INFO_TEXT_JOB_LISTING.'</a>',
  'INFO_TEXT_APPLICATION'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,"jobID=".$job_id).'">'.INFO_TEXT_APPLICATION.'</a>',
  ...
  'titulodeltrabajo' =>   $job_datos_post['job_title'], 
  'descripciondeltrabajo' =>   $job_datos_post['job_short_description'], 
  'nombredeempresa' =>   $job_datos_post['recruiter_company_name'], 
  'emaildeempresa' =>   $job_datos_post['recruiter_email_address'],
  'logodeempresa' =>   $job_datos_post['recruiter_logo'],

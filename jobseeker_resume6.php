...
{
 $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
 $reference_button='<button class="btn btn-primary mr-2" type="submit" onclick=set_action1("reference_add")>Agregar nueva referencia</button> <button class="btn btn-secondary mb-2 mr-2" type="submit" onclick=set_action1("reference_add_next")>Guardar y siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'add_new_reference.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action1('reference_add')")."".tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action1('reference_add_next')");
 ...
 if($_POST['action']=="reference_edit" || $_POST['action']=="reference_add" || $_POST['action']=="reference_add_next")
 {
  if($_POST['action']=="reference_edit")
  {
   ...
   $refname            = $refname;
   $company_name    = $company_name;
   $reference_button= '<button class="btn btn-secondary mb-2 mr-2" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
   $query_string    = encode_string("resume_id@@@".$resume_id."@@@resume");
			$add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME3."?query_string=".$query_string)."' class='btn btn-secondary mb-2'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
  ...
  {
   $reference_button='<button class="btn btn-primary mb-2 mr-2" type="submit" onclick=set_action1("reference_add")>Agregar nueva referencia</button> <button class="btn btn-secondary mb-2 mr-2" type="submit" onclick=set_action1("reference_add_next")>Guardar y siguiente</button>';//tep_image_submit(PATH_TO_BUTTON.'add_new_reference.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action1('reference_add')")."".tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action1('reference_add_next')");
   $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
			$add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME3."?query_string=".$query_string)."' class='btn btn-secondary mb-2'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
 ...
 $resume1='<a class="list-group-item list-group-item-action" href ="#"  onclick="document.resume.submit()"><i class="fa fa-file-text icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_RESUME.'</a>';
    ...
    $resume3='<a class="list-group-item list-group-item-action" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME3.'?query_string='.$query_string).'" ><i class="fa fa-graduation-cap icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_EDUCATION.'</a>';
		  ...
		  $resume5='<a class="list-group-item list-group-item-action" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME5.'?query_string='.$query_string).'" ><i class="fa fa-upload icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_UPLOAD.'</a>';
				$resume6='<a class="list-group-item list-group-item-action active" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME6.'?query_string='.$query_string).'" ><i class="fa fa-bookmark icon-resume-left active-icon" aria-hidden="true"></i> '.INFO_TEXT_LEFT_REFERENCE.'</a>';
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_REF_COUNTRY1'       => LIST_SET_DATA(COUNTRIES_TABLE,"",TEXT_LANGUAGE.'country_name','id',TEXT_LANGUAGE."country_name",'class="form-control" name="country"',INFO_TEXT_PLEASE_SELECT,"",$ref_country),

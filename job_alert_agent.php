...
//   'INFO_TEXT_KEYWORD1'     => tep_draw_input_field('keyword', $keyword,'class="form-control" size="40"',false).INFO_TEXT_KEYWORD_EXAMPLE,
  'INFO_TEXT_KEYWORD1'     => tep_draw_input_field('keyword', $keyword,'class="form-control" size="40" placeholder="Ingresar"',false),
  ...
  'INFO_TEXT_JOB_ALERT_CRITERIA1'=> tep_draw_radio_field('job_alert', 'daily', '', $job_alert,'id=radio_daily').' <label for="radio_daily" class="mr-3">'.INFO_TEXT_DAILY.'</label>'.tep_draw_radio_field('job_alert', 'weekly', '', $job_alert,'id=radio_weekly').' <label class="mr-3" for="radio_weekly">'.INFO_TEXT_WEEKLY.'</label>'.tep_draw_radio_field('job_alert', 'monthly', '', $job_alert,'id=radio_monthly').' <label for="radio_monthly">'.INFO_TEXT_MONTHLY.'</label>',
  'INFO_TEXT_LOCATION'     => (($search_zip_code==2)?INFO_TEXT_ZIP_CODE:INFO_TEXT_LOCATION_NAME),
  'INFO_TEXT_LOCATION1'    => INFO_TEXT_STATE1,
...
':''),
  ...
  'INFO_TEXT_EXPERIENCE1'  => experience_drop_down('name="experience" class="form-control"', 'Sin Experiencia', '', $experience),
  ...
  'button'                 => '<button class="btn btn-primary" type="submit">Guardar</button>',
...
//	$agent_id=getAnyTableWhereData(SEARCH_JOB_RESULT_TABLE,"id=".tep_db_insert_id(),"id");
 $template->assign_vars(array( 'hidden_fields' => $hidden_fields,
  ...
  'INFO_TEXT_APPLY_BUTTON' => (($x>0)?(check_login("jobseeker")?tep_image_button(PATH_TO_BUTTON.'button_apply_selectedjob.gif', IMAGE_APPLY,'onclick="ckeck_application(\'\');" style="cursor:pointer;"'):tep_image_button(PATH_TO_BUTTON.'button_registered_user.gif', IMAGE_APPLY,'onclick="ckeck_application(\'\');" style="cursor:pointer;"').' '.tep_image_button(PATH_TO_BUTTON.'button_new_user.gif', IMAGE_APPLY,'onclick="ckeck_application(\'new\');" style="cursor:pointer;"')):''),
		...
		'INFO_TEXT_CHANGE_CRITERIA' => (tep_not_null($sID)?'<a class="btn btn-sm btn-danger float-right text-right" href="'.tep_href_link(FILENAME_JOB_ALERT_AGENT,'sID='.$sID).'" class="btn btn-sm btn-danger float-right text-right">Modificar</a>':'<a href="'.tep_href_link(FILENAME_JOB_ALERT_AGENT,'sID='.$save_search_id).'" class="btn btn-sm btn-danger float-right text-right">Modificar</a>'),

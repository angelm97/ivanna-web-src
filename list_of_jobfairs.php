...
/**********************/
   ...
   $template->assign_block_vars('jobfairs', array( 'row_selected' => $row_selected,
    ...
    'status' => ($row_check['approved']=='Yes'?'<font color="#0000ff">APPROVED</font>':($row_check['approved']=='No'?'<font color="#ff0000">PENDING</font>':'NOT APPLIED')),
    'noofjobs' => '<a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS,'jfID='.$ide).'">'.$no_of_jobs.'</a>',
    'action' => (!tep_not_null($row_check['approved'])?'<a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBFAIRS, tep_get_all_get_params(array('jobfairID','action'))).'&jobfairID='.$ide.'&action=apply_jobfair' . '" class="btn-text text-primary">'.TEXT_INFO_APPLY_NOW.'</a>':($row_check['approved']=='No'?'Applied':'Member')),//$action_image,

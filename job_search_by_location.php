...
if($action!='search' && $_GET['country']=='')
{
		...
		while($continent = tep_db_fetch_array($continent_result))
		{
			...
			$continent="<div class='label-text'>".tep_db_output($key1)."</div>";
			...
			$div_id='';
			$country_list='';
			$country_list.='<table class="table" border="0" width="100%"><tr>';
 ...
 $template->assign_vars(array(
  ...
  'button' => '<button class="btn btn-sm btn-primary" type="submit" id="button-addon2">Buscar</button>',//tep_image_submit(PATH_TO_BUTTON.'button_search.gif', IMAGE_SEARCH),
  ...
  'INFO_TEXT_COUNTRY1'  => LIST_SET_DATA(COUNTRIES_TABLE,"",TEXT_LANGUAGE.'country_name','id',"country_name",'name="country" class="form-control form-control-sm" ',INFO_TEXT_ALL_COUNTRIES,'',$country),
...
elseif($action!='search' && $_GET['country']!='')
{
	 $state_query  = "select * from ".ZONES_TABLE." where zone_country_id='".$country_id."' order by zone_name  asc";
		...
		$s=0;
		$state_list='';
		$state_list.='<table class="table" border="0" width="100%"><tr>';
		if($no_of_states>1)
...
else
{
 ...
 'INFO_TEXT_APPLY_BUTTON' => (($x>0)?(check_login("jobseeker")?'<a class="btn btn-success" onclick="ckeck_application(\'\');" role="button">Aplicar a oportunidades laborales seleccionadas</a>':'<a class="btn btn-primary" onclick="ckeck_application(\'\');" role="button">Usuario registrado</a> <a class="btn btn-success ml-2" onclick="ckeck_application(\'new\');" role="button">Nuevo usuario</a>'):''),

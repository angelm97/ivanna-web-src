...
$searchClause.=" ( jr1.experience_year >= '".$work_experince."' and jr1.experience_year <= '".$work_experince1."' ) and ";
  ...
  if($x > 0 && $x1 > 0)
  {
   ...
   while ($application = tep_db_fetch_array($result))
   {
    ...
    //  $applicant_selected=tep_image(PATH_TO_IMAGE.'icon_selected.gif',IMAGE_SELECTED);
     $applicant_selected='<span class="badge badge-success"><i class="fa fa-check" aria-hidden="true"></i> Selected</span>';
    else
     $applicant_selected ='';
    ...
    $match_percentage= getMacth($application['resume_id'],$application['job_id']);
    $template->assign_block_vars('jobs', array( 'row_selected' => $row_selected,
     ...
     'photo'=>(tep_not_null($application['jobseeker_photo']) && is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$application['jobseeker_photo'])?'<a class="img-fluid img-thumbnail applicant-photo mr-3" href="#" onclick="'.js_popup(PATH_TO_PHOTO.$application['jobseeker_photo'],SITE_TITLE).'">'.tep_image(FILENAME_IMAGE."?size=70&image_name=".PATH_TO_PHOTO.$application['jobseeker_photo'],tep_db_output(SITE_TITLE)).'</a>':''),
     'match'=>$match_percentage,
     'round'=>tep_not_null($round)?$round:'- - - - -',
     ));
     $alternate++;
     $lower = $lower + 1;
   }
 ...
 //////////////Right Round/////////////
  ...
  if($db_round_num_row > 0)
  {
   ...
   {
    $round_array[]=array('id'=>$s_round['id'],'name'=>INFO_TEXT_ROUND.' '.$s_round[TEXT_LANGUAGE.'round_name'].' ');
   }
  }
  tep_db_free_result($db_round_query);

  $heading2[]  = array('params'=>'','text'  => '<div class="list-group mt-4"><div class="list-group-item list-group-item-action info-bg font-weight-bold">'.INFO_TEXT_VIEW_CURRENT_STATUS.'</div></div>');
  $total_round_array=count($round_array);
  for($i=0;$i<$total_round_array;$i++)
  {
   $contents2[] = array('align' => 'left','params'=>'onmouseout="rowOutEffect(this)"', 'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="#" onclick="view_status(\''.$round_array[$i]['id'].'\')" >'.$round_array[$i]['name'].'</a>');
  }
  //////////////Shambhu/////////////
}
/////
...
if (is_object($aInfo))
{
 if($process_round)
 {
  $heading1[]  = array('text'  => '<div class="list-group"><div class="list-group-item list-group-item-action info-bg font-weight-bold">'.INFO_TEST_HEADER_SUMMERY.'</div></div>');
  $contents1[] = array('align' => 'left', 'text' => "<div class='list-group-item list-group-item-action px-3 py-2'>" .INFO_TEST_SUMMERY_NEW." : ".$no_of_new_application."</div>");
  $contents1[] = array('align' => 'left', 'text' => "<div class='list-group-item list-group-item-action px-3 py-2'>" .INFO_TEST_SUMMERY_SELECTED." : ".$no_of_select_application."</div>");
  //$contents1[] = array('align' => 'left', 'text' => INFO_TEST_SUMMERY_WAITING." : ".$no_of_waiting_application);
  $contents1[] = array('align' => 'left', 'text' => "<div class='list-group-item list-group-item-action px-3 py-2'>" .INFO_TEST_SUMMERY_REJECTED." : ".$no_of_reject_application."</div>");
  $contents1[] = array('align' => 'left', 'text' => "<div class='list-group-item list-group-item-action px-3 py-2'>" .INFO_TEST_SUMMERY_PROCESS." : ".$no_of_process_application."</div>");
  $contents1[] = array('align' => 'left', 'text' => "<div class='list-group-item list-group-item-action px-3 py-2'>".INFO_TEST_SUMMERY_TOTAL."</b> : ".$no_of_total_application."</div>"."");
 }

 $result_log_query_phone = tep_db_query("select jobseeker_id from application where application_id = '" . $aInfo->application_id . "'");
  $result_log_phone = tep_db_fetch_array($result_log_query_phone);
  $id_job_seeker = $result_log_phone['jobseeker_id'];

  $result_log_query_phone = tep_db_query("select jobseeker_mobile from jobseeker where jobseeker_id = '" . $id_job_seeker . "'");
  $result_log_phone = tep_db_fetch_array($result_log_query_phone);
  $job_seeker_phone_number = $result_log_phone['jobseeker_mobile'];
 ...
 $heading[] = array('params'=>'','text' => '<div class="list-group"><div class="list-group-item list-group-item-action info-bg font-weight-bold">' . tep_db_output($aInfo->full_name) . '<div><small>'.TEXT_INFO_APPLICANT_OPRATION.'</div></small></div></div>');
//  $contents[] = array('align' => 'center', 'text' => TEXT_INFO_APPLICANT_OPRATION);
 $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.$query_string2).'" target="_blank"  >'.IMAGE_PROFILE.'</a>');
 ...
 $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="#" onclick="set_action1(\''.$aInfo->id.'\',\'change_status\')" >'.IMAGE_CHANGE_STATUS.'</a>');
 $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="#" onclick="set_action1(\''.$aInfo->id.'\',\'show_history\')" >'.INFO_TEXT_SELECTION_HISTORY.'</a>');
  $contents[] = array('align' => 'left','params'=>'', 'text' => (($aInfo->applicant_select=='No')
  ?'<a class="list-group-item list-group-item-action px-3 py-2" href="#" onclick="set_action1(\''.$aInfo->id.'\',\'applicant_selected\')" >
  '.IMAGE_SELECTED.' <span class="badge badge-success float-right"><i class="fa fa-check" aria-hidden="true"></i> Select</span></a>'
  :'<a class="list-group-item list-group-item-action px-3 py-2" href="#" onclick="set_action1(\''.$aInfo->id.'\',\'applicant_selection_reset\')" >'.IMAGE_APPLICANT_SELECTION_RESET.'
  <span class="badge badge-danger float-right"><i class="fa fa-check" aria-hidden="true"></i> Unselect</span></a>'));
 $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_EMPLOYER_INTERACTION,"query_string=".$query_string.(tep_not_null($search_id)?"&search_id=".$search_id:"")).'" >'.IMAGE_CONTACT.'</a>');
 $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,"query_string=".$query_string2).'"  target="_blank" >'.INFO_TEXT_ADD_COMMENT.'</a>');
 $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.$query_string2).'" target="_blank" >'.IMAGE_RATING.'</a>');
 if($from_list=='Y')
 {
  ...
  // $contents[] = array('align' => 'left','params'=>'class="dataTableRightRow1" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',  'text' => '<img src="img/red_rec.gif"> <a href="#" onclick="set_action1(\''.$aInfo->id.'\',\'\')" class="right_black" >'.IMAGE_CANCEL.'</a>');
   $contents[] = array('align' => 'left', 'text' => TEXT_DELETE_INTRO);
   $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="#" onclick="set_action1(\''.$aInfo->id.'\',\'applicant_confirm_delete\')"  >'.INFO_TEXT_DELETE_CONFIRM.'</a>');
  // $contents[] = array('align' => 'left', 'text' => TEXT_DELETE_WARNING);

  }
  else
   $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-2" href="#" onclick="set_action1(\''.$aInfo->id.'\',\'applicant_delete\')"  >'.INFO_TEXT_DELETE.'</a>');
 }

 $contents[] = array('align' => 'left',
                    'params'=>'',
                    'text' => '<a class="list-group-item list-group-item-action px-3 py-2"
                                  href="https://wa.me/' . '1' . $job_seeker_phone_number . '?text="  onclick="window.open(\''.tep_href_link(FILENAME_RECRUITER_RESUME_MATCH,
...
$template->assign_vars(array(
  ...
  'INFO_TEXT_ALL_APPLICANT'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,'jobID='.$job_id).'">'.INFO_TEXT_ALL_APPLICANT.'</a>',
  'INFO_TEXT_SELECTED_APPLICANT'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_SELECTD_APPLICANT,'jobID='.$job_id).'" >'.INFO_TEXT_SELECTED_APPLICANT.'</a>',
  'INFO_TEXT_SEARCH_APPLICANT'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,'jobID='.$job_id."&search=applicant").'" >'.INFO_TEXT_SEARCH_APPLICANT.'</a>',
  'INFO_TEXT_JOB_DETAIL'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_POST_JOB,'jobID='.$job_id).'" target="_blank">'.INFO_TEXT_JOB_DETAIL.'</a>',
  'INFO_TEXT_REPORT_PIPELINE'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT,'jobID='.$job_id)."#Pipeline ".'">'.INFO_TEXT_REPORT_PIPELINE.'</a>',
  'INFO_TEXT_REPORT_ROUNDWISE'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT,'jobID='.$job_id)."#roundwise".'">'.INFO_TEXT_REPORT_ROUNDWISE.'</a>',
  'INFO_TEXT_REPORT_ROUNDWISE_SUMMARY'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT,'jobID='.$job_id)."#roundwise_summary".'">'.INFO_TEXT_REPORT_ROUNDWISE_SUMMARY.'</a>',
  'INFO_TEXT_VIEW_DATE_REPORT'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT,'jobID='.$job_id."#date_report").'">'.INFO_TEXT_VIEW_DATE_REPORT.'</a>',
  'INFO_TEXT_ADD_APPLICANT'=>'<a class="list-group-item list-group-item-action px-3 py-2" href="'.tep_href_link(FILENAME_RECRUITER_SEARCH_RESUME).'" target="_blank">'.INFO_TEXT_ADD_APPLICANT.'</a>',
 ));
...
if($action1 =='compair_profile')
{
 ...
 {
  $change_order='<a class="btn btn-sm btn-outline-secondary" href="#" onclick="document.compair_profile.submit();"><i class="fa fa-exchange"></i></a>';
...
//  'back_buttons'              =>'<a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,"jobID=".$job_id).'"><button class="btn btn-sm btn-secondary">Back</button></a>',
 'back_buttons'              =>'<a class="btn btn-sm btn-secondary float-right" href="#" onclick="javascript:history.back();">Atrás</a>',
...
//////////////////////////////////////////////////
 ...
 if($db_applicant_num_row > 0)
 {
  ...
  while ($application = tep_db_fetch_array($db_applicant_query))
  {
   ...
   //else
    $status_drop_down=LIST_SET_DATA(APPLICATION_STATUS_TABLE,"",'application_status','id',"priority desc,id ",'class="form-control form-control-sm" name="new_status"',"",'',$latest_status['id']);
   $template->assign_block_vars('application_set_status',array( 'row_selected' => $row_selected,
    'round'         => tep_db_output(INFO_TEXT_ROUND.' '.$application[TEXT_LANGUAGE.'round_name']).tep_draw_hidden_field('round',$application['id']),
 ...
 $template->assign_vars(array(
  ...
  'button'=>'<button class="btn btn-sm btn-primary" type="submit">Actualizar</button>',//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE),
  'LEFT_BOX_WIDTH'=>LEFT_BOX_WIDTH1,
  'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,
  'LEFT_HTML'=>'',
  'RIGHT_HTML'=>$RIGHT_HTML,
 ...
 if($db_app_status_num_row > 0)
 {
  ...
  {//print_r($application_status);die();
   ...
   $template->assign_block_vars('application_status',array( 'row_selected' => $row_selected,
    ...
    'rate_button'  => '<button class="btn btn-primary" type="submit">Velocidad</button>',//tep_image_submit(PATH_TO_BUTTON.'button_rate.gif',IMAGE_RATE),
    ));
    $alternate++;
  }
 }
...
//  $list_application=
 ...
 $template->assign_vars(array(
  ...
  'buttons'           => '<button class="btn btn-primary mr-2" type="submit">Vista previa</button><a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,"jobID=".$job_id).'"><button class="btn btn-secondary">Cancelar</button></a>',
...
elseif($action1=='preview')
{
 ...
 $template->assign_vars(array(
  ...
  'buttons'=>'<button class="btn btn-primary mr-2" type="submit" name="send_mail">Enviar correo</button> <a href="#" onclick="javascript: set_action(\'back\');"><button class="btn btn-secondary">Atrás</button></a>',//tep_image_submit(PATH_TO_BUTTON.'button_send_mail.gif', IMAGE_SEND_MAIL, 'name="send_mail"'),
...
elseif($action1=='search_applicant')
{
 ...
 'buttons'               => '<button class="btn btn-primary" type="submit">Buscar</button>',//tep_image_submit(PATH_TO_BUTTON.'button_search.gif', IMAGE_SEARCH),
 ...
 'INFO_TEXT_TRACKING_ROUND1' => LIST_SET_DATA(SELECTION_ROUND_TABLE,"",TEXT_LANGUAGE.'round_name','id',"value",'name="process_round" class="form-control"',INFO_TEXT_ALL,'',$process_round),
 'INFO_TEXT_APPLICATION_STATUS1' => LIST_SET_DATA(APPLICATION_STATUS_TABLE,"",'application_status','id',"priority",'name="application_status" class="form-control" ',INFO_TEXT_ALL,'',$application_status),
 ...
 'INFO_TEXT_EMAIL_ADDRESS1'=> tep_draw_input_field('TNEF_email_address',$email_address, 'class="form-control" placeholder="Correo electrónico"'),
  'INFO_TEXT_EXPERIENCE'   => INFO_TEXT_EXPERIENCE,
  'INFO_TEXT_EXPERIENCE1'  => '<select name="experience" class="form-control">
							<option value="">Cualquier experiencia</option>
							<option value="0-1">Menos de 1 año</option>
							<option value="2-5">2 años - 5 años</option>
							<option value="6-9">6 años -9 años</option>
							<option value="10-100">Más de 10 años</option>
...
else
{
 ...
 $check_link1='
              <div class="card mt-2">
				...
				<div class="badge badge-secondary mb-2">'.INFO_TEXT_WITH_SELECTED.'</div>
				<select name="select_action"  class="form-control form-control-sm" onchange="select_action2();">
 ...
 $template->assign_vars(array(
  ...
  "INFO_TEXT_APPLICATION_STATUS1" => LIST_SET_DATA(APPLICATION_STATUS_TABLE,"",'application_status','id',"priority",'name="application_status" class="form-control form-control-sm" onchange="document.page.submit()"',INFO_TEXT_ALL,'',$application_status),
  "INFO_TEXT_TRACKING_ROUND" => INFO_TEXT_TRACKING_ROUND,
  "INFO_TEXT_TRACKING_ROUND1" => LIST_SET_DATA(SELECTION_ROUND_TABLE,"",TEXT_LANGUAGE.'round_name','id',"value",'name="process_round" class="form-control form-control-sm" onchange="document.page.submit()"',INFO_TEXT_ALL,'',$process_round),
  'new_button'=>'',

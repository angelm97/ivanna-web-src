...
switch($request)
{
 case 'admin_info':
  if(!isset($_SESSION['access_token']))
   ...
   if(isset($_SESSION['facebook_page']))
   {
    $page_url = $_SESSION['facebook_page'];
     $content = $connection->get('https://graph.facebook.com/v2.8/'.urlencode($page_url).'?&access_token='.$token);
     $page_id=$content['id'];
   ...
   else
   {
    ...
    $content = $connection->get('https://graph.facebook.com/v2.8/me?fields=id&&access_token='.$token);
	...
	{
     $content = $connection->get('https://graph.facebook.com/v2.8/me/accounts?fields=id&limit=100&&access_token='.$token);
 ...
 case 'jobseeker_info':
   ...
   $connection->setAccessToken($_SESSION['access_token']);
   $token=$connection->accessToken['access_token'];
   $content = $connection->get('https://graph.facebook.com/v2.8/me?fields=id,email&access_token='.$token);

 ...
 if($x > 0)
 {
  ...
  while($row = tep_db_fetch_array($result))
  {
   ...
   $template->assign_block_vars('rates'.$z, array(
    ...
    'radio' =>tep_draw_radio_field('product_id',$row['id'],'','','id="product_'.$row['id'].'"'),
    'name' => '<label for="product_'.$row['id'].'">'.tep_db_output($row[TEXT_LANGUAGE.'plan_type_name']).'</label>',
...
$template->assign_vars(array(
 ...
 'BUY_BUTTON'=>'<button class="btn btn-sm btn-primary">'.BUY_NOW.'</button>',

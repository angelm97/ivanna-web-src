...
{
 $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
 $skills_button='<button class="btn btn-primary mr-2" type="submit">Guardar</button>';//tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE,"");
 ...
 $languages_button='<button class="btn btn-primary mr-2" type="submit">Guardar</button>';//tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE,"");
 ...
 if($_POST['action']=="edit_skills" || $_POST['action']=="add_skills")
 {
  if($_POST['action']=="edit_skills")
  {
   ...
   $skills_button     = '<button class="btn btn-secondary mr-2" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
  ...
  {
  $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
  $skills_button = '<button class="btn btn-primary mr-2" type="submit" onclick=set_action("add_skills")>Guardar</button>';//tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action('add_skills')");
  $skills_form=tep_draw_form('skills', FILENAME_JOBSEEKER_RESUME4, '', 'post', 'onsubmit="return ValidateForm(this)" ').tep_draw_hidden_field('query_string',$query_string).tep_draw_hidden_field('action','add_skills');
  }
 }
 elseif($_POST['action']=="edit_languages" || $_POST['action']=="add_languages")
 {
  if($_POST['action']=="edit_languages")
  {
   ...
   $proficiency         = $proficiency;
   $query_string=encode_string("resume_id@@@".$_POST['resume_id']."@@@resume");
   $languages_button     = '<button class="btn btn-secondary mr-2" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
  ...
  {
   $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
   $languages_button = '<button class="btn btn-primary mr-2" type="submit" onclick=set_action1("add_languages")>Guardar</button>';//tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action1('add_languages')");
   $languages_form=tep_draw_form('languages', FILENAME_JOBSEEKER_RESUME4, '', 'post', 'onsubmit="return ValidateForm(this)" ').tep_draw_hidden_field('query_string',$query_string).tep_draw_hidden_field('action','add_languages');
  }
 }
}
...
$skills_input_new ='<select id="country" name="TR_skill" class="form-control" style="height:100px !important; padding:15px !important;" >
...
$skills_input_new.= '</select>';
 ...
 /*********************************************************/
$query_string       = encode_string("resume_id@@@".$resume_id."@@@resume");
$add_next_button    = "<a href='".tep_href_link(FILENAME_JOBSEEKER_RESUME5."?query_string=".$query_string)."' class='btn btn-secondary'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
...
$add_next_button1    = "<a class='btn btn-secondary mr-2' href='".tep_href_link(FILENAME_JOBSEEKER_RESUME5."?query_string=".$query_string)."' >Siguiente página</a>";
...
$resume1='<a class="list-group-item list-group-item-action" href ="#"  onclick="document.resume.submit()"><i class="fa fa-file-text icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_RESUME.'</a>';
    ...
    $resume3='<a class="list-group-item list-group-item-action" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME3.'?query_string='.$query_string).'" ><i class="fa fa-graduation-cap icon-resume-left" aria-hidden="true"></i> '.INFO_TEXT_LEFT_EDUCATION.'</a>';
		  $resume4='<a class="list-group-item list-group-item-action active" href ="'.tep_href_link(FILENAME_JOBSEEKER_RESUME4.'?query_string='.$query_string).'" ><i class="fa fa-wrench icon-resume-left active-icon" aria-hidden="true"></i> '.INFO_TEXT_LEFT_SKILLS.'</a>';
...
'TABLE_HEADING_PROFICIENCY'        => TABLE_HEADING_PROFICIENCY,
 ...
 'INFO_TEXT_SKILL1'                => $skills_input_new,
 'INFO_TEXT_SKILL_LEVEL'           => INFO_TEXT_SKILL_LEVEL,
 'INFO_TEXT_SKILL_LEVEL1'          => LIST_SET_DATA(SKILL_LEVEL_TABLE,"",TEXT_LANGUAGE.'skill_name','id',"priority",'name="skill_level" class="form-control" ',"Seleccionar",'',$skill_level),
 'INFO_TEXT_LAST_USED'             => INFO_TEXT_LAST_USED,
 'INFO_TEXT_LAST_USED1'            => LIST_SET_DATA(SKILL_LAST_USED_TABLE,"",TEXT_LANGUAGE.'skill_last_used','id',"priority",'name="last_used" class="form-control" ',"Seleccionar",'',$last_used),
 'INFO_TEXT_YEARS_OF_EXP'          => INFO_TEXT_YEARS_OF_EXP,
 'INFO_TEXT_YEARS_OF_EXP1'         => tep_draw_input_field('years_of_exp', $years_of_exp,'class="form-control"  size="46"', true, 'number'),

 'INFO_TEXT_LANGUAGE'              => INFO_TEXT_LANGUAGE,
 'INFO_TEXT_LANGUAGE1'             => LIST_SET_DATA(JOBSEEKER_LANGUAGE_TABLE,"",TEXT_LANGUAGE.'name','languages_id',"name",'name="TR_languages" class="form-control" ',"Seleccionar",'',$languages),
 'INFO_TEXT_PROFICIENCY'           => INFO_TEXT_PROFICIENCY,
 'INFO_TEXT_PROFICIENCY1'          => LIST_SET_DATA(LANGUAGE_PROFICIENCY_TABLE,"",TEXT_LANGUAGE.'language_proficiency','id',"priority",'name="TR_proficiency" class="form-control" ',"Seleccionar",'',$proficiency),

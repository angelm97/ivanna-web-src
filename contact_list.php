...
{
 $add_save_button='<button class="btn btn-primary" type="submit">Agregar</button>';
...
//   if(tep_not_null($user_state) && tep_not_null($row['country_name']))
  ...
  see_page_number();
  $template->assign_vars(array('total'=>"Aún $x1 tienes contactos."));
 }
 else
 {
  $template->assign_vars(array('total'=>"Aún no tienes contactos."));
 }
...
$template->assign_vars(array(
 ...
 'add_new'                => "<a class='btn btn-sm btn-primary' href='#' onclick='set_action(\"new\")'>".HEADING_TITLE1."</a>",
 'contact_form'           => $contact_form,

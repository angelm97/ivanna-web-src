 ...
 switch($action1)
 {
  case 'save_search':
   ...
   $template->assign_vars(array(
        ...
        'INFO_TEXT_TITLE_NAME1'=>tep_draw_input_field('TR_search_name', $search_name,'size="26" class="form-control form-control-sm" aria-describedby="button-addon2" placeholder="'.INFO_TEXT_TITLE_NAME.'" maxlength="32"',true)));
 }
}
// search
...
$whereClause=(tep_not_null($whereClause)?$whereClause.' and ( ':' ( ');
if($job_salary==1)
		$whereClause.=" j.job_salary >=10000 and j.job_salary <=15000";
elseif($job_salary==2)
		$whereClause.=" j.job_salary >15000 and j.job_salary <=20000";
elseif($job_salary==3)
		$whereClause.=" j.job_salary >20000 and j.job_salary <=25000";
elseif($job_salary==4)
		$whereClause.=" j.job_salary >25000 and j.job_salary >=30000";
elseif($job_salary==5)
		$whereClause.=" j.job_salary >30000 and j.job_salary <=35000";
elseif($job_salary==6)
		$whereClause.=" j.job_salary >35000 and j.job_salary <=50000";
else
		$whereClause.=" j.job_salary >50000";
...
// job_salary ends ///
   ...
   $field_names="j.job_id, j.job_title, j.re_adv, j.inserted, j.job_short_description,  j.recruiter_id,j.min_experience,j.max_experience,j.job_salary,j.job_industry_sector,j.job_type,jt.type_name,j.expired,j.recruiter_id,r.recruiter_company_name,r.recruiter_logo,r.recruiter_applywithoutlogin, j.job_source,j.post_url,j.url,j.job_featured,concat(case when j.job_location='' then '' else concat(j.job_location,', ') end, if(j.job_state_id,z.zone_name,j.job_state)) as location ,c.country_name,job_skills"; //j.job_state, j.job_state_id,j.job_country_id
...
/////////////////////////////////////////////////////////////////
     ...
     $recruiter_logo=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_LOGO.$company_logo."&size=120");
					...
					$email_job    ='<a class="small mr-3" href="'.tep_href_link(FILENAME_TELL_TO_FRIEND,'query_string='.$query_string).'" title="'.tep_db_output(INFO_TEXT_EMAIL_THIS_JOB).'" target="_blank"><i class="fa fa-envelope-o mr-1" aria-hidden="true"></i> '.INFO_TEXT_EMAIL_THIS_JOB.'</a>';
...
//////**********currency display ***************************/
     $save_job    ='<div id="'.$query_string.'"  class="nav-item" ><a class="small mr-3" href="'.tep_href_link(FILENAME_JOBSEEKER_LOGIN).'" title="'.INFO_TEXT_SAVE_JOB.'"><i class="fa fa-heart-o icon-unsaved mobile-absolute-right"></i><!-- '.INFO_TEXT_SAVE_JOB.'--></a></div>';
...
$template->assign_block_vars('job_search_result', array(
			...
			'description' => nl2br(tep_db_output(strip_tags(substr($row['job_short_description'],0,200).'. . .'))),
    ...
    $template->assign_vars(array('total'=>"<font color='red'><b>".$x1." </b></font> ".INFO_TEXT_TO_YOUR_SEARCH_CRITERIA));
   }
   else
   {
    $template->assign_vars(array('content_hide'=>'result_hide',
...
if(check_login("jobseeker"))
{
	$save_search= tep_draw_form('save_search', FILENAME_JOB_SEARCH,($edit?'sID='.$save_search_id:''),'post','onsubmit="return ValidateForm(this)" class="float-right"').tep_draw_hidden_field('action1','save_search');
    $INFO_TEXT_ALERT_TEXT=(($action1=='save_search')?'':"<a href='#' class='btn btn-sm btn-primary float-right' onclick='document.save_search.submit();'><i class='fa fa-bell' aria-hidden='true'></i> Crear alerta de oportunidad laboral</a>");
}
else
{
	 ...
	 $INFO_TEXT_ALERT_TEXT=$save_search.tep_draw_input_field('TREF_job_alert_email', $TREF_job_alert_email,'class="form-control form-control-sm" placeholder="Email id" ',false)."<button type='submit' class='btn btn-primary btn-sm float-left'><i class='fa fa-bell' aria-hidden='true'></i> Crear alerta de oportunidad laboral</button></form>";
}
...
if($action!='search')
{
 $template->assign_vars(array( 'hidden_fields' => $hidden_fields,
  ...
  'INFO_TEXT_KEYWORD1'     => tep_draw_input_field('keyword', $keyword, 'placeholder="Busca por palabras clave" class="form-control"'),
  'INFO_TEXT_KEYWORD_CRITERIA'=>INFO_TEXT_KEYWORD_CRITERIA,
  'INFO_TEXT_KEYWORD3'     => '<div class="custom-control custom-radio custom-control-inline">'.tep_draw_radio_field('word1', 'Yes', '', $word1,'id=radio_word1 class="custom-control-input"').'<label for="radio_word1" class="custom-control-label ml-1">'.INFO_TEXT_KEYWORD_WORD1.'</label></div>
<div class="custom-control custom-radio custom-control-inline">'.tep_draw_radio_field('word1', 'No', '', $word1,'id=radio_word2 class="custom-control-input"').'<label for="radio_word2" class="custom-control-label">'.INFO_TEXT_KEYWORD_WORD2.'</label></div>',
  ...
  'INFO_TEXT_COUNTRY1'     => LIST_TABLE(COUNTRIES_TABLE,TEXT_LANGUAGE."country_name","priority","name='country' class='form-control'","All countries","",$country),
...
':''),
  ...
  'INFO_TEXT_JOB_SALARY1'  => LIST_TABLE(JOB_SALARY_TABLE,TEXT_LANGUAGE."sal_name","priority","name='job_salary' class='form-control'","Cualquier salario","",$job_salary),
  'INFO_TEXT_JOB_TYPE'      => INFO_TEXT_JOB_TYPE,
  'INFO_TEXT_JOB_TYPE1'     => LIST_TABLE(JOB_TYPE_TABLE,TEXT_LANGUAGE."type_name","priority","name='job_type' class='form-control'","Todas","",$job_type),

  'INFO_TEXT_JOB_CATEGORY' => INFO_TEXT_JOB_CATEGORY,
  'INFO_TEXT_JOB_CATEGORY_TEXT' => INFO_TEXT_JOB_CATEGORY_TEXT,
  'INFO_TEXT_JOB_CATEGORY1'=> tep_draw_pull_down_menu('job_category[]', $cat_array, explode(",",$job_category1), 'class="form-control"', false),
  'INFO_TEXT_EXPERIENCE'   => INFO_TEXT_EXPERIENCE,
  'INFO_TEXT_EXPERIENCE1'  => experience_drop_down('name="experience" class="form-control"', INFO_EXPERIENCE, '', $experience),
  'INFO_TEXT_JOB_POSTED'   => INFO_TEXT_JOB_POSTED,
  'INFO_TEXT_JOB_POSTED1'  => LIST_SET_DATA(JOB_POSTED_TABLE,"",TEXT_LANGUAGE.'type_name','value',"priority","name='job_post_day' class='form-control'" ,INFO_TEXT_DEFAULT_JOB_POST_DAY,'',$job_post_day),
  'button'                 => '<button class="btn btn-primary" type="submit">Buscar</button>',//tep_image_submit(PATH_TO_BUTTON.'button_search.gif', IMAGE_SEARCH),
...
{$key1=(tep_not_null($keyword)?$key1=$keyword:'keyword');
 ...
 'INFO_TEXT_APPLY_BUTTON' => (($x>0)?(check_login("jobseeker")?'<a class="btn btn-primary" onclick="ckeck_application(\'\');" role="button">Apply to Selected Jobs</a>':'<a class="btn btn-primary" href="'.tep_href_link(FILENAME_JOBSEEKER_LOGIN).'">Registered User</a><a class="ml-2 btn btn-success" href="'.tep_href_link(FILENAME_JOBSEEKER_REGISTER1).'">New User</a>'):''),
...
// 'INFO_TEXT_ALERT_SAVE'   => (($action1=='save_search')
  ...
  'INFO_TEXT_ALERT_SAVE'   => (($action1=='save_search')
  ?tep_button_submit('btn btn-sm btn-primary', IMAGE_SAVE)
  .($action1=='save_search'?'<a href="'.tep_href_link(FILENAME_JOBSEEKER_LIST_OF_SAVED_SEARCHES).'" class="btn btn-sm btn-secondary">'.IMAGE_CANCEL.'</a>':'')
  .'<a href="'.tep_href_link(FILENAME_JOB_SEARCH).'" class="btn btn-sm btn-secondary">'.IMAGE_BACK.'</a>':''),

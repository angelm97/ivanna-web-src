  ...
  if(tep_not_null($action))
  {
    ...
    if($db_job_num_row > 0)
    {
      ...
      while ($job = tep_db_fetch_array($db_job_query))
      {
        ...
        {
          $status='<a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID','action'))).'&jobID='.$job['job_id'].'&action=job_inactive' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOB_INACTIVATE, 30, 17) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOB_ACTIVE, 30, 17);
        }
        else
        {
          $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOB_INACTIVE, 30, 17) . '&nbsp;<a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID','action'))).'&jobID='.$job['job_id'] . '&action=job_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOB_ACTIVATE, 30, 17) . '</a>';
        }
if($job['deleted']!=NULL)
	echo 'deleted';
...
switch ($action)
{
  case 'delete_screener':
	...
	<div class="list-group">
	  <a href="#" class="list-group-item list-group-item-action font-weight-bold info-bg">
...
';
  ...
  case 'del_job':
    ...
    <div class="list-group">
	  ...
	  <div class="list-group-item">
	  <div class="text-danger small mb-2">'.TEXT_DELETE_INTRO.'</div>
	  ...
	  <div class="small text-muted mt-3">'.TEXT_DELETE_WARNING.'</div>
	  </div>
	</div>
';
  break;
...
<div class="list-group">
  <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID'))).'&jobID='.$jInfo->job_id.'&action=readv_job' . '" class="list-group-item list-group-item-action px-3 py-2">'.IMAGE_READVERTISE.'</a>
</div>';
...
<div class="list-group">
  <a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID'))).'&jobID='.$jInfo->job_id.'&action=edit_screener'.'" class="list-group-item list-group-item-action px-3 py-2">'.IMAGE_EDIT_SCREENER.' <span class="float-right"><i class="fa fa-edit text-muted" aria-hidden="true"></i></span></a>
</div>

<div class="list-group">
  <a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID'))).'&jobID='.$jInfo->job_id.'&action=delete_screener'.'" class="list-group-item list-group-item-action px-3 py-2">'. IMAGE_DELETE_SCREENER.' <span class="float-right"><i class="fa fa-trash text-muted" aria-hidden="true"></i></span></a>
</div>
';
}
else
{
  ...
  <a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID'))).'&jobID='.$jInfo->job_id.'&action=add_screener'.'" class="list-group-item list-group-item-action px-3 py-2">'.IMAGE_ADD_SCREENER.' <span class="float-right"><i class="fa fa-plus text-muted" aria-hidden="true"></i></span></a>';
}
if ((tep_not_null($jInfo->job_id)) && !(tep_not_null($action)))
{
  ...
  <div class="list-group">
	  <span class="list-group-item list-group-item-action font-weight-bold info-bg">
		'.ucfirst(tep_db_output($jInfo->job_title)).'
	  </span>
	  <div class="small text-muted p-2">'.TEXT_INFO_EDIT_JOB_INTRO.'</div>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_VIEW_JOB,'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action px-3 py-2">'.IMAGE_VIEW_JOB.' <span class="float-right"><i class="fa fa-eye text-muted" aria-hidden="true"></i></span></a>
	  '.$re_adv.
	  (tep_not_null($jInfo->deleted)?'<a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID','action'))).'&jobID='.$jInfo->job_id.'&action=undelete_job' . '" class="list-group-item list-group-item-action px-3 py-2">'.IMAGE_UNDELETE_JOB.' <span class="float-right"><i class="fa fa-undo" aria-hidden="true"></i></span></a>':'<a href="' . tep_href_link(FILENAME_RECRUITER_POST_JOB,'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action px-3 py-2">'.IMAGE_EDIT_JOB.' <span class="float-right"><i class="fa fa-pencil text-muted" aria-hidden="true"></i></span></a>').'
	  <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID','action'))).'&jobID='.$jInfo->job_id.'&action=del_job' . '" class="list-group-item list-group-item-action px-3 py-2">'.IMAGE_DELETE_JOB.' <span class="float-right"><i class="fa fa-trash text-muted" aria-hidden="true"></i></span></a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action px-3 py-2" tabindex="-1" aria-disabled="true">'.IMAGE_APPLICATIONS.' <span class="float-right"><i class="fa fa-file-o text-muted" aria-hidden="true"></i></span></a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_SELECTD_APPLICANT, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action px-3 py-2" tabindex="-1" aria-disabled="true">'.IMAGE_SELECTED_APPLICATIONS.' <span class="float-right"><i class="fa fa-users text-muted" aria-hidden="true"></i></span></a>

	  '.$add_edit_screener.'

	  <a href="' . tep_href_link(FILENAME_RECRUITER_RESUME_WEIGHT, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action px-3 py-2" tabindex="-1" aria-disabled="true">'.INFO_TEXT_RESUME_WEIGHT.' <span class="float-right"><i class="fa fa-tasks text-muted" aria-hidden="true"></i></span></a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action px-3 py-2" tabindex="-1" aria-disabled="true">'.IMAGE_REPORT.' <span class="float-right"><i class="fa fa-pie-chart text-muted" aria-hidden="true"></i></span></a>
    <a href="/images_post.php?jobID='.$jInfo->job_id . '" class="list-group-item list-group-item-action px-3 py-2" tabindex="-1" aria-disabled="true">Generar Imagen <span class="float-right"><i class="fa fa-picture-o text-muted" aria-hidden="true"></i></span></a>
	</div>
  ...
  <div class="list-group">
	  ...
	  <a href="dddd" class="list-group-item list-group-item-action">ddd</a>
	  '.$re_adv.'
	  <a href="' . tep_href_link(FILENAME_RECRUITER_VIEW_JOB,'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action">'.IMAGE_VIEW_JOB.'</a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_POST_JOB,'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action">'.IMAGE_EDIT_JOB.'</a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS, tep_get_all_get_params(array('jobID','action'))).'&jobID='.$jInfo->job_id.'&action=del_job' . '" class="list-group-item list-group-item-action">'.IMAGE_DELETE_JOB.'</a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action">'.IMAGE_APPLICATIONS.'</a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_SELECTD_APPLICANT, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action">'.IMAGE_SELECTED_APPLICATIONS.'</a>
	  '.$add_edit_screener.'
	  <a href="' . tep_href_link(FILENAME_RECRUITER_RESUME_WEIGHT, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action">'.INFO_TEXT_RESUME_WEIGHT.'</a>
	  <a href="' . tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT, 'jobID='.$jInfo->job_id).'" class="list-group-item list-group-item-action">'.IMAGE_REPORT.'</a>
	</div>
 -->
  ';
}
...
if (isset($_SESSION["puntaje_localizacion"])) {

  $result_log_query = tep_db_query("select job_id from jobs where recruiter_id = '" . (int)$_SESSION['sess_recruiterid'] . "' ORDER BY job_id DESC LIMIT 1");
  $result_log = tep_db_fetch_array($result_log_query);
  $jobseeker_info = $result_log;

  $last_job_id = ($jobseeker_info['job_id']);
  ...
  tep_db_perform('resume_weight', $sql_data_array);


  unset($_SESSION["puntaje_localizacion"]);
  unset($_SESSION["puntaje_industria"]);
  unset($_SESSION["puntaje_education"]);
  unset($_SESSION["puntaje_experiencia"]);
  unset($_SESSION["puntaje_skill"]);

}
//$_SESSION['sess_recruiterid']
if($screener_file)
{
  $template->assign_vars(array(
    ...
    'button'=>'<button class="btn btn-primary" type="submit">Guardar Preguntas</button>',//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE),

...
class all_recruiters 
{
 ...
 function confirm() 
 {
  $mail_query = tep_db_query("select count(recruiter_id) as count from " . RECRUITER_TABLE." where  recruiter_newsletter='Yes'" );
 ...
 function send($id) 
 {
  ...
  else
  {
   ...
   /////////////////////////////////////
   $query_count=getAnyTableWhereData(RECRUITER_LOGIN_TABLE . " as rl left outer  join ".RECRUITER_TABLE." as r  on (rl.recruiter_id=r.recruiter_id )","recruiter_newsletter ='Yes'",'count(r.recruiter_id) as count ');
   ...
   <html>
   <head>
   <title>Recruiter Newslewtter</title>
  ...
  <div id="status1" class="statusbox" style="z-index:1; top: 90px;">
	 ...
	 <br>
	 <br>
	 Total Recruiter <?php echo $loopsize; ?> 
	 </div>
  <div class="mailbar">
  <div class="baritems">
  <?
   ...
   {
    $mail_query = tep_db_query("select jl.recruiter_email_address,concat(j.recruiter_first_name,' ',j.recruiter_last_name) as full_name from " . RECRUITER_LOGIN_TABLE . " as jl, ".RECRUITER_TABLE." as j where jl.recruiter_id=j.recruiter_id and recruiter_newsletter='Yes' limit $lower_limit ,$upper_limit");
    ...
    while ($mail = tep_db_fetch_array($mail_query)) 
    {
     ...
     $email_address_unsub=encode_string("email###".$mail['recruiter_email_address']."###unsubscribe");
     $email_message.="<br><p>  <font color='#ff0000'>*</font> This is not an unsolicited message. You are getting this mail as you are a registered user of ".SITE_TITLE." To unsubscribe please <a href='". tep_href_link(FILENAME_UNSUBSCRIBE,'user_type=recruiter&email='.$email_address_unsub)."'><font color='#0000ff'>Click Here</font></a> ";
     $mimemessage->add_html(stripslashes($email_message));
     $mimemessage->build_message();
     ///////////
     $to_name=tep_db_output($mail['full_name']);
     $to_addr=tep_db_output($mail['recruiter_email_address']);
     $from_name=tep_db_output(SITE_OWNER);
     $from_addr=tep_db_output(EMAIL_FROM);
     $subject=tep_db_output($this->title);
     $mimemessage->send($to_name, $to_addr, $from_name, $from_addr, $subject);
   ...
   <strong>Recruiter Newsletter Sending Complated</strong>
   <br>
   <br>
   Recruiter <?php echo $loopsize; ?>
   </div>
   <?php
	  if($redirect_url != "")
   { 

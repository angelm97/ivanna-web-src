...
if(tep_db_num_rows($zip_code_query) > 0)
{
 ...
 while ($zip_codes = tep_db_fetch_array($zip_code_query)) 
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_ZIP_CODES . '?page='.$_GET['page'].'&id=' . $cInfo->zip_code. '&action=edit&'.tep_get_all_get_params(array('action','id','page')).'\'"';
  } 
  else 
  {
...
switch ($action) 
{
 case 'new':
 case 'insert':
		$heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_ZIP_CODE.'</b>');
  $contents = array('form' => tep_draw_form('zip_code', PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => TEXT_INFO_ZIP_CODE_STATE.'<br>'.LIST_SET_DATA(ZONES_TABLE," where zone_country_id in (222,223)",'zone_name','zone_id',"zone_country_id,zone_name",'name="TR_state"',"",'',$state));
		...
		$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'edit':
 case 'save':
  ...
  $contents = array('form' => tep_draw_form('zip_code', PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES, 'id=' . $cInfo->zip_code.'&page='.$_GET['page'].'&action=save','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_ZIP_CODE.'<br>'.tep_draw_input_field('TR_zip_code',isset($_POST['zip_code'])?$zip_code:$cInfo->zip_code, '' ,true));
		$contents[] = array('text' => '<br>'.INFO_TEXT_ZIP_CODE_CITY.'<br>'.tep_draw_input_field('city',isset($_POST['city'])?$city: $cInfo->city, '' ));
  if(!$error)
  {
  ...
  $contents[] = array('text' => TEXT_INFO_ZIP_CODE_STATE.'<br>'.LIST_SET_DATA(ZONES_TABLE," where zone_country_id in (222,223)",'zone_name','zone_id',"zone_country_id,zone_name",'name="TR_state"',"",'',$state));
		$contents[] = array('text' => TEXT_INFO_ZIP_CODE_LATITUDE.'<br>'.tep_draw_input_field('TR_latitude', $latitude, '',true ));
		$contents[] = array('text' => TEXT_INFO_ZIP_CODE_LONGITUDE.'<br>'.tep_draw_input_field('TR_longitude', $longitude,'',true ));		
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES,'id=' . $cInfo->zip_code). '&'.tep_get_all_get_params(array('action','id')).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
  $heading[] = array('text' => '<b>' . $cInfo->zip_code. '</b>');
  ...
  $contents[] = array('text' => '<br><b>' . $cInfo->zip_code. '</b>');
  $contents[] = array('align' => 'center', 'text' => '
  <table>
   <tr>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete&'.tep_get_all_get_params(array('action','id','page'))) . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>
   <td>       
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES, 'page=' . $_GET['page'] . '&id=' . $_GET['id']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>
   </td>
   </tr>
   </table>
  ');
 ...
 default:
  if (isset($cInfo) && is_object($cInfo)) 
		{
   $heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_ZIP_CODE.'</b>');
   $contents[] = array('text' => tep_db_output($cInfo->zip_code));
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES, 'page=' . $_GET['page'] .'&id=' . $cInfo->zip_code. '&action=edit&'.tep_get_all_get_params(array('action','id','page'))) . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES, 'page=' . $_GET['page'] .'&id=' . $cInfo->zip_code. '&action=delete&'.tep_get_all_get_params(array('action','id','page'))) . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_SEARCH_ZIP_CODE1'=> tep_draw_input_field('search_zip_code', $search_zip_code, '' ),
 'INFO_TEXT_SEARCH_STATE'    => LIST_SET_DATA(ZONES_TABLE," where zone_country_id in (222,223)",'zone_name','zone_id',"zone_country_id,zone_name",'name="search_state"',"State",'',$search_state),
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ZIP_CODES, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

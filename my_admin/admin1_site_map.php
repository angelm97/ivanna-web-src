...

{
 ...
 if(tep_db_num_rows($site_map_query) > 0)
 {
  ...
  while ($site_map = tep_db_fetch_array($site_map_query))
  {
   ...
   {
    $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_SITE_MAP . '?page='.$_GET['page'].'&id=' . $cInfo->id . '&action=edit\'"';
   }
   else
   {
...
{
 case 'new':
   $heading[]  = array('text' => '<b>'.TEXT_INFO_HEADING_SITE_MAP.'</b>');
   ...
   $contents[] = array('text' => '<br>'.TEXT_INFO_SITE_MAP_PAGE.'<br>'.tep_draw_input_field('TR_page_title', ($error?$site_map_title:''), '' ));
   $contents[] = array('text' => '<br>'.TEXT_INFO_SITE_MAP_URL.'<br>'.tep_draw_input_field('TR_page_url', ($error?$page_url:''), '' ));
   $contents[] = array('text' => '<br>'.TEXT_INFO_PRIORITY.'<br>'.tep_draw_input_field('priority',($error?$priority:'1'),'size="2"'));
   $contents[] = array('align'=> 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP,'page='.$_GET['page']).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'edit':
   $heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_SITE_MAP.'</b>');
   ...
   $contents[] = array('text' => '<br>'.TEXT_INFO_SITE_MAP_PAGE.'<br>'.tep_draw_input_field('TR_page_title', ($error?$site_map_title:$cInfo->title), '' ));
   $contents[] = array('text' => '<br>'.TEXT_INFO_SITE_MAP_URL.'<br>'.tep_draw_input_field('TR_page_url', ($error?$page_url:$cInfo->page_url), '' ));
   $contents[] = array('text' => '<br>'.TEXT_INFO_PRIORITY.'<br>'.tep_draw_input_field('priority',($error?$priority:$cInfo->priority),'size="2"'));
   $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP,'page='.$_GET['page']).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
  ...
  <table>
   <tr>
   <td>       
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP, 'page='.$_GET['page'].'&id='.$cInfo->id.'&action=confirm_delete').'">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP,'page='.$_GET['page'].'&id='.$cInfo->id) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>
   </td>
   </tr>
   </table>
  ');
 ...
 default:
  if (isset($cInfo) && is_object($cInfo))
		{
   $heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_SITE_MAP.'</b>');
   $contents[] = array('text' => tep_db_output($cInfo->title));
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP,'page='.$_GET['page'].'&id='.$cInfo->id.'&action=edit').'">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP, 'page='.$_GET['page'].'&id=' . $cInfo->id. '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
...
$template->assign_vars(array(
 ...
 'new_button'               => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

...
elseif ($action == 'confirm_send')
{
 ...
 <tr>
  <td>
   <table border="0" cellspacing="0" cellpadding="2">
    <tr>
     <td valign="middle">'.tep_image(PATH_TO_IMAGE."ani_send_email.gif", IMAGE_SEND_EMAIL).'</td>
 ...
 $send_news_module.='
  ...
  <tr>
    <td><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_back.gif', IMAGE_BACK) . '</a>'. '</td>
  </tr>';
}
if(!tep_not_null($action) || $action=="delete")
{
 ...
 if(tep_db_num_rows($newsletters_query) > 0)
 {
  ...
  while ($newsletters = tep_db_fetch_array($newsletters_query))
  {
   ...
   {
    $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=preview') . '\'"' . "\n";
   }
   else
   {
   ...
   $action_image=(isset($nInfo) && is_object($nInfo) && ($newsletters['id'] == $nInfo->id) ? tep_image(PATH_TO_IMAGE.'icon_arrow_right.gif', IMAGE_PREVIEW) : '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $newsletters['id']) . '">' . tep_image(PATH_TO_IMAGE.'icon_info.gif', IMAGE_INFO) . '</a>');
   ...
   $template->assign_block_vars('news_letter', array( 'row_selected' => $row_selected,
    'action' => $action_image,
    'newsletters' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $newsletters['id'] . '&action=preview') . '">' . tep_image(PATH_TO_IMAGE.'preview.gif', IMAGE_PREVIEW) . '</a>&nbsp;' . $newsletters['title'],
...
switch ($action)
{
 case 'delete':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit(PATH_TO_BUTTON.'button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 default:
  if (is_object($nInfo))
  {
   ...
   if ($nInfo->locked > 0)
   {
    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=new') . '">' . tep_image_button(PATH_TO_BUTTON.'button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=delete') . '">' . tep_image_button(PATH_TO_BUTTON.'button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=preview') . '">' . tep_image_button(PATH_TO_BUTTON.'button_preview.gif', IMAGE_PREVIEW) . '</a> <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=send') . '">' . tep_image_button(PATH_TO_BUTTON.'button_send.gif', IMAGE_SEND) . '</a> <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=unlock') . '">' . tep_image_button(PATH_TO_BUTTON.'button_unlock.gif', IMAGE_UNLOCK) . '</a>');
   }
   else
   {
    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=preview') . '">' . tep_image_button(PATH_TO_BUTTON.'button_preview.gif', IMAGE_PREVIEW) . '</a> <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $nInfo->id . '&action=lock') . '">' . tep_image_button(PATH_TO_BUTTON.'button_lock.gif', IMAGE_LOCK) . '</a>');
   }
...
if($action=='new')
{
 ...
 $template->assign_vars(array(
  ...
  'news_module'=>tep_draw_pull_down_menu('TR_module', $modules_array, $nInfo->module),
  'news_title'=>tep_draw_input_field('TR_title', $nInfo->title, '', true ),
  'news_content'=>tep_draw_textarea_field('TR_content', 'soft', '110', '30', stripslashes($nInfo->content), '', '', true),
  'news_letter_button1'=>(($form_action == 'insert') ? tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE) : tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE)). '&nbsp;&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . (isset($_GET['nID']) ? 'nID=' . $_GET['nID'] : '')) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>',
  'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,
...
else if($action=='preview')
{
 ...
 $template->assign_vars(array(
  ...
  'preview_news_letter_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_back.gif', IMAGE_BACK) . '</a>',
  'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,
  'ADMIN_RIGHT_HTML'=>$ADMIN_RIGHT_HTML,
  'update_message'=>$messageStack->output()));
...
else
{
 $template->assign_vars(array(
  ...
  'new_news_letter_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'action=new') . '">' . tep_image_button(PATH_TO_BUTTON.'button_new_newsletter.gif', IMAGE_NEW) . '</a>',

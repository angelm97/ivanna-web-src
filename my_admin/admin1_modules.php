...
for ($i=0, $n=sizeof($directory_array); $i<$n; $i++) 
{
 ...
 if (tep_class_exists($class)) 
 {
  ...
  if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) 
  {
   if ($module->check() > 0) 
   {
    $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MODULES, 'set=' . $set . '&module=' . $class . '&action=edit') . '\'"';
   } 
   else 
   {
...
switch ($action) 
{
 case 'edit':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MODULES, 'set=' . $set . '&module=' . $_GET['module']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 default:
  $heading[] = array('text' => '<b>' . $mInfo->title . '</b>');
  if ($mInfo->status == '1') 
  {
   ...
   $keys = substr($keys, 0, strrpos($keys, '<br><br>'));
   $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=remove') . '">' . tep_image_button(PATH_TO_BUTTON.'button_module_remove.gif', IMAGE_MODULE_REMOVE) . '</a> <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MODULES, 'set=' . $set . (isset($_GET['module']) ? '&module=' . $_GET['module'] : '') . '&action=edit') . '">' . tep_image_button(PATH_TO_BUTTON.'button_edit.gif', IMAGE_EDIT) . '</a>');
   $contents[] = array('text' => '<br>' . $mInfo->description);
   $contents[] = array('text' => '<br>' . $keys);
  } 
  else 
  {
   $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=install') . '">' . tep_image_button(PATH_TO_BUTTON.'button_module_install.gif', IMAGE_MODULE_INSTALL) . '</a>');
   $contents[] = array('text' => '<br>' . $mInfo->description);
  }
 break;
}

...
if ($action!="")
{
 switch ($action)
  ...
  case 'save':
   ...
   $es_language_name=tep_db_prepare_input($_POST['TR_es_language_name']);
   $priority = tep_db_prepare_input($_POST['IN_priority']);
   $sql_data_array['name'] = $language_name;
			$sql_data_array['es_name'] = $es_language_name;
   $sql_data_array['priority'] = $priority;
			...
			if($action=='insert')
			{
				...
				else if($row_chek=getAnyTableWhereData(JOBSEEKER_LANGUAGE_TABLE," es_name='".tep_db_input($es_language_name)."'",'languages_id'))
				{
    ...
    $languages_id=(int)$_GET['languages_id'];
				...
				else if($row_chek=getAnyTableWhereData(JOBSEEKER_LANGUAGE_TABLE," es_name='".tep_db_input($es_language_name)."' and languages_id!='$languages_id'",'languages_id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values
$language_level_query_raw="select languages_id, name,es_name,priority from " . JOBSEEKER_LANGUAGE_TABLE ." order by priority";
...
if(tep_db_num_rows($language_level_query) > 0)
{
 ...
 while ($language_level = tep_db_fetch_array($language_level_query))
 {
  ...
  {
   $row_selected='languages_id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE . '?page='.$_GET['page'].'&languages_id=' . $cInfo->languages_id . '&action=edit\'"';
  }
  else
  {
  ...
  $template->assign_block_vars('language_level', array( 'row_selected' => $row_selected,
   ...
   'es_name' => tep_db_output($language_level['es_name']),
...
switch ($action)
{
 ...
 case 'insert':
 case 'save':
		$heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_LANGUAGE.'</b>');
  $contents = array('form' => tep_draw_form('language_level', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_LANGUAGE_NAME.'<br>'.tep_draw_input_field('TR_language_name', $_POST['TR_language_name'], '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_FR_LANGUAGE_NAME.'<br>'.tep_draw_input_field('TR_es_language_name', $_POST['TR_es_language_name'], '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_LANGUAGE_PRIORITY.'<br>'.tep_draw_input_field('IN_priority', $_POST['IN_priority'], '' ));
		$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'edit':
  $value_field=tep_draw_input_field('TR_language_name', $cInfo->name, '' );
  $heading[] = array('text' => '<b>'.TEXT_INFO_LANGUAGE_NAME.'</b>');
  $contents = array('form' => tep_draw_form('language_level', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'languages_id=' . $cInfo->languages_id.'&page='.$_GET['page'].'&action=save','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_LANGUAGE_NAME.'<br>'.tep_draw_input_field('TR_language_name', $cInfo->name, '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_FR_LANGUAGE_NAME.'<br>'.tep_draw_input_field('TR_es_language_name', $cInfo->es_name, '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_LANGUAGE_PRIORITY.'<br>'.tep_draw_input_field('IN_priority', $cInfo->priority, '' ));
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'gid=' . $_GET['gid'] . '&languages_id=' . $cInfo->languages_id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
  ...
  <table>
   <tr>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'page=' . $_GET['page'] . '&languages_id=' . $_GET['languages_id'].'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'page=' . $_GET['page'] . '&languages_id=' . $_GET['languages_id']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>
   </td>
   </tr>
   </table>
  ');
 ...
 default:
  if (isset($cInfo) && is_object($cInfo))
		{
   $heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_LANGUAGE.'</b>');
   $contents[] = array('text' => tep_db_output($cInfo->name));
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'page=' . $_GET['page'] .'&languages_id=' . $cInfo->languages_id . '&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'page=' . $_GET['page'] .'&languages_id=' . $cInfo->languages_id . '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

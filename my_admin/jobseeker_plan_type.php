...
if ($action!="")
{
 switch ($action)
  ...
  case 'save':
   ...
   $sql_data_array['es_plan_type_name'] = $es_plan_type_name;
   ...
   if($action=='insert')
	{
	 if($row_chek=getAnyTableWhereData(JOBSEEKER_PLAN_TYPE_TABLE,"plan_type_name='".tep_db_input($plan_type_name)."'",'id'))
	  ...
	  else if($row_chek=getAnyTableWhereData(JOBSEEKER_PLAN_TYPE_TABLE,"es_plan_type_name='".tep_db_input($es_plan_type_name)."'",'id'))
	  {
	...
	else
	 {
	  ...
	  if($row_chek=getAnyTableWhereData(JOBSEEKER_PLAN_TYPE_TABLE,"es_plan_type_name='".tep_db_input($es_plan_type_name)."' and id!='$id'",'id'))
	   {
		$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
		$action='edit';
	   }
...
if(tep_db_num_rows($plan_type_query) > 0)
{
 ...
 while ($plan_type = tep_db_fetch_array($plan_type_query))
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_ADMIN_JOBSEEKER_PLAN_TYPE . '?page='.$_GET['page'].'&id=' . $cInfo->id . '&action=edit\'"';
  }
  else
  {
...
switch ($action)
{
 ...
 case 'save':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_PLAN_TYPE).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
 break;
 case 'edit':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_PLAN_TYPE, 'gid=' . $_GET['gid'] . '&id=' . $cInfo->id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
 break;
 case 'delete':
  ...
  <table>
   <tr>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_PLAN_TYPE, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete') . '">'
   .tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_PLAN_TYPE, 'page=' . $_GET['page'] . '&id=' . $_GET['id']) . '">'
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_PLAN_TYPE, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

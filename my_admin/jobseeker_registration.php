  ...
  case 'back':
   ...
   $sql_data_array['description'] = $description;
    ...
    $messageStack->add(MESSAGE_FR_DESCRIPTION_ERROR, 'error');
     ...
     fwrite($handle, stripslashes($row_template['description']));
     fclose($handle);
     $handle1 = fopen(PATH_TO_MAIN_PHYSICAL.PATH_TO_TEMPLATE.'es_jobseeker_registration_template.htm', "w");
...
if(tep_db_num_rows($jobseeker_registration_query) > 0)
{
 ...
 while ($jobseeker_registration = tep_db_fetch_array($jobseeker_registration_query))
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_ADMIN_JOBSEEKER_REGISTRATION . '?page='.$_GET['page'].'&id=' . $cInfo->id . '&action=edit\'"';
  }
  else
  {
...
switch ($action)
{
 case 'delete':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_REGISTRATION, 'page=' . $_GET['page'] . '&jobseeker_registration_id=' . $jobseeker_registration_id.'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_REGISTRATION, 'page=' . $_GET['page'] . '&jobseeker_registration_id=' . $jobseeker_registration_id) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 default:
  if (isset($cInfo) && is_object($cInfo))
   ...
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_REGISTRATION, 'page=' . $_GET['page'] .'&jobseeker_registration_id=' . $cInfo->id . '&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_REGISTRATION, 'page=' . $_GET['page'] .'&jobseeker_registration_id=' . $cInfo->id . '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_JOBSEEKER_REGISTRATION_NAME1'=>tep_draw_input_field('TR_jobseeker_registration_name', $jobseeker_registration_name, 'size="35"', true ).'&nbsp;'.tep_draw_checkbox_field('default', 'Yes', false, $default, 'id="check_box_default"') . ' <label for="check_box_default">' . TEXT_SET_DEFAULT.'</label>',
 'INFO_TEXT_JOBSEEKER_REGISTRATION_PRIORITY'=>INFO_TEXT_JOBSEEKER_REGISTRATION_PRIORITY,
 'INFO_TEXT_JOBSEEKER_REGISTRATION_PRIORITY1'=>tep_draw_input_field('IN_priority', $priority, 'size="5"', true ),
 'INFO_TEXT_JOBSEEKER_REGISTRATION_DESCRIPTION'=>INFO_TEXT_JOBSEEKER_REGISTRATION_DESCRIPTION,
 'INFO_TEXT_JOBSEEKER_REGISTRATION_DESCRIPTION1'=>tep_draw_textarea_field('description2', 'soft', '80%', '10', $description, 'style="height: 300px; width: 800px;"  id="description2"', true, true),
 'INFO_TEXT_FR_JOBSEEKER_REGISTRATION_DESCRIPTION'=>INFO_TEXT_FR_JOBSEEKER_REGISTRATION_DESCRIPTION,
 'INFO_TEXT_FR_JOBSEEKER_REGISTRATION_DESCRIPTION1'=>tep_draw_textarea_field('es_description2', 'soft', '80%', '10', $es_description, 'style="height: 300px; width: 800px;"  id="es_description2"', true, true),
 'HOST_NAME'                       =>HOST_NAME,
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_REGISTRATION, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',
 ...
 else if($action=='preview' || $action=='save' || $action=='insert')
 {
  $template->assign_vars(array(
   ...
   'INFO_TEXT_FR_JOBSEEKER_REGISTRATION_DESCRIPTION'=>INFO_TEXT_FR_JOBSEEKER_REGISTRATION_DESCRIPTION,
   'INFO_TEXT_FR_JOBSEEKER_REGISTRATION_DESCRIPTION1'=>stripslashes($es_description),

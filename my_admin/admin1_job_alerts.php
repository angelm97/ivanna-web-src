...
if(tep_db_num_rows($job_alert_query) > 0)
{
 ...
 while ($job_alert = tep_db_fetch_array($job_alert_query))
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_JOBALERTS . '?page='.$_GET['page'].'&id=' . $jaInfo->id . '&action=edit\'"';
  }
  else
  {
...
switch ($action)
{
 ...
 case 'save':
  ...
  $contents = array('form' => tep_draw_form('job_alert', PATH_TO_ADMIN.FILENAME_ADMIN1_JOBALERTS, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBALERTS).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
  $heading[] = array('text' => '<b>' . $jaInfo->email_id . '</b>');
  ...
  $contents[] = array('text' => '<br><b>' . $jaInfo->email_id . '</b>');
  $contents[] = array('align' => 'center', 'text' => '
  <table>
  <tr>
  <td>      
  <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBALERTS, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete') . '">'
  .tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>
  </td>
  <td>      
  <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBALERTS, 'page=' . $_GET['page'] . '&id=' . $_GET['id']) . '">' 
 ...
 default:
  if (isset($jaInfo) && is_object($jaInfo))
   ...
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBALERTS, 'page=' . $_GET['page'] .'&id=' . $jaInfo->id . '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');

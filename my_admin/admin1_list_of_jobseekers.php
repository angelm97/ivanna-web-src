...
if(tep_not_null($search_status1))
{
 ...
 {
  $db_jobseeker_query_raw = "select j.jobseeker_id, concat(j.jobseeker_first_name,' ',j.jobseeker_last_name) as jobseeker_name, j.jobseeker_featured, jl.updated, jl.jobseeker_email_address, jl.jobseeker_status, jl.ip_address, jl.last_login_time, jl.number_of_logon  from " . JOBSEEKER_LOGIN_TABLE . " as jl, " . JOBSEEKER_TABLE . " as j where jl.jobseeker_id=j.jobseeker_id and jl.jobseeker_email_address!='demo@aynsoft.com' and jobseeker_status='Yes' and j.jobseeker_id NOT IN(select jobseeker_id from ".JOBSEEKER_ORDER_TABLE.") $whereClause order by ".$order_by_clause;
 }
 elseif($search_status1=='inactive')
 {
  $db_jobseeker_query_raw = "select j.jobseeker_id, concat(j.jobseeker_first_name,' ',j.jobseeker_last_name) as jobseeker_name, j.jobseeker_featured, jl.updated, jl.jobseeker_email_address, jl.jobseeker_status, jl.ip_address, jl.last_login_time, jl.number_of_logon  from " . JOBSEEKER_LOGIN_TABLE . " as jl, " . JOBSEEKER_TABLE . " as j where jl.jobseeker_id=j.jobseeker_id and jl.jobseeker_email_address!='demo@aynsoft.com' and jobseeker_status='No' and j.jobseeker_id NOT IN(select jobseeker_id from ".JOBSEEKER_ORDER_TABLE.") $whereClause order by ".$order_by_clause;
 }
}
else
$db_jobseeker_query_raw = "select j.jobseeker_id, concat(j.jobseeker_first_name,' ',j.jobseeker_last_name) as jobseeker_name, j.jobseeker_featured, jl.updated, jl.jobseeker_email_address, jl.jobseeker_status, jl.ip_address, jl.last_login_time, jl.number_of_logon  from " . JOBSEEKER_LOGIN_TABLE . " as jl, " . JOBSEEKER_TABLE . " as j where jl.jobseeker_id=j.jobseeker_id and jl.jobseeker_email_address!='demo@aynsoft.com' and j.jobseeker_id NOT IN(select jobseeker_id from ".JOBSEEKER_ORDER_TABLE.") $whereClause order by ".$order_by_clause;
...
{
 case 'delete':
  $heading[] = array('text' => '<b>' . $rInfo->jobseeker_name . '</b>');
  $contents[] = array('text' => TEXT_DELETE_INTRO);
  $contents[] = array('text' => '<br><b>' . $rInfo->jobseeker_name . '</b>');
  $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS, tep_get_all_get_params(array('page','action','selected_box'))).'&action=confirm_delete' . '">'.tep_button('Confirm','class="btn btn-delete"').'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS, tep_get_all_get_params(array('action','selected_box'))) . '">'.tep_button('Cancel','class="btn btn-cancel"').'</a>');
 ...
 if (isset($rInfo) && is_object($rInfo))
 {
  ...
  <a href="' . tep_href_link(FILENAME_JOBSEEKER_REGISTER1, 'jID=' . $rInfo->jobseeker_id ) . '">'.tep_button('Edit','class="btn btn-secondary"').'</a>&nbsp;
  <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS, tep_get_all_get_params(array('jID','selected_box'))).'&jID=' . $rInfo->jobseeker_id.'&action=delete' . '">'
  .tep_button('Delete','class="btn btn-delete"').'</a>');
...
if($action1=='delete')
{
 $check_link='<br>'.TEXT_DELETE_WARNING.'<br><br><a href="#"  style="color:#ffffff" onclick="DeleteSelected(\'confirm_bulk_delete\')" class="btn btn-primary">Confirm</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS).'">'.tep_button('Cancel','class="btn btn-primary"').'</a>';
// $check_link='<br>'.TEXT_DELETE_WARNING.'<br><br><a href="#"  style="color:#0000ff" onclick="DeleteSelected(\'confirm_bulk_delete\')">'.tep_button('Confirm','class="btn btn-primary"').'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS).'">'.tep_button('Cancel','class="btn btn-primary"').'</a>';
...
else
{
 ...
 'new_button'=>'<a href="' . tep_href_link(FILENAME_JOBSEEKER_REGISTER1, 'add=jobseeker') . '">' . tep_image_button(PATH_TO_BUTTON.'button_new.gif', IMAGE_NEW) . '</a>',
  ));
}

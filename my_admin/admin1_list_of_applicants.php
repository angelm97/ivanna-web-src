...
///only for sorting ends
 ...
 $table_names=APPLICATION_TABLE . " as a left outer join  ".JOBSEEKER_TABLE." as j on(a.jobseeker_id=j.jobseeker_id) left outer join ".JOBSEEKER_RESUME1_TABLE." as jr1 on (j.jobseeker_id=jr1.jobseeker_id) left outer join ".JOBSEEKER_LOGIN_TABLE." as jl on (a.jobseeker_id=jl.jobseeker_id) left outer join ".JOB_TABLE. " as jb on (a.job_id=jb.job_id ) left outer join ".RECRUITER_TABLE." as r on (r.recruiter_id=jb.recruiter_id)";
...
$query_string3=encode_string("application*=*".$aInfo->application_id."*=*application_id");
...
switch ($action)
{
 ...
 {
  $heading[] = array('text' => '<b>'.$aInfo->jobseeker_name.'</b>');
  $contents[] = array('text' => TEXT_INFO_EDIT_ACCOUNT_INTRO);
  $contents[]= $contentvar;
if ($aInfo->applicant_select=='Yes')
{
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"','text' => '<i class="fa fa-angle-right" aria-hidden="true"></i> <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS, 'jobID=' . $job_id . '&action1=change_status'.'&aID=' . $aInfo->id.'&rID=' . $aInfo->recruiter_id).'" class="right_black">Edit Status</a>');
}
else
{
 $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"','text' => '<i class="fa fa-angle-right" aria-hidden="true"></i> <a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,'query_string3='.$query_string3.'&job_id='.$job_id.'&rID=' . $aInfo->recruiter_id.'&action1=change_status').'" class="right_black">Edit Status</a>');
}

  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow1" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"','text' => '<i class="fa fa-angle-right" aria-hidden="true"></i> <a href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.encode_string("application_id=".$aInfo->resume_id."=application_id")).'" class="right_black">View Resume</a>');
 }
 break;
}
////
...
if($action1=='delete')
{
 $check_link='<br>'.TEXT_DELETE_WARNING.'<br><br><a href="#"  style="color:#0000ff" onclick="DeleteSelected(\'confirm_bulk_delete\')">'.tep_button('Confirm','class="btn btn-primary"').'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_APPLICANTS_LIST).'">'.tep_button('Cancel','class="btn btn-primary"').'</a>';
...
else
{
 ...
 $template->assign_vars(array('TABLE_HEADING_APPLICANT_NAME'=>"<a href='#' class='white' onclick=\"submit_thispage('".$obj_sort_by_clause->return_sort_array['name'][0]."','".$lower."');\"><u>".TABLE_HEADING_APPLICANT_NAME.'</u>'.$obj_sort_by_clause->return_sort_array['image'][0]."</a>",
                              'TABLE_HEADING_APPLICANT_EMAIL' =>"<a href='#' class='white' onclick=\"submit_thispage('".$obj_sort_by_clause->return_sort_array['name'][1]."','".$lower."');\"><u>".TABLE_HEADING_APPLICANT_EMAIL.'</u>'.$obj_sort_by_clause->return_sort_array['image'][1]."</a>",
                              'TABLE_HEADING_APPLICANT_COMP_APPLIED'=>"<a href='#' class='white' onclick=\"submit_thispage('".$obj_sort_by_clause->return_sort_array['name'][2]."','".$lower."');\"><u>".TABLE_HEADING_APPLICANT_COMP_APPLIED.'</u>'.$obj_sort_by_clause->return_sort_array['image'][2]."</a>",
                              'TABLE_HEADING_APPLICANT_JOB_APPLIED'=>"<a href='#' class='white' onclick=\"submit_thispage('".$obj_sort_by_clause->return_sort_array['name'][3]."','".$lower."');\"><u>".TABLE_HEADING_APPLICANT_JOB_APPLIED.'</u>'.$obj_sort_by_clause->return_sort_array['image'][3]."</a>",
                              ...
                              'TABLE_HEADING_APPLICANT_SELECT'=>"<a href='#' class='white' onclick=\"submit_thispage('".$obj_sort_by_clause->return_sort_array['name'][5]."','".$lower."');\"><u>".TABLE_HEADING_APPLICANT_SELECT.'</u>'.$obj_sort_by_clause->return_sort_array['image'][5]."</a>"
  ));
}

...
if(tep_db_num_rows($ipaddress_query) > 0)
{
 ...
 while ($ipaddress = tep_db_fetch_array($ipaddress_query)) 
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_IPADDRESS_BLOCK . '?page='.$_GET['page'].'&id=' . $iInfo->id . '&action=edit\'"';
  } 
  else 
  {
...
switch ($action) 
{
 ...
 case 'save':
  ...
  $contents = array('form' => tep_draw_form('ipaddress', PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_IPADDRESS_FROM.'<br>'.tep_draw_input_field('TR_ipaddress_from', $_POST['TR_ipaddress_from'], '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_IPADDRESS_TO.'<br>'.tep_draw_input_field('ipaddress_to', $_POST['ipaddress_to'], '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_IPADDRESS_PRIORITY.'<br>'.tep_draw_input_field('IN_priority', $_POST['IN_priority'], 'size="5"' ));
		$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'edit':
  ...
  $contents = array('form' => tep_draw_form('ipaddress', PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'id=' . $iInfo->id.'&page='.$_GET['page'].'&action=save','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_IPADDRESS_FROM.'<br>'.tep_draw_input_field('TR_ipaddress_from', $iInfo->ip_address1, '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_IPADDRESS_TO.'<br>'.tep_draw_input_field('ipaddress_to', $iInfo->ip_address2, '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_IPADDRESS_PRIORITY.'<br>'.tep_draw_input_field('IN_priority', $iInfo->priority, '' ));
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'gid=' . $_GET['gid'] . '&id=' . $iInfo->id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
  $heading[] = array('text' => '<b>' . $iInfo->ipaddress_name . '</b>');
  ...
  $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'page=' . $_GET['page'] . '&id=' . $_GET['id']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 default:
  if (isset($iInfo) && is_object($iInfo)) 
   ...
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'page=' . $_GET['page'] .'&id=' . $iInfo->id . '&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'page=' . $_GET['page'] .'&id=' . $iInfo->id . '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IPADDRESS_BLOCK, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

...
if ( (($action == 'new_file') && ($directory_writeable == true)) || ($action == 'edit') ) 
{
 ...
 if ($file_writeable == true) 
  $new_button=tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>';
} 
else 
{
 ...
 $new_button='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, 'action=reset') . '">' . tep_image_button(PATH_TO_BUTTON.'button_reset.gif', IMAGE_RESET) . '</a>&nbsp;';
 $new_button.='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&' : '') . 'action=upload') . '">' . tep_image_button(PATH_TO_BUTTON.'button_upload.gif', IMAGE_UPLOAD) . '</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&' : '') . 'action=new_file') . '">' . tep_image_button(PATH_TO_BUTTON.'button_new_file.gif', IMAGE_NEW_FILE) . '</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&' : '') . 'action=new_folder') . '">' . tep_image_button(PATH_TO_BUTTON.'button_new_folder.gif', IMAGE_NEW_FOLDER) . '</a>';;
}
//////////
//// right starts
$ADMIN_RIGHT_HTML="";
...
switch ($action) 
{
 case 'delete':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit(PATH_TO_BUTTON.'button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, (tep_not_null($fInfo->name) ? 'info=' . urlencode($fInfo->name) : '')) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 case 'new_folder':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . (($directory_writeable == true) ? tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE) : '') . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 case 'upload':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . (($directory_writeable == true) ? tep_image_submit(PATH_TO_BUTTON.'button_upload.gif', IMAGE_UPLOAD) : '') . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 default:
  if (isset($fInfo) && is_object($fInfo)) 
  {
   $heading[] = array('text' => '<b>' . $fInfo->name . '</b>');
   if (!$fInfo->is_dir) 
    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILE_MANAGER, 'info=' . urlencode($fInfo->name) . '&action=edit') . '">' . tep_image_button(PATH_TO_BUTTON.'button_edit.gif', IMAGE_EDIT) . '</a>');
...
if ( (($action == 'new_file') && ($directory_writeable == true)) || ($action == 'edit') ) 
{
 $template->assign_vars(array(
  ...
  'TEXT_FILE_CONTENTS1'=>tep_draw_textarea_field('file_contents', 'soft', '90', '20', $file_contents, (($file_writeable) ? '' : 'readonly')),

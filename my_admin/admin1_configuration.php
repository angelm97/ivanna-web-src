...
if(tep_db_num_rows($configuration_query) > 0)
{
 ...
 while ($configuration = tep_db_fetch_array($configuration_query))
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_CONFIGURATION . '?gid=' . $gid . '&id=' . $cInfo->id . '&action=edit\'"';
  }
  else
  {
...
switch ($action)
{
 ...
 case 'save':
		...
		$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_TITLE . '<br>'.tep_draw_input_field('TR_configuration_title',$configuration_title));
	//	$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_NAME . '<br>'.tep_draw_input_field('TR_configuration_name',$configuration_name,));
		$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_VALUE . '<br>'.tep_draw_input_field('TR_configuration_value',$configuration_value));
	//	$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_DESCRIPTION . '<br>'.tep_draw_input_field('configuration_description',$configuration_description));
		$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_DESCRIPTION . '<br>'.$configuration_description);
		$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_PRIORITY . '<br>'.tep_draw_input_field('IN_configuration_priority',$configuration_priority,'size="5"'));
		$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT ).'&nbsp;<a href="' . FILENAME_ADMIN1_CONFIGURATION. '?gid='.$gid.'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'edit':
  ...
  $contents = array('form' => tep_draw_form('configuration',PATH_TO_ADMIN.FILENAME_ADMIN1_CONFIGURATION, 'gid=' . $gid . '&id=' . $cInfo->id . '&action=save', 'post','onsubmit="return ValidateForm(this)" enctype="multipart/form-data"'));
		...
		$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_PRIORITY . '<br>'.tep_draw_input_field('IN_configuration_priority',$cInfo->priority,'size="5"'));
		 $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_CONFIGURATION, 'gid=' . $gid . '&id=' . $cInfo->id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
  break;
 default:
  if (isset($cInfo) && is_object($cInfo))
		{
   $heading[] = array('text' => '<b>' . tep_db_output($cInfo->configuration_title) . '</b>');
   $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_CONFIGURATION, 'gid=' . $gid . '&id=' . $cInfo->id . '&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif', IMAGE_EDIT).'</a>');
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_CONFIGURATION, 'gid=' . $gid . '&action=new') .'">'. tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

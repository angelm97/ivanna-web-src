...
include_once(PATH_TO_MAIN_ADMIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_ADMIN1_MOBILE_THEME_EDITOR);
$template->set_filenames(array('themes' =>'admin1_mobile_theme_editor.htm','themes1' =>'admin1_mobile_theme_editor1.htm'));
...
if($action!="") 
{
 switch ($action) 
  ...
  case 'create_folder':
    ...
    tep_redirect(tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_MOBILE_THEME_EDITOR,'curr_dir=' . urlencode($curr_dir).'&theme_name=' . urlencode($theme_name)));       
  break;
    case 'upload':
    if(tep_not_null($curr_dir) &&  is_dir(PATH_TO_MAIN_PHYSICAL_THEMES.$theme_name.'/'.$curr_dir))
    {
...
{
 if($theme['feature']=='mobile-theme')
 $new_theme_array[$key]= array('name'=>$theme['name'],'description'=>$theme['description'],'version'=>$theme['version'],'screenshot'=>$theme['screenshot']);
}
$themes=$new_theme_array;
$theme_list=array();
...
if($action =='edit' )
{
 ...
 if(file_exists($curr_directory.'/'.$file_name))
 {
  ...
  switch($file_ext)
  {
   ...
   {
    $add_script='<script language="javascript" type="text/javascript" src="../tinymce/tinymce.js"></script>
  ...
  image_advtab: true,
   ...
   $file_content=$add_script.' '.tep_draw_textarea_field('file_contents', 'soft', '120', '25', $file_contents, (($file_writeable) ? '' : 'readonly'));
   if ($file_writeable == true) 
   $file_content.=' '.tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE);
   ...
   if($default_theme_directory==$curr_directory)
   {
    switch($contents[$i]['name'])
    {
     ...
     case 'info.txt':
      ...
      break;
     case 'page_header.php':
      $name='Intenal Page Header';
      break;
     case 'page_footer.php':
      $name='Intenal Page Footer';
      break;
...
$template->assign_vars(array(
 ...
 'theme_form'                   => tep_draw_form('thems', PATH_TO_ADMIN.FILENAME_ADMIN1_MOBILE_THEME_EDITOR,'','post',' onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('theme_name',$theme_name).tep_draw_hidden_field('action','').tep_draw_hidden_field('file','').$hidden_fields,
 'upload_form'                  => tep_draw_form('upload', PATH_TO_ADMIN.FILENAME_ADMIN1_MOBILE_THEME_EDITOR,'','post',' onsubmit="return ValidateForm(this)" enctype="multipart/form-data"').tep_draw_hidden_field('theme_name',$theme_name).tep_draw_hidden_field('action','upload').$hidden_fields,
 'folder_form'                  => tep_draw_form('upload', PATH_TO_ADMIN.FILENAME_ADMIN1_MOBILE_THEME_EDITOR,'','post',' onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('theme_name',$theme_name).tep_draw_hidden_field('action','create_folder').$hidden_fields,
 ...
 'INFO_TEXT_FOLDER1'            => tep_draw_input_field('TR_folder_name'),
 
 'UPLOAD_BUTTON'                => tep_image_submit(PATH_TO_BUTTON.'button_upload.gif', IMAGE_UPLOAD).' <a href="#" onclick="show_content(\'\');">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>',
 'FOLDER_SAVE_BUTTON'           => tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE).' <a href="#" onclick="show_content(\'\');">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>',

 'theme_editor_form'            => tep_draw_form('theme_editor_form', PATH_TO_ADMIN.FILENAME_ADMIN1_MOBILE_THEME_EDITOR,'','post',' onsubmit="return ValidateForm(this)"'),
 'INFO_TEXT_NEW_BUTTON'         => '<a href="#upload_file" onclick="show_content(\'upload_file\')">'.tep_image_button(PATH_TO_BUTTON.'button_upload.gif', IMAGE_UPLOAD) . '</a>&nbsp;<a href="#add_folder" onclick="show_content(\'add_folder\')">'. tep_image_button(PATH_TO_BUTTON.'button_new_folder.gif', IMAGE_NEW_FOLDER) . '</a>',
 ));
...
if($action=='edit')
{
 $template->assign_vars(array(
  ...
  'INFO_TEXT_THEME_NAME'         => tep_db_output($theme_detail['name']),
  'INFO_TEXT_THEME_SCREENSHOT'   => tep_not_null(!$theme_detail['screenshot'])?'':tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_THEMES."/".$theme_name.'/'.$theme_detail['screenshot']."&size=200"),
  'edit_form'                    => tep_draw_form('edit', PATH_TO_ADMIN.FILENAME_ADMIN1_MOBILE_THEME_EDITOR,'','post',' onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('theme_name',$theme_name).tep_draw_hidden_field('action','save').tep_draw_hidden_field('file',$file_name).$hidden_fields,

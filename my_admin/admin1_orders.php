...
if (($action == 'edit') && ($order_exists == true))
{
 ...
 for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++)
 {
  $order_total_text.='
	...
	$template->assign_vars(array(
		'HEADING_TITLE'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ORDERS, tep_get_all_get_params(array('action','selected_box'))) . '">' . tep_image_button(PATH_TO_BUTTON.'button_back.gif', IMAGE_BACK) . '</a>',
...
else
{
	...
	$order_text=' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.HEADING_TITLE_STATUS . ' ' . tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_ALL_ORDERS)), $orders_statuses), $_GET['status'], 'class="form-control" onChange="this.form.submit();"');
	if (isset($_GET['cID']))
	{
...
switch ($action)
{
	case 'delete':
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit(PATH_TO_BUTTON.'button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ORDERS, tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $oInfo->orders_id) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
	break;
	default:
		if (isset($oInfo) && is_object($oInfo))
		{
			$heading[] = array('text' => '<b>[' . $oInfo->orders_id . ']&nbsp;&nbsp;' . tep_date_short($oInfo->date_purchased) . '</b>');
			$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ORDERS, tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $oInfo->orders_id . '&action=edit') . '">' . tep_image_button(PATH_TO_BUTTON.'button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ORDERS, tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $oInfo->orders_id . '&action=delete') . '">' . tep_image_button(PATH_TO_BUTTON.'button_delete.gif', IMAGE_DELETE) . '</a>');

  ...
  case 'back':
   ...
   $sql_data_array['es_description'] = $es_description;
    ...
    $messageStack->add(MESSAGE_FR_DESCRIPTION_ERROR, 'error');
     ...
     fclose($handle);
     $handle1 = fopen(PATH_TO_MAIN_PHYSICAL.PATH_TO_TEMPLATE.'es_recruiter_order_template_template.htm', "w");
...
if(tep_db_num_rows($recruiter_order_template_query) > 0)
{
 ...
 while ($recruiter_order_template = tep_db_fetch_array($recruiter_order_template_query))
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_ADMIN_RECRUITER_ORDER_TEMPLATE . '?page='.$_GET['page'].'&id=' . $cInfo->id . '&action=edit\'"';
  }
  else
  {
...
switch ($action)
{
 case 'delete':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_RECRUITER_ORDER_TEMPLATE, 'page=' . $_GET['page'] . '&order_template_id=' . $order_template_id.'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_RECRUITER_ORDER_TEMPLATE, 'page=' . $_GET['page'] . '&order_template_id=' . $order_template_id) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 default:
  if (isset($cInfo) && is_object($cInfo))
   ...
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_RECRUITER_ORDER_TEMPLATE, 'page=' . $_GET['page'] .'&order_template_id=' . $cInfo->id . '&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_RECRUITER_ORDER_TEMPLATE, 'page=' . $_GET['page'] .'&order_template_id=' . $cInfo->id . '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_ORDER_TEMPLATE_NAME1'=>tep_draw_input_field('TR_recruiter_order_template_name', $order_template_name, 'size="35"', true ).'&nbsp;'.tep_draw_checkbox_field('default', 'Yes', false, $default, 'id="check_box_default"') . ' <label for="check_box_default">' . TEXT_SET_DEFAULT.'</label>',
 ...
 'INFO_TEXT_RECRUITER_ORDER_TEMPLATE_DESCRIPTION1'=>tep_draw_textarea_field('description2', 'soft', '80%', '10', $description, 'style="height: 400px; width: 800px;"  id="description2"', true, true),
 'INFO_TEXT_FR_RECRUITER_ORDER_TEMPLATE_DESCRIPTION'=>INFO_TEXT_FR_RECRUITER_ORDER_TEMPLATE_DESCRIPTION,
 'INFO_TEXT_FR_RECRUITER_ORDER_TEMPLATE_DESCRIPTION1'=>tep_draw_textarea_field('es_description2', 'soft', '80%', '10', $es_description, 'style="height: 400px; width: 800px;"  id="es_description2"', true, true),
 'HOST_NAME'                       =>HOST_NAME,
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_RECRUITER_ORDER_TEMPLATE, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',
 ...
 else if($action=='preview' || $action=='save' || $action=='insert')
 {
  $template->assign_vars(array(
   ...
   'INFO_TEXT_FR_RECRUITER_ORDER_TEMPLATE_DESCRIPTION'=>INFO_TEXT_FR_RECRUITER_ORDER_TEMPLATE_DESCRIPTION,
   'INFO_TEXT_FR_RECRUITER_ORDER_TEMPLATE_DESCRIPTION1'=>stripslashes($es_description),

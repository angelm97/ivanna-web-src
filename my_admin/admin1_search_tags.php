...
if(tep_db_num_rows($search_tag_query) > 0)
{
 ...
 while ($search_tag = tep_db_fetch_array($search_tag_query)) 
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_ADMIN_SEARCH_TAGS . '?page='.$_GET['page'].'&id=' . $cInfo->id . '&action=edit\'"';
  } 
  else 
  {
...
switch ($action) 
{
 ...
 case 'insert':
 case 'save':
		$heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_SEARCH_TAG.'</b>');
  $contents = array('form' => tep_draw_form('search_tag', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_SEARCH_TAG_NAME.'<br>'.tep_draw_input_field('TR_search_tag_key', $_POST['TR_search_tag_key'], '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_SEARCH_TAG_POINT.'<br>'.tep_draw_input_field('IN_point', $_POST['IN_point'], '' ));
  $contents[] = array('text' => '<br>'.TEXT_INFO_STATUS.'<br>'.tep_draw_radio_field("status", 'active',true,$status).'&nbsp; Active &nbsp;'.tep_draw_radio_field("status", 'inactive',false,$status).'&nbsp;Inactive');
		$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'edit':
  ...
  $contents = array('form' => tep_draw_form('search_tag', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'id=' . $cInfo->id.'&page='.$_GET['page'].'&action=save','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_SEARCH_TAG_NAME.'<br>'.tep_draw_input_field('TR_search_tag_key', $cInfo->tag, '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_SEARCH_TAG_POINT.'<br>'.tep_draw_input_field('IN_point', $cInfo->point, '' ));
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'gid=' . $_GET['gid'] . '&id=' . $cInfo->id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'page=' . $_GET['page'] . '&id=' . $_GET['id']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 default:
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'page=' . $_GET['page'] .'&action=reset_point') . '">'.tep_image_button(PATH_TO_BUTTON.'button_reset_point.gif',IMAGE_RESET_POINT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

...
///only for sorting ends
 ...
 if(tep_db_num_rows($article_query) > 0)
 {
  ...
  while ($article = tep_db_fetch_array($article_query))
  {
   ...
   {
    $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_LIST_OF_ARTICLES . '?page='.$_GET['page'].'&id=' . $aInfo->id . '&action=edit\'"';
   }
   else
   {
...
switch ($action)
{
 case 'delete':
  ...
  <table>
   <tr>
   <td>       
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'page=' . $_GET['page'] . '&id=' . $aInfo->id.'&action=confirm_delete') . '">'
   .tep_button('Confirm','class="btn btn-primary"').'</a>
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'page=' . $_GET['page'] . '&id=' . $aInfo->id) . '">' 
 ...
 case 'edit_metatags':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_draw_submit_button_field('','Save','class="btn btn-primary"').'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'page=' . $_GET['page'] . '&id=' . $aInfo->id) . '">' . tep_button('Cancel','class="btn btn-primary"') . '</a>');
  break;
 default:
  if (isset($aInfo) && is_object($aInfo))
   ...
   <tr>
   <td>       
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'id=' . $aInfo->id . '&action=edit') . '">'
   .tep_button('Edit','class="btn btn-primary"').'</a>
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'page=' . $_GET['page'] .'&id=' . $aInfo->id . '&action=delete') . '">'
   .tep_button('Delete','class="btn btn-primary"').'</a>
   </td>
   </tr>
   <tr>
   <td colspan="2">
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'id=' . $aInfo->id . '&action=edit_metatags').'">'
  ...
  //  $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'id=' . $aInfo->id . '&action=edit_metatags').'">'.tep_button('Title/Metatags','class="btn btn-primary"').'</a>');
  }
  break;
}
...
elseif($action=='new' || $action=='edit' || $action=='back')
{
 $template->assign_vars(array(
 'INFO_TEXT_TITLE'=>INFO_TEXT_TITLE,
 'INFO_TEXT_TITLE1'=>tep_draw_input_field('TR_title',$title,'size="45"','class="form-control"',true),
 'INFO_TEXT_SEO_NAME'     => (!$edit)?'':INFO_TEXT_SEO_NAME,
 'INFO_TEXT_SEO_NAME1'    => (!$edit)?'':tep_draw_input_field('TR_seo_name',$seo_name,'size="45"',true),
 'INFO_TEXT_SEO_NAME_DES' => (!$edit)?'':INFO_TEXT_SEO_NAME_DES,
 'INFO_TEXT_CATEGORY'=>INFO_TEXT_CATEGORY,
 'INFO_TEXT_CATEGORY1'=>get_drop_down_list1(ARTICLE_CATEGORY_TABLE,'name="TR_category"',"","",$category_id),
 'INFO_TEXT_AUTHOR'=>INFO_TEXT_AUTHOR,
 'INFO_TEXT_AUTHOR1'=>tep_draw_input_field('TR_author',$author,'size="45"',true),
 'INFO_TEXT_SHOW_DATE'=>INFO_TEXT_SHOW_DATE,
 'INFO_TEXT_SHOW_DATE1'=>datelisting($show_date, 'name="TR_date"', 'name="TR_month"', 'name="TR_year"', "2004", date("Y")+1, true),
 'INFO_TEXT_ARTICLE_PHOTO'=>INFO_TEXT_ARTICLE_PHOTO,
 'INFO_TEXT_ARTICLE_PHOTO1'=>tep_draw_file_field("article_photo").$artcle_photo1,

 'INFO_TEXT_DESCRIPTION'=>INFO_TEXT_DESCRIPTION,
 'INFO_TEXT_DESCRIPTION1'=>tep_draw_textarea_field('description1', 'soft', '80', '20', stripslashes($description), '', '', true),
 'INFO_TEXT_SHORT_DESCRIPTION'=>INFO_TEXT_SHORT_DESCRIPTION,
 'INFO_TEXT_SHORT_DESCRIPTION1'=>tep_draw_textarea_field('TR_short_description', 'soft', '110', '10', stripslashes($short_description), '', '', true),
 'INFO_TEXT_SHOW'=>INFO_TEXT_SHOW,
 'INFO_TEXT_SHOW1'=>tep_draw_radio_field('show', 'Yes', '', $is_show, 'id="radio_show1"').'&nbsp;<label for="radio_show1">Yes</label>&nbsp;'.tep_draw_radio_field('show', 'No', '', $aInfo->is_show, 'id="radio_show2"').'&nbsp;<label for="radio_show2">No</label>',
...
else
{
 $template->assign_vars(array(
  ...
  'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',
 ));

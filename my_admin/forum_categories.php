...
if($action!="")
{
 switch ($action)
  ...
  case 'save1':
   ...
   $es_category_name=tep_db_prepare_input($_POST['TR_es_forum_category_name']);
   ...
   if($action=='insert')
	{
	 ...
	 elseif($row_chek=getAnyTableWhereData(FORUM_CATEGORIES_TABLE,"es_category_name='".tep_db_input($es_category_name)."'",'id'))
	  {
	   $messageStack->add(MESSAGE_NAME_ERROR, 'error');
	  }
	 else
	...
	else if($action=='save')
	 {
	  ...
	  else if($row_chek=getAnyTableWhereData(FORUM_CATEGORIES_TABLE,"es_category_name='".tep_db_input($es_category_name)."' and id!='$id'",'id'))
	   {
		$messageStack->add(MESSAGE_NAME_ERROR, 'error');
		$action='edit';
	   }
...
///////////// Middle Values
$forum_category_query_raw="select id, category_name,es_category_name from " . FORUM_CATEGORIES_TABLE ." order by category_name asc";
...
if(tep_db_num_rows($forum_category_query) > 0)
{
 ...
 while ($forum_category = tep_db_fetch_array($forum_category_query))
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_FORUM_CATEGORIES . '?page='.$_GET['page'].'&id=' . $cInfo->id . '&action=edit\'"';
  }
  else
  {
  ...
  else
  {
   ...
   $category_name=$forum_category['category_name'];
  		///french//
   $es_category_name=$forum_category['es_category_name'];
...
switch ($action)
{
 ...
 case 'insert':
	...
	$contents[] = array('text' => '<br>'.TEXT_INFO_FR_FORUM_CATEGORY_NAME.'<br>'.tep_draw_input_field('TR_es_forum_category_name', ($action=='insert'?$_POST['TR_es_forum_category_name']:''), '' ));
	$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FORUM_CATEGORIES).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a></form>');
	break;
 case 'edit':
 case 'save':
  $heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_FORUM_CATEGORY.'</b>');
  ...
  $contents[] = array('text' => '<br>'.TEXT_INFO_FR_FORUM_CATEGORY_NAME.'<br>'.tep_draw_input_field('TR_es_forum_category_name', ($action=='save'?$_POST['TR_es_forum_category_name']:$cInfo->es_category_name), '' ));
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FORUM_CATEGORIES).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FORUM_CATEGORIES, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

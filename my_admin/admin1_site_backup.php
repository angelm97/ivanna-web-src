...
if ( ($action != 'backup') && (isset($dir)) ) 
 $backup_buttons='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_BACKUP, 'action=backup') . '">'.tep_image_button(PATH_TO_BUTTON.'button_backup.gif',IMAGE_BACKUP).'</a>'; 
...
switch ($action) 
{
 case 'backup':
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit(PATH_TO_BUTTON.'button_backup.gif', IMAGE_BACKUP) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_BACKUP) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
 break;
 case 'delete':

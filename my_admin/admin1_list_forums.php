...
///only for sorting ends
 ...
 if(tep_db_num_rows($forum_query) > 0)
 {
  ...
  while ($forum = tep_db_fetch_array($forum_query))
  {
   ...
   {
    $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_LIST_OF_FORUMS . '?page='.$_GET['page'].'&id=' . $aInfo->id . '&action=edit\'"';
   }
   else
   {
...
elseif($action=='new' || $action=='edit' || $action=='back')
{
 ...
 'INFO_TEXT_CATEGORY'     => INFO_TEXT_CATEGORY,
 'INFO_TEXT_CATEGORY1'    => LIST_SET_DATA(FORUM_CATEGORIES_TABLE,"",TEXT_LANGUAGE.'category_name','id',TEXT_LANGUAGE."category_name",'name="TR_category"',INFO_TEXT_PLEASE_SELECT."...",'',$category_id),
 'INFO_TEXT_AUTHOR'       => INFO_TEXT_AUTHOR,
 ...
 'INFO_TEXT_DESCRIPTION'  => INFO_TEXT_DESCRIPTION,
 'INFO_TEXT_DESCRIPTION1' => tep_draw_textarea_field('TR_description', 'soft', '90', '4', stripslashes($description), '', '', true),
 'INFO_TEXT_SHOW'         => INFO_TEXT_SHOW,
...
else
{
 $template->assign_vars(array(
  ...
  'new_button'                    => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUMS, 'action=new') . '">'.tep_button('New','class="btn btn-primary"').'</a>&nbsp;&nbsp;',
 ));

 ...
 if($boxnum > 0)
 {
  ...
  while ($i < $boxnum) 
  {
   ...
   if ( (is_object($cInfo)) && ($boxes[$i]['admin_boxes_id'] == $cInfo->admin_boxes_id) ) 
   {
    ...
    {
     $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILES, 'cID=' . $boxes[$i]['admin_boxes_id']) . '\'"';
    } 
    else 
    {
     $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILES, 'cPath=' . $boxes[$i]['admin_boxes_id'] . '&action=store_file') . '\'"';
    }
   } 
   else 
   {
...
{  
 case 'store_file': 
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILES, 'cPath=' . $_GET['cPath']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');    
 break;
 case 'remove_file': 
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM) . ' <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILES, 'cPath=' . $_GET['cPath'] . '&fID=' . $_GET['fID']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');    
 break;
 default:
  if (is_object($cInfo)) 
  {
   ...
   else 
   {
    ...
    $contents[] = array('align' => 'center', 'text' => tep_image_submit(PATH_TO_BUTTON.'button_admin_files.gif', IMAGE_INSERT_FILE) );
...
if ($_GET['fID'] || $_GET['cPath']) 
{
 $template->assign_vars(array(
  ...
  'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILES, 'cID=' . $_GET['cPath']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_back.gif', IMAGE_BACK) . '</a>&nbsp<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FILES, 'cPath=' . $_GET['cPath'] . '&action=store_file') . '">' . tep_image_button(PATH_TO_BUTTON.'button_admin_files.gif', IMAGE_INSERT_FILE) . '</a>&nbsp;',
  'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,

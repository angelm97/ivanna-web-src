...
$action = (isset($_GET['action']) ? $_GET['action'] : '');
    ...
    switch ($action) {
      ...
      case 'delete':
        ...
        if ($oID == DEFAULT_ORDERS_STATUS_ID) {
          ...
          $history_query = tep_db_query("select count(*) as count from " . ORDER_STATUS_TABLE_HISTORY . " where orders_status_id = '" . (int)$oID . "'");
  ...
  if(tep_db_num_rows($orders_status_query) > 0)
  {
   ...
   while ($orders_status = tep_db_fetch_array($orders_status_query)) 
   {
    ...
    {
     $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_ORDERS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->orders_status_id . '&action=edit') . '\'"';
    } 
    else 
    {

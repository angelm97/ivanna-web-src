...
if($action=='update' ||$action=='edit')
{
 ...
 $feed_button   = '<a href="'.FILENAME_ADMIN1_LIST_OF_FORUM_POST.'">'.tep_button('Cancel','class="btn btn-primary"').'</a> <button class="btn btn-primary" type="submit">Update</button>';
 }
 ...
 $template->assign_vars(array(
  ...
  'TEXT_INFO_POST_FORUM1' => LIST_SET_DATA(FORUM_TABLE,"",'title','id',"title",'name="TR_forum"',"",'',$forum_id),
  'TEXT_INFO_POST_DESCRIPTION' => TEXT_INFO_POST_DESCRIPTION,
  'TEXT_INFO_POST_DESCRIPTION1'=> tep_draw_textarea_field('description1', 'soft', '100', '20', stripslashes($description), '', '', true),
...
else
{
 ...
 switch ($action)
 {
  case 'delete':
   ...
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUM_POST, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete'.$query_string) . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUM_POST, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].$query_string) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
  ...
  default:
   if (isset($fInfo) && is_object($fInfo))
   {
    ...
    $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUM_POST, 'page=' . $_GET['page'] .'&id=' . $fInfo->id .'&action=edit'.$query_string) . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUM_POST, 'page=' . $_GET['page'] .'&id=' . $fInfo->id. '&action=delete'.$query_string) . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUM_REPLY,'search_post=' . $fInfo->id ) . '" target="left">'.tep_image_button(PATH_TO_BUTTON.'post_reply1.gif',IMAGE_POST_REPLY).'</a>');
    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUM_POST, 'page=' . $_GET['page'] .'&id=' . $fInfo->id .'&action=delete_reply'.$query_string) . '" onclick="return confirm(\''.tep_db_output(INFO_TEXT_DELETE_REPLY_TEXT).'\')" >'.tep_db_output(INFO_TEXT_DELETE_REPLY).'</a>');
 ...
 $template->assign_vars(array(
  ...
  'INFO_TEXT_SEARCH_POST1' =>tep_draw_input_field('search_post',$search_post,'size="3"'),
  'INFO_TEXT_SEARCH_FORUM' =>INFO_TEXT_SEARCH_FORUM,
  'INFO_TEXT_SEARCH_FORUM1'=>tep_draw_input_field('search_forum',$search_forum,'size="3"'),
  'INFO_TEXT_SEARCH_EMAIL' =>INFO_TEXT_SEARCH_EMAIL,
  'INFO_TEXT_SEARCH_EMAIL1'=> tep_draw_input_field('TNEF_email',$email_address,'size="25"'),

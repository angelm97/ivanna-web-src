...
{ 
 case "edit_account":
  if($obj_recruiter_account->check_status()==true)
  {
   ...
   $heading[] = array('text' => '<b>&nbsp;' . tep_db_output($rInfo->recruiter_name) . '</b>');
   $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_COMPANY.'</b><br>&nbsp;'.tep_db_output($rInfo->recruiter_company_name));
   $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_STARTS.'</b><br>&nbsp;' . datelisting(date("Y-m-d",mktime(0,0,0, substr($rInfo->start_date,5,2), substr($rInfo->start_date,8,2), substr($rInfo->start_date,0,4))), "name='TR_date'", "name='TR_month'", "name='TR_year'", "2004", date("Y")+4));
   $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_ENDS.'</b><br>&nbsp;' . datelisting(date("Y-m-d",mktime(0,0,0, substr($rInfo->end_date,5,2), substr($rInfo->end_date,8,2), substr($rInfo->end_date,0,4))), "name='TR_Date'", "name='TR_Month'", "name='TR_Year'", "2004", date("Y")+4));
   if($plan_for=='job_post')
   {
    $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_JOB_STATUS.'</b><br>&nbsp;' . tep_draw_radio_field("TR_job_status", 'Yes', ($rInfo->recruiter_job_status=='Yes'?true:false)).'&nbsp;Yes&nbsp;'.tep_draw_radio_field("TR_job_status", 'No', ($rInfo->recruiter_job_status=='No'?true:false)).'&nbsp;No' );
    $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_NO_OF_JOBS.'</b><br>&nbsp;' . tep_draw_input_field('no_of_jobs', ($unlimited_job?'':$rInfo->recruiter_job), 'size="5" maxlength="5"'.($unlimited_job?' disabled':'')).'&nbsp;&nbsp;'.tep_draw_checkbox_field('ch_no_of_jobs', '',($unlimited_job?true:false),'','id="check_ch_no_of_jobs" onclick="unlimited();"').'&nbsp;<label for="check_ch_no_of_jobs">Unlimited</label>');
				$featured_job_array   = array();
    $featured_job_array[] = array('id'=>'No','text'=>'No');
    $featured_job_array[] = array('id'=>'Yes','text'=>'Yes');

    $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_F_JOBS.'</b><br>&nbsp;' . tep_draw_pull_down_menu('featured_job', $featured_job_array, $featured_job,''));

   }
   else
   {
    $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_CV_STATUS.'</b><br>&nbsp;' . tep_draw_radio_field("TR_cv_status", 'Yes', ($rInfo->recruiter_cv_status=='Yes'?true:false)).'&nbsp;Yes&nbsp;'.tep_draw_radio_field("TR_cv_status", 'No', ($rInfo->recruiter_cv_status=='No'?true:false)).'&nbsp;No' );
    $contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_NO_OF_CVS.'</b><br>&nbsp;' . tep_draw_input_field('no_of_days', ($unlimited_cv?'':$cv), 'size="5" maxlength="5"'.($unlimited_cv?' disabled':'')).'&nbsp;&nbsp;'.tep_draw_checkbox_field('ch_no_of_days', '',($unlimited_cv?true:false),'','id="check_ch_no_of_days" onclick="unlimited();"').'&nbsp;<label for="check_ch_no_of_days">Unlimited</label>');
   }
   //$contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_SMS_STATUS.'</b><br>&nbsp;' . tep_draw_radio_field("TR_sms_status", 'Yes', ($rInfo->recruiter_sms_status=='Yes'?true:false)).'&nbsp;Yes&nbsp;'.tep_draw_radio_field("TR_sms_status", 'No', ($rInfo->recruiter_sms_status=='No'?true:false)).'&nbsp;No' );
   //$contents[] = array('text' => '<b>&nbsp;'.TEXT_INFO_ACCOUNT_NO_OF_SMS.'</b><br>&nbsp;' . tep_draw_input_field('no_of_sms', ($unlimited_sms?'':$rInfo->recruiter_sms), 'size="5" maxlength="5"'.($unlimited_sms?' disabled':'')).'&nbsp;&nbsp;'.tep_draw_checkbox_field('ch_no_of_sms', '',($unlimited_sms?true:false),'','id="check_ch_no_of_sms" onclick="unlimited();"').'&nbsp;<label for="check_ch_no_of_sms">Unlimited</label>');
   $contents[] = array('align' => 'center', 'text' => tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ACCOUNTS,'rID='.$rInfo->recruiter_id ) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a><br>&nbsp;');
  }
 break;
 default:
  ...
  {
   $heading[] = array('text' => '<b>&nbsp;' . tep_db_output($rInfo->recruiter_name) . '</b>');
   $contents[] = array('align' => 'center', 'text' => TEXT_INFO_EDIT_ACCOUNT_INTRO);
   $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ACCOUNTS, 'rID=' . $rInfo->recruiter_id . '&action=edit_account&plan_for='.$plan_for) . '">' . tep_image_button(PATH_TO_BUTTON.'button_edit.gif', IMAGE_EDIT) . '</a>');// <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ACCOUNTS, 'rID=' . $rInfo->recruiter_id . '&action=del_account') . '">' . tep_image_button(PATH_TO_BUTTON.'button_delete.gif', IMAGE_DELETE) . '</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ACCOUNTS, 'rID='.$rInfo->recruiter_id.'&action=preview') . '">' . tep_image_button(PATH_TO_BUTTON.'button_preview.gif', IMAGE_PREVIEW) . '</a><br>&nbsp;');
  }
  else
  {
   $heading[] = array('text' => '<b>&nbsp;' . tep_db_output($rInfo->recruiter_name) . '</b>');
   $contents[] = array('text' => TEXT_INFO_NEW_ACCOUNT_INTRO);
   //$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ACCOUNTS, 'rID=' . $rInfo->recruiter_id ) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a><br>&nbsp;');
  }
  break;
}
////

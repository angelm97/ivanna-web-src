...
if(tep_db_num_rows($experience_query) > 0)
{
 ...
 while ($experience = tep_db_fetch_array($experience_query)) 
 {
  ...
  {
   $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_ADMIN_EXPERIENCE . '?page='.$_GET['page'].'&id=' . $cInfo->id . '&action=edit\'"';
  } 
  else 
  {
...
switch ($action) 
{
 ...
 case 'insert':
 case 'save':
		$heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_EXPERIENCE.'</b>');
  $contents = array('form' => tep_draw_form('experience', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_MIN_AGE.'<br>'.tep_draw_input_field('TR_min_experience', $_POST['TR_min_experience'], '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_MAX_AGE.'<br>'.tep_draw_input_field('TR_max_experience', $_POST['TR_max_experience'], '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_EXPERIENCE_PRIORITY.'<br>'.tep_draw_input_field('IN_priority', $_POST['IN_priority'], '' ));
		$contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  break;
 case 'edit':
  ...
  $contents = array('form' => tep_draw_form('experience', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'id=' . $cInfo->id.'&page='.$_GET['page'].'&action=save','post',' onsubmit="return ValidateForm(this)"'));
		...
		$contents[] = array('text' => '<br>'.TEXT_INFO_MIN_AGE.'<br>'.tep_draw_input_field('TR_min_experience', $cInfo->min_experience, '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_MAX_AGE.'<br>'.tep_draw_input_field('TR_max_experience', $cInfo->max_experience, '' ));
		$contents[] = array('text' => '<br>'.TEXT_INFO_EXPERIENCE_PRIORITY.'<br>'.tep_draw_input_field('IN_priority', $cInfo->priority, '' ));
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'gid=' . $_GET['gid'] . '&id=' . $cInfo->id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
  break;
 case 'delete':
   ...
   if($minage*12==(int)$maxage)
    ...
    $maxage=($maxage >=12?(($maxage/12) > 1?($maxage/12).' years':($maxage/12)." year"):($maxage>1?$maxage.' months':$maxage.' month'));
  $heading[] = array('text' => '<b>' . $minage.' - '.$maxage . '</b>');
  ...
  $contents[] = array('text' => '<br><b>' . $minage.' - '.$maxage . '</b>');
  $contents[] = array('align' => 'center', 'text' => '
  <table>
   <tr>
   <td>       
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'page=' . $_GET['page'] . '&id=' . $_GET['id']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>
   </td>
   </tr>
   </table>
  ');
 ...
 default:
  if (isset($cInfo) && is_object($cInfo)) 
   ...
   if($minage*12==(int)$maxage)
    ...
    $maxage=($maxage >=12?(($maxage/12) > 1?($maxage/12).' years':($maxage/12)." year"):($maxage>1?$maxage.' months':$maxage.' month'));
   $heading[] = array('text' => '<b>'.TEXT_INFO_HEADING_EXPERIENCE.'</b>');
   $contents[] = array('text' => $minage.' - '.$maxage);
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'page=' . $_GET['page'] .'&id=' . $cInfo->id . '&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'page=' . $_GET['page'] .'&id=' . $cInfo->id . '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EXPERIENCE, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',

...
include_once(PATH_TO_MAIN_ADMIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_ADMIN1_IMPORT_JOBSEEKER);
$template->set_filenames(array('user_import' => 'admin1_import_jobseeker.htm'));
   ...
   if(!$error)
   {
    ...
    switch($file_type)
    {
     case 'csv' :
      ...
      if(is_array($content))
      {
       $message = import_user_jobseeker($content);
	   if($message)
	   {
      ...
      tep_redirect(tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_IMPORT_JOBSEEKER));
      break;
    }
   }
   break;
...
$template->assign_vars(array(
 ...
 'import_form'           => tep_draw_form('user_import',PATH_TO_ADMIN.FILENAME_ADMIN1_IMPORT_JOBSEEKER,'', 'post','enctype="multipart/form-data" onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','add'),

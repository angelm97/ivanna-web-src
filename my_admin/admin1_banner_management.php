...
if(tep_db_num_rows($banner_query) > 0)
{
 ...
 while ($banner = tep_db_fetch_array($banner_query)) 
 {
  ...
  if ( (isset($bInfo) && is_object($bInfo)) && ($banner['id'] == $bInfo->id) ) 
  {
			...
			{
				$row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" ';// onclick="document.location.href=\'' . FILENAME_ADMIN1_BANNER_MANAGEMENT . '?page='.$_GET['page'].'&bID=' . $bInfo->id . '&b_status='.$_GET['b_status'].'&action=edit\'"
			}
			else
			{
				$row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" ';//onclick="document.location.href=\'' . FILENAME_ADMIN1_BANNER_MANAGEMENT . '?page='.$_GET['page'].'&bID=' . $bInfo->id . '&b_status='.$_GET['b_status'].'\'"
			}
  } 
  else 
  {
  ...
  $location='<tr><td valign="top" class="dataTableContentbold" colspan="6">'.tep_db_output($banner['banner_location_name']).'</td></tr>';
  $view='';
...
{ 
   ...
   if ($error) 
   {
    ...
    $contents[] = array('text' => '<br>&nbsp;' . INFO_TEXT_BANNER_SIZE .  $imgsize[0].' x '.$imgsize[1]); 
	   $contents[] = array('text' => '<br>&nbsp;' .INFO_TEXT_BANNER_TYPE.'<br>&nbsp;'. tep_draw_radio_field("TR_banner_type", 'image','',$banner_type,'onclick="set_type()"').'&nbsp;Image&nbsp;'.tep_draw_radio_field("TR_banner_type",'script', '',$banner_type,'onclick="set_type()"').'&nbsp;Script' );
				$contents[]=array('text' => '<br>'.INFO_TEXT_BANNER_HREF.'<br>'.tep_draw_input_field('banner_href', $href, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_BANNER_ALT.'<br>'.tep_draw_input_field('banner_alt', $alt, '' ));
    ...
    $contents[]=array('text' => '<br>'.INFO_TEXT_COMPANY_CONTACT.'<br>'.tep_draw_input_field('company_contact', $company_contact, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_COMPANY_EMAIL.'<br>'.tep_draw_input_field('TNEF_company_email', $company_email, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_BANNER_COSTING.'<br>'.tep_draw_input_field('banner_costing', $banner_costing, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_COMPANY_COMMENTS.'<br>'.tep_draw_textarea_field('company_comments',1,30,2, $company_comments, '' ));
    $contents[]=array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
   }
   else 
   {
    //$contents[] = array('text' => tep_draw_hidden_field('TR_banner_src', $bInfo->src));
    $contents[] = array('text' => '<br>&nbsp;' . INFO_TEXT_BANNER_SIZE .  $imgsize[0].' x '.$imgsize[1]); 
	   $contents[] = array('text' => '<br>&nbsp;' .INFO_TEXT_BANNER_TYPE.'<br>&nbsp;'. tep_draw_radio_field("TR_banner_type", 'image','',$banner_type,'onclick="set_type()"').'&nbsp;Image&nbsp;'.tep_draw_radio_field("TR_banner_type",'script', '',$banner_type,'onclick="set_type()"').'&nbsp;Script' );
				$contents[]=array('text' => '<br>'.INFO_TEXT_BANNER_HREF.'<br>'.tep_draw_input_field('banner_href', $bInfo->href, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_BANNER_ALT.'<br>'.tep_draw_input_field('banner_alt', $bInfo->alt, '' ));
	   $contents[]=array('text' => '<br><div id=image>'.INFO_TEXT_BANNER.'<br>'.tep_draw_file_field("banner").$src1.'</div>');
	   $contents[]=array('text' => '<br><div id=script>'.INFO_TEXT_BANNER_SCRIPT.'<br>'.tep_draw_textarea_field('banner_script','','30','4',$bInfo->script,'',"",false).'</div>');
    ...
    $contents[]=array('text' => '<br><hr><br><font size=2><b>Company Information</b></font><br>'.INFO_TEXT_COMPANY_NAME.'<br>'.tep_draw_input_field('company_name', $bInfo->company_name, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_COMPANY_CONTACT.'<br>'.tep_draw_input_field('company_contact', $bInfo->company_contact, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_COMPANY_EMAIL.'<br>'.tep_draw_input_field('TNEF_company_email', $bInfo->company_email, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_BANNER_COSTING.'<br>'.tep_draw_input_field('banner_costing', $bInfo->banner_costing, '' ));
    $contents[]=array('text' => '<br>'.INFO_TEXT_COMPANY_COMMENTS.'<br>'.tep_draw_textarea_field('company_comments',1,30,2, $bInfo->company_comments, '' ));
    $contents[]=array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
   }
		}
  break;
 case 'delete':
  ...
  <table>
   <tr>
   <td>       
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT, 'page=' . $_GET['page'] . '&bID=' . $bInfo->id.'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>
   </td>
   <td>
   <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT, 'page=' . $_GET['page'] ) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>
   </td>
   </tr>
   </table>
  ');
 ...
 default:
	...
	if ($_GET['b_status']!='deleted')
	{
		if (isset($bInfo) && is_object($bInfo)) 
		{
			...
			$contents[] = array('align' => 'center', 'text' => '<br>'.'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT, 'bID=' . $bInfo->id.'&action=edit&b_status='.$_GET['b_status'].'&page='.$_GET['page']) . '">'.tep_image(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;'.'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT, 'bID=' . $bInfo->id.'&action=delete&b_status='.$_GET['b_status'].'&page='.$_GET['page']) . '">'.tep_image(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>&nbsp;');
		} 
	}
	else // b_status=deleted
	{
		if (isset($bInfo) && is_object($bInfo)) 
		{
			$heading[] = array('text' => '<b>&nbsp;' . tep_db_output($bInfo->title) . '</b>');
			$contents[] = array('align' => 'center', 'text' => TEXT_INFO_EDIT_INTRO);			
			$contents[] = array('align' => 'center', 'text' => '<br>'.'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT, 'bID=' . $bInfo->id.'&action=restore&b_status='.$_GET['b_status'].'&page='.$_GET['page']) . '">'.tep_image(PATH_TO_BUTTON.'button_restore.gif',IMAGE_RESTORE).'</a>&nbsp;'.'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_MANAGEMENT, 'bID=' . $bInfo->id.'&action=purge&b_status='.$_GET['b_status'].'&page='.$_GET['page']) . '">'.tep_image(PATH_TO_BUTTON.'button_delete.gif',IMAGE_PURGE).'</a>&nbsp;');
			}
	}
}

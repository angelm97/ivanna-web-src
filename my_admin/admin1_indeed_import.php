...
if($action=='new' || $action=='insert' || $action=='update' ||$action=='edit' ||$action=='save')
{
 ...
 if(($action=='update' || $action=='edit') && $feed_id>0)
 {
  ...
  $feed_button   = '<a href="'.FILENAME_ADMIN1_INDEED_FEED_IMPORT.'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a> '.tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);  
 }
 else
 {
  ...
  $feed_button   = '<a href="'.FILENAME_ADMIN1_INDEED_FEED_IMPORT.'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a> '.tep_image_submit(PATH_TO_BUTTON.'button_insert.gif', IMAGE_INSERT);
 }
/////////////////////////////////////////////////////////////
 ...
 $template->assign_vars(array(
  ...
  'TEXT_INFO_PUBLISHER_ID1' => tep_draw_input_field('TR_publisher_id',$publisher_id,'',true), 
  ...
  'TEXT_INFO_CHANNEL1'      => tep_draw_input_field('channel',$channel), 
...
else
{
 ...
 if(tep_db_num_rows($indeed_feeds_query) > 0)
 {
  ...
  while ($indeed_feeds = tep_db_fetch_array($indeed_feeds_query)) 
  {
   ...
   {
    $row_selected=' id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . FILENAME_ADMIN1_INDEED_FEED_IMPORT . '?page='.$_GET['page'].'&id=' . $fInfo->feed_id . '&action=edit\'"';
   } 
   else 
   {
 ...
 switch ($action) 
 {
  case 'delete':
   ...
   $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_INDEED_FEED_IMPORT, 'page=' . $_GET['page'] . '&id=' . $_GET['id'].'&action=confirm_delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_INDEED_FEED_IMPORT, 'page=' . $_GET['page'] . '&id=' . $_GET['id']) . '">' . tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL) . '</a>');
  break;
  default:
   if (isset($fInfo) && is_object($fInfo)) 
   {
    ...
    $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_INDEED_FEED_IMPORT, 'page=' . $_GET['page'] .'&id=' . $fInfo->feed_id .'&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif',IMAGE_EDIT).'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_INDEED_FEED_IMPORT, 'page=' . $_GET['page'] .'&id=' . $fInfo->feed_id. '&action=delete') . '">'.tep_image_button(PATH_TO_BUTTON.'button_delete.gif',IMAGE_DELETE).'</a>');
 ...
 $template->assign_vars(array(
  ...
  'new_button'=>'<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_INDEED_FEED_IMPORT, 'page=' . $_GET['page'] .'&action=new') . '">'.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;',
  'HEADING_TITLE'=>HEADING_TITLE,
  'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,

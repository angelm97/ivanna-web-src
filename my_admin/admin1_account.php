...
if ($action=='edit_process')
{
 ...
 {
  $buttons.=tep_draw_submit_button_field('','Save','class="btn btn-primary margins"');//tep_image_submit(PATH_TO_BUTTON.'button_save.gif', IMAGE_SAVE);
 }
}
elseif ($action == 'check_account')
{
...
$contents = array();
  switch ($action) {
    case 'edit_process':
      ...
      //$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ACCOUNT) . '">' . tep_image('button_back.gif', IMAGE_BACK) . '</a> ' . tep_image_submit('button_confirm.gif', IMAGE_CONFIRM, 'onClick="validateForm();return document.returnValue"') . '<br>&nbsp');
      break;
    case 'check_account':
      ...
      $contents[] = array('align' => 'center', 'text' => '<a style="display: flex;margin: 10px 0 0 0;" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ACCOUNT) . '">' . tep_button('Back','class="btn btn-secondary"') . '</a> ' . tep_draw_submit_button_field('','Confirm','class="btn btn-primary margins"') . '<br>&nbsp');
      break;
    default:
      $heading[] = array('text' => '<b>&nbsp;' . TEXT_INFO_HEADING_DEFAULT . '</b>');

      $contents[] = array('text' => TEXT_INFO_INTRO_DEFAULT);
      $contents[] = array('align' => 'center', 'text' => tep_draw_submit_button_field('','Edit','class="btn btn-primary"') . '<br>&nbsp');

 ...
 case 'edit':
  ...
  else
  {
   if($cInfo->configuration_name=='DEFAULT_SITE_LOGO')
   {
    ...
    if(file_exists(PATH_TO_MAIN_PHYSICAL.PATH_TO_IMG.DEFAULT_SITE_LOGO)&& tep_not_null(DEFAULT_SITE_LOGO))
    $value_field .= '<br>'.tep_image(PATH_TO_IMG.DEFAULT_SITE_LOGO);
   }
///////////////////ADDED CODE/////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////
  ...
  $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_MODULE, 'id=' . $cInfo->id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
  break;
 default:
  if (isset($cInfo) && is_object($cInfo))
		{
   $heading[] = array('text' => '<b>' . tep_db_output($cInfo->configuration_title) . '</b>');
   $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_MODULE, 'id=' . $cInfo->id . '&action=edit') . '">'.tep_image_button(PATH_TO_BUTTON.'button_edit.gif', IMAGE_EDIT).'</a>');

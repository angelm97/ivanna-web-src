...
for($i=0;$i<$count_job;$i++)
{
 ...
 if($row=getAnyTableWhereData($table_names,$where_clause,$field_names))
 {
  ...
  $display_job_title.="<li class='text-danger'>".tep_db_output($row['job_title'])."</li>";
...
if($db_resume_name_num_row > 0)
{
 while ($resume_name = tep_db_fetch_array($db_resume_name_query))
 {
  ...
  $resume_names_array[]=tep_draw_radio_field('my_resume_id', $ide, '', $ide, 'id="radio_resume_name'.$ide.'"').'&nbsp;<label for="radio_resume_name'.$ide.'" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'">'.'<a href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME."?resume=".$query_string1).'" target="_resume">'.tep_db_output($resume_name['resume_title']).'</a></label>&nbsp;';
 }
}
else
{
...
//  'button'=>tep_image_submit(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM),
 'button'=>tep_button_submit('btn btn-primary', 'Confirm'),
 'screeners'=>$screeners,
 'INFO_TEXT_DEFALUT_COVER_LETTER1'=>sprintf("<a href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_COVER_LETTERS, 'action=add_new')."' class='red'>".INFO_TEXT_ADD_NEW_COVER_LETTER."</a>"),
